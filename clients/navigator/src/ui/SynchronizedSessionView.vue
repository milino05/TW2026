...
function applySharedPlayback() {
  ...
  if (ttsState.value === "paused") browserTts.resume();
    ...
    notice.value = "La guida ha avviato l’ascolto.";
  }
}
...
<template>
  <main :class="group?.synchronizedSession.status === 'active' ? 'session-experience' : 'together-page'">
    <p v-if="busy" class="full-state">Sto preparando la visita…</p>
    <p v-else-if="error && !group" class="full-state error" role="alert">{{ error }}</p>
    <template v-else-if="group && runtime">
      ...
      <header v-else class="session-header">
        ...
        <button v-if="isHost" class="group-control-button" type="button" :aria-expanded="groupPanelOpen" @click="groupPanelOpen = true">
      ...
      <p v-else-if="notice" class="inline-notice" role="status" aria-live="polite">{{ notice }}</p>
      ...
      <section v-if="isHost && cancelAction && group.synchronizedSession.status !== 'active'" class="cancel-session" aria-labelledby="cancel-session-title">
        ...
        <template v-else>
          <div><strong id="cancel-session-title">Vuoi annullare la sessione?</strong><p>I partecipanti non potranno più rientrare con questo alias.</p></div>
          <div class="cancel-session__actions"><button type="button" :disabled="actionBusy" @click="confirmingCancel = false">Continua la visita</button><button class="danger" type="button" :disabled="actionBusy" @click="cancelSession">{{ actionBusy ? 'Annullamento…' : 'Sì, annulla' }}</button></div>
        </template>
      </section>
      ...
      <div v-if="groupPanelOpen && isHost" class="modal-overlay group-overlay" @click.self="groupPanelOpen = false">
        <section class="group-sheet" role="dialog" aria-modal="true" aria-labelledby="group-panel-title">
          <header>
            ...
            <button type="button" aria-label="Chiudi controllo del gruppo" @click="groupPanelOpen = false">×</button>
          </header>
          ...
          <div class="group-danger">
            <button v-if="!confirmingCancel" type="button" :disabled="interactionBusy" @click="confirmingCancel = true">Annulla questa sessione</button>
            <template v-else>
              <p>La sessione verrà chiusa per tutti i partecipanti.</p>
              <div><button type="button" @click="confirmingCancel = false">Indietro</button><button class="danger" type="button" :disabled="interactionBusy" @click="cancelSession">Conferma annullamento</button></div>
            </template>
          </div>
        </section>
      </div>
...
.inline-notice{padding:.7rem 1rem;border:1px solid color-mix(in srgb,var(--navigator-brand-primary) 28%,var(--navigator-border));border-radius:.8rem;background:color-mix(in srgb,var(--navigator-brand-primary) 7%,var(--navigator-surface-raised));color:var(--navigator-text)}
...
.modal-overlay{position:fixed;z-index:50;inset:0;display:grid;align-items:end;justify-items:center;padding-top:4rem;background:rgba(7,12,11,.52)}
