...
import {
  ...
  type MapProjection,
  type SelectableLocationProjection,
...
import SessionRuntimeState from "./SessionRuntimeState.vue";
import SessionStopsPanel from "./SessionStopsPanel.vue";

type InteractionChannel = "button" | "controlled_voice";
type ActiveView = "content" | "map" | "stops";
type LocationSelectionMode = "confirm" | "correct" | null;
...
const map = ref<MapProjection | null>(null);
const activeView = ref<ActiveView>("content");
const locationSelectionMode = ref<LocationSelectionMode>(null);
const busy = ref(true);
...
watch(() => runtime.value?.experience?.phase, (phase, previous) => {
  if (!phase || phase === previous) return;
  if (phase === "location_required") {
    activeView.value = "map";
    locationSelectionMode.value = "confirm";
  } else if (["navigating_to_visit_stop", "navigating_detour"].includes(phase)) {
    activeView.value = "map";
    if (locationSelectionMode.value === "confirm") locationSelectionMode.value = null;
  } else if (["approaching_visit_target", "presenting_visit_content", "presenting_semantic_content"].includes(phase)) {
    activeView.value = "content";
    locationSelectionMode.value = null;
  }
}, { immediate: true });
...
const groupActions = computed(() => (runtime.value?.availableActions || []).filter((action) =>
  action.runtimeScope === "synchronized_visit_session" || action.family === "synchronization"));
const personalActions = computed(() => (runtime.value?.availableActions || []).filter((action) =>
  action.runtimeScope !== "synchronized_visit_session"
  && !["synchronization", "lifecycle", "quiz"].includes(action.family)));
...
const groupPreviousAction = computed(() => (runtime.value?.availableActions || []).find((action) =>
  action.type === "PROGRESS_PREVIOUS" && action.runtimeScope === "synchronized_visit_session") || null);
const groupNextAction = computed(() => (runtime.value?.availableActions || []).find((action) =>
  action.type === "PROGRESS_NEXT" && action.runtimeScope === "synchronized_visit_session") || null);
const personalNextAction = computed(() => (runtime.value?.availableActions || []).find((action) =>
  action.type === "PROGRESS_NEXT" && action.runtimeScope !== "synchronized_visit_session") || null);
const primaryNextAction = computed(() => personalNextAction.value || (isHost.value ? groupNextAction.value : null));
const stopSelectAction = computed(() => (runtime.value?.availableActions || []).find((action) => action.type === "VISIT_STOP_SELECT") || null);
const locationConfirmAction = computed(() => actionOfType(runtime.value?.availableActions || [], "LOCATION_CONFIRM"));
const locationCorrectAction = computed(() => actionOfType(runtime.value?.availableActions || [], "LOCATION_CORRECT"));
const returnToVisitAction = computed(() => actionOfType(runtime.value?.availableActions || [], "NAVIGATION_RETURN_TO_VISIT"));
...
const currentAnchorId = computed(() => runtime.value?.progress?.contextVisitAnchorId || map.value?.narrativeContextStop?.visitAnchorId || null);
...
const progressLabel = computed(() => {
  ...
  if (current && total) return `Tappa ${current.order} di ${total}`;
  const entry = (group.value?.synchronizedSession.currentEntryIndex || 0) + 1;
  const count = group.value?.synchronizedSession.contentEntryCount || 0;
  return count ? `Contenuto ${entry} di ${count}` : "Visita in corso";
});
const progressPercent = computed(() => {
  const current = map.value?.narrativeContextStop;
  const totalStops = map.value?.venues.flatMap((venue) => venue.stops).length || 0;
  if (current && totalStops) return Math.min(100, (current.order / totalStops) * 100);
...
const participantPlaybackVoiceAction = computed<AvailableAction | null>(() => {
  ...
  if (ttsState.value === "speaking") return { actionId:"local.playback.pause", type:"LOCAL_PLAYBACK_PAUSE", family:"playback", label:"Metti in pausa", controlledVoiceAliases:["metti in pausa","pausa lettura"] };
  if (ttsState.value === "paused") return { actionId:"local.playback.resume", type:"LOCAL_PLAYBACK_RESUME", family:"playback", label:"Riprendi lettura", controlledVoiceAliases:["riprendi lettura","continua a leggere"] };
  return { actionId:"local.playback.play", type:"LOCAL_PLAYBACK_PLAY", family:"playback", label:"Ascolta il contenuto", controlledVoiceAliases:["ascolta il contenuto","avvia la lettura"] };
});
...
function desiredParticipantActivity() {
  if (isHost.value || group.value?.synchronizedSession.status !== "active") return { activity:"inactive", mode:null } as const;
  if (ttsState.value === "speaking") return { activity:"active", mode:"audio" } as const;
  if (document.visibilityState === "visible") return { activity:"active", mode:"reading" } as const;
...
function publishParticipantActivity({ deferInactive = true } = {}) {
  ...
  if (participantInactiveTimer != null) { window.clearTimeout(participantInactiveTimer); participantInactiveTimer = null; }
...
function handleVisibilityChange() { publishParticipantActivity(); }
function currentParticipantRequest(participant: SynchronizedParticipant) {
  ...
  return participant.requests?.find((request) => !contentEntryId || String(request.contentEntryId || "") === String(contentEntryId)) || null;
}
...
async function refresh({ quiet = false } = {}) {
  ...
  try {
    ...
    const visitSessionId = projection.membership.visitSessionId;
    const [current, currentMap] = await Promise.all([
      sessionRepository.current(visitSessionId),
      projection.synchronizedSession.status === "active"
        ? navigationRepository.map(visitSessionId).catch(() => null)
        : Promise.resolve(null),
    ]);
    group.value = projection;
    runtime.value = current;
    if (currentMap) map.value = currentMap;
    publishParticipantActivity({ deferInactive:false });
    if (["quiz", "completed"].includes(projection.synchronizedSession.status)) quiz.value = await synchronizedVisitRepository.quiz(synchronizedSessionId.value);
    error.value = null;
  } catch (cause) {
...
async function dispatch(action: AvailableAction, input: unknown = null, interactionChannel: InteractionChannel = "button") {
  ...
  try {
    ...
    if (["progress", "synchronization"].includes(action.family)) {
      groupPanelOpen.value = false;
...
async function requestPersonalAction(action: AvailableAction) {
  if (action.type === "LOCATION_CONFIRM") {
    locationSelectionMode.value = "confirm";
    activeView.value = "map";
    notice.value = "Seleziona sulla mappa il punto in cui ti trovi.";
    return;
  }
  if (action.type === "LOCATION_CORRECT") {
    locationSelectionMode.value = "correct";
    activeView.value = "map";
    notice.value = "Seleziona sulla mappa la tua posizione attuale.";
    return;
  }
  await dispatch(action);
}

async function selectLocation(location: SelectableLocationProjection["locationRef"]) {
  const action = locationSelectionMode.value === "correct" ? locationCorrectAction.value : locationConfirmAction.value;
  if (!action) return;
  await dispatch(action, { location });
  locationSelectionMode.value = null;
}

async function selectStop(visitAnchorId: string) {
  if (!isHost.value || !stopSelectAction.value) return;
  await dispatch(stopSelectAction.value, { visitAnchorId });
}

function playLocally() {
  const presentation = currentPresentation.value;
  if (!presentation) return;
  if (ttsState.value === "speaking") { browserTts.pause(); return; }
  if (ttsState.value === "paused") { browserTts.resume(); return; }
  if (!browserTts.speak(presentation.text, presentation.locale || "it-IT")) notice.value = "Sintesi vocale non supportata dal browser";
}
async function toggleSpeech() {
  if (isHost.value) { if (playbackPrimaryAction.value) await dispatch(playbackPrimaryAction.value); return; }
  playLocally();
}
async function stopSpeech() {
  if (isHost.value && playbackStopAction.value) { await dispatch(playbackStopAction.value); return; }
  browserTts.stop();
}
...
async function listenControlledVoice() {
  if (!runtime.value || interactionBusy.value) return;
  if (!browserControlledVoice.supported) { notice.value = "Il browser non supporta il riconoscimento vocale; usa i bottoni equivalenti."; return; }
  ...
  try {
    ...
    else if (["LOCATION_CONFIRM", "LOCATION_CORRECT"].includes(result.action.type)) await requestPersonalAction(result.action);
  ...
  } finally { if (requestId === voiceSequence) voiceBusy.value = false; }
}
...
async function submitQuiz() {
  ...
  if (!action) return;
  await dispatch(action, { answers: quiz.value.questions.map((question) => ({ questionId:question.id, selectedOptionIndex:quizAnswers.value[question.id] })) });
}
async function confirmEvaluation(userId: string) {
  ...
  } catch (cause) { error.value = cause instanceof Error ? cause.message : "Valutazione non salvata"; }
  finally { actionBusy.value = false; }
}
...
onMounted(async () => {
  unsubscribeTts = browserTts.subscribe((state) => { ttsState.value = state; publishParticipantActivity(); });
  ...
  <main :class="group?.synchronizedSession.status === 'active' ? 'session-experience' : 'together-page'">
    ...
    <template v-else-if="group && runtime">
      <header v-if="group.synchronizedSession.status !== 'active'" class="together-header">
        <div><p class="eyebrow">Visita sincronizzata</p><h1>{{ group.synchronizedSession.title }}</h1></div>
      ...
      <header v-else class="session-header">
        ...
        <div class="session-brand"><span><strong>{{ group.synchronizedSession.title }}</strong><small>{{ isHost ? "Visita sincronizzata · Guida" : "Visita sincronizzata · Partecipante" }}</small></span></div>
        <button v-if="isHost" class="group-control-button" type="button" aria-label="Apri controllo del gruppo" :aria-expanded="groupPanelOpen" @click="groupPanelOpen = true"><span aria-hidden="true">{{ group.synchronizedSession.participantCount }}</span><em>Gruppo</em></button>
      ...
      <div v-if="group.synchronizedSession.status === 'active'" class="view-tabs" role="tablist" aria-label="Vista visita">
        <button type="button" role="tab" :aria-selected="activeView === 'content'" @click="activeView = 'content'">Visita</button>
        <button type="button" role="tab" :aria-selected="activeView === 'map'" :disabled="!map" @click="activeView = 'map'">Mappa</button>
        <button type="button" role="tab" :aria-selected="activeView === 'stops'" :disabled="!map" @click="activeView = 'stops'">Tappe</button>
      </div>
      ...
      <section v-if="group.synchronizedSession.status === 'lobby'" class="lobby-card">
        <template v-if="isHost">
          <p class="eyebrow">Fai entrare il gruppo</p><h2>Mostra queste parole</h2>
          ...
          <ul v-if="group.participants?.length" class="participant-list">
            <li v-for="participant in group.participants" :key="participant.userId"><span class="avatar">{{ participant.username.slice(0,1).toUpperCase() }}</span><strong>{{ participant.username }}</strong><small>{{ participant.role === 'host' ? 'Guida' : onlineUserIds.has(String(participant.userId)) ? 'Online' : 'Entrato' }}</small></li>
          </ul>
          <button v-for="action in groupActions.filter((entry) => entry.type === 'SYNCHRONIZED_START')" :key="action.actionId" class="primary-action" type="button" :disabled="actionBusy" @click="dispatch(action)">{{ actionBusy ? "Avvio…" : action.label }}</button>
        </template>
        <template v-else>
          <div class="waiting-symbol" aria-hidden="true">✓</div><p class="eyebrow">Sei dentro</p><h2>Aspettiamo la guida</h2><p>La visita inizierà qui. Puoi lasciare aperta questa schermata.</p><div class="waiting-dots" aria-hidden="true"><span></span><span></span><span></span></div>
        </template>
      </section>

      <section v-else-if="group.synchronizedSession.status === 'active'" class="active-session-body">
        <div class="session-scroll">
          <section v-show="activeView === 'content'" class="runtime-panel">
            <article v-if="runtime.current" class="content-panel">
              ...
              <figure v-if="primaryMedia" class="content-media"><img :src="primaryMedia.url" :alt="primaryMedia.altText || ''"></figure>
            ...
            <SessionRuntimeState
              ...
              :snapshot="runtime"
              :map="map"
              :next-action="personalNextAction"
              :return-action="returnToVisitAction"
              :busy="interactionBusy"
              @show-map="activeView = 'map'"
          ...
          <section v-show="activeView === 'stops'" class="stops-panel-wrapper">
            <SessionStopsPanel
              v-if="map"
              :map="map"
              :can-select="Boolean(isHost && stopSelectAction)"
              :busy="interactionBusy"
              @select="selectStop"
            />
            <p v-else>Le tappe non sono disponibili.</p>
          </section>
        </div>

        <nav class="session-bottom" aria-label="Comandi della visita">
          <button v-if="isHost && groupPreviousAction" class="progress-action previous-action" type="button" :disabled="interactionBusy" @click="dispatch(groupPreviousAction)">← {{ groupPreviousAction.label }}</button>
          ...
          <button v-if="primaryNextAction" class="progress-action next-action" type="button" :disabled="interactionBusy" @click="dispatch(primaryNextAction)">{{ primaryNextAction.label }} →</button>
          <span v-else aria-hidden="true"></span>
        </nav>
      </section>
      ...
      <section v-else-if="group.synchronizedSession.status === 'quiz'" class="quiz-card">
        ...
        <template v-else-if="quiz?.role === 'host'">
          ...
          <div class="quiz-results"><article v-for="result in quiz.results" :key="result.userId"><div><strong>{{ result.username }}</strong><small>{{ result.status === 'submitted' ? `${result.score} su ${result.maxScore}` : 'In attesa' }}</small></div><form v-if="result.status === 'submitted'" @submit.prevent="confirmEvaluation(result.userId)"><input v-model="evaluationDrafts[result.userId]" :placeholder="result.evaluation?.value || 'Valutazione facoltativa'"><button type="submit" :disabled="actionBusy">{{ result.evaluation?.confirmedByHost ? 'Aggiorna' : 'Conferma' }}</button></form></article></div>
      ...
      <section v-else class="lobby-card"><div class="waiting-symbol" aria-hidden="true">✓</div><p class="eyebrow">Visita conclusa</p><h2>Grazie per aver partecipato</h2><RouterLink class="primary-link" :to="{ name: 'museums' }">Torna al Navigator</RouterLink></section>

      <section v-if="isHost && cancelAction && group.synchronizedSession.status !== 'active'" class="cancel-session" aria-labelledby="cancel-session-title"><div><strong id="cancel-session-title">Gestione sessione</strong><p>I partecipanti non potranno più rientrare dopo l’annullamento.</p></div><button type="button" :disabled="actionBusy" @click="confirmingCancel = true">Annulla questa sessione</button></section>
      ...
      <Teleport to="body">
        <div v-if="groupPanelOpen && isHost" class="modal-overlay group-overlay" @click.self="groupPanelOpen = false" @keydown="onGroupPanelKeydown">
          <section ref="groupPanel" class="group-sheet" role="dialog" aria-modal="true" aria-labelledby="group-panel-title">
            <header><div><p class="eyebrow">Controllo del gruppo</p><h2 id="group-panel-title">{{ group.synchronizedSession.participantCount }} partecipanti</h2></div><button ref="groupPanelClose" type="button" aria-label="Chiudi controllo del gruppo" @click="groupPanelOpen = false">×</button></header>
            <div class="group-sheet-scroll"><ul class="participant-list compact"><li v-for="participant in group.participants?.filter((entry) => entry.role === 'participant')" :key="participant.userId"><span class="avatar">{{ participant.username.slice(0,1).toUpperCase() }}</span><strong>{{ participant.username }}</strong><small class="participant-presence-status">{{ participantStatusLabel(participant) }}</small><small>{{ participantDetailLabel(participant) }}</small><small v-if="participant.requests?.length">Storico richieste: {{ participant.requests.slice(0,3).map((request) => request.label).join(' · ') }}</small></li></ul><p v-if="!group.synchronizedSession.participantCount" class="empty-participants">Nessun partecipante è ancora collegato.</p></div>
            <div v-if="groupPanelActions.length" class="host-actions group-sheet-actions"><button v-for="action in groupPanelActions" :key="action.actionId" type="button" :class="{ primary: ['SYNCHRONIZED_START_QUIZ','SYNCHRONIZED_COMPLETE'].includes(action.type) }" :disabled="interactionBusy" @click="dispatch(action)">{{ action.label }}</button></div>
            <div class="group-danger"><button type="button" :disabled="interactionBusy" @click="confirmingCancel = true">Annulla questa sessione</button></div>
          </section>
        </div>
      </Teleport>
      ...
      <Teleport to="body">
        <div v-if="voiceSheetOpen" class="modal-overlay" @click.self="cancelVoice" @keydown="onVoicePanelKeydown">
          <section ref="voicePanel" class="voice-sheet" role="dialog" aria-modal="true" aria-labelledby="voice-title"><div class="voice-orb" aria-hidden="true"><svg viewBox="0 0 24 24" width="34" height="34"><rect x="8" y="3" width="8" height="12" rx="4" fill="none" stroke="currentColor" stroke-width="2"/><path d="M5 11a7 7 0 0 0 14 0M12 18v3" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round"/></svg></div><h2 id="voice-title">Ti ascolto…</h2><p>Pronuncia uno dei comandi disponibili</p><div class="voice-examples" aria-label="Esempi di comandi disponibili"><span v-for="action in voiceExamples" :key="action.actionId">“{{ action.controlledVoiceAliases[0] || action.label }}”</span></div><button ref="voiceCancel" type="button" @click="cancelVoice">Annulla</button></section>
        </div>
      </Teleport>
    </template>
  </main>
