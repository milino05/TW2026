...
<template>
  <main class="session-experience">
        ...
        >
          ...
          <template v-if="quickActions.length">
            ...
            <div class="quick-actions">
              ...
              >{{ busyActionId === action.actionId ? "…" : action.label }}</button>
            </div>
          </template>

          <button
            v-if="hasActionSheetActions"
            class="all-actions"
            type="button"
            @click="actionSheetOpen = true"
          >Tutte le azioni disponibili</button>
        </section>
    ...
    <nav v-if="snapshot && !isCompleted" class="session-bottom" aria-label="Avanzamento della visita">
      <button
        v-if="previousAction"
        class="progress-action previous-action"
        type="button"
        :disabled="interactionBusy"
        @click="requestAction(previousAction)"
      >← {{ previousAction.label }}</button>
      <span v-else aria-hidden="true"></span>

      <button
        class="voice-action"
        type="button"
        aria-label="Usa un comando vocale"
        :disabled="interactionBusy || !browserControlledVoice.supported || !snapshot.availableActions.length"
      ...
      <button
        v-if="nextAction"
        class="progress-action next-action"
        type="button"
        :disabled="interactionBusy"
        @click="requestAction(nextAction)"
...
.session-experience {
  ...
  box-shadow: 0 0 42px var(--navigator-shadow);
...
.session-header {
  ...
  align-items: center;
  gap: .55rem;
  padding: max(.55rem, env(safe-area-inset-top)) 1rem .45rem;
}
...
.view-tabs {
  ...
  padding: .25rem;
  border-radius: .85rem;
  background: color-mix(in srgb, var(--navigator-ink) 8%, transparent);
}
...
.view-tabs button[aria-selected="true"] {
  ...
  box-shadow: 0 2px 7px var(--navigator-shadow);
}

.session-feedback {
  margin: 0 1.25rem .55rem;
  padding: .65rem .75rem;
  border: 1px solid color-mix(in srgb, var(--navigator-accent) 45%, var(--navigator-border));
  border-radius: .75rem;
  color: color-mix(in srgb, var(--navigator-accent) 70%, var(--navigator-ink));
  background: color-mix(in srgb, var(--navigator-accent) 10%, var(--navigator-surface-raised));
  font-size: .78rem;
}

.error-feedback {
  border-color: color-mix(in srgb, #b33138 52%, var(--navigator-border));
  color: #b33138;
  background: color-mix(in srgb, #b33138 8%, var(--navigator-surface-raised));
}
...
.physical-context em {
  ...
  border-radius: 999px;
  color: var(--navigator-primary);
  background: color-mix(in srgb, var(--navigator-primary) 10%, transparent);
  font-size: .65rem;
  font-style: normal;
  letter-spacing: .04em;
  text-transform: uppercase;
...
.audio-toggle {
  ...
  background: var(--navigator-brand-primary);
  box-shadow: 0 6px 16px var(--navigator-shadow);
}
...
.audio-panel strong { font-size: .9rem; }
.audio-panel span {
  color: var(--navigator-muted);
  font-size: .74rem;
}
...
.session-bottom {
  ...
  align-items: center;
  gap: .65rem;
  padding: .7rem 1.1rem max(.9rem, env(safe-area-inset-bottom));
  border-top: 1px solid var(--navigator-border);
  background: color-mix(in srgb, var(--navigator-surface-raised) 94%, transparent);
...
.voice-action {
  ...
  display: grid;
  place-items: center;
  border: 5px solid var(--navigator-surface);
  border-radius: 50%;
  color: var(--navigator-on-primary);
  background: var(--navigator-brand-primary);
  box-shadow: 0 7px 20px var(--navigator-shadow);
}
...
.voice-orb {
  ...
  color: var(--navigator-on-primary);
  background: var(--navigator-brand-primary);
  box-shadow: 0 0 0 13px color-mix(in srgb, var(--navigator-primary) 12%, transparent);
...
.completion-state > p:not(.eyebrow) { color: var(--navigator-muted); }
...
@media (min-width: 53rem) {
  .session-experience {
    border-inline: 1px solid var(--navigator-border);
  }
}

@media (max-width: 420px) {
  .session-header { padding-inline: .75rem; }
  .session-progress,
  .session-scroll { padding-inline: 1rem; }
  .view-tabs,
  .session-feedback { margin-inline: 1rem; }
  .session-brand small { max-width: 12rem; }
  .audio-panel { grid-template-columns: 54px minmax(0, 1fr) auto; }
  .session-bottom { padding-inline: .8rem; }
  .progress-action { font-size: .76rem; }
}

@media (max-width: 350px) {
  .session-brand img { display: none; }
  .audio-panel { grid-template-columns: 52px minmax(0, 1fr); }
  .audio-stop { grid-column: 2; justify-self: start; }
}
</style>
