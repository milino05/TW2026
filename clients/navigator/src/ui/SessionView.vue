...
import { navigationRepository, type SelectableLocationProjection } from "../infrastructure/http/navigationRepository";
...
const locationCorrectAction = computed(() => actionOfType(snapshot.value?.availableActions || [], "LOCATION_CORRECT"));
...
function synchronizeView(runtime: SessionProjection, { initial = false } = {}) {
  const phase = runtime.experience?.phase;
  if (phase === "location_required") {
    activeView.value = "map";
    locationSelectionMode.value = "confirm";
    return;
  }
  if (["navigating_to_visit_stop", "navigating_detour"].includes(phase || "")) {
    activeView.value = "map";
    if (locationSelectionMode.value === "confirm") locationSelectionMode.value = null;
    return;
  }
  if (phase === "approaching_visit_target") {
    activeView.value = "content";
    locationSelectionMode.value = null;
    return;
  }
  if (["presenting_visit_content", "presenting_semantic_content"].includes(phase || "")) {
    if (initial || activeView.value === "map") activeView.value = "content";
    locationSelectionMode.value = null;
  }
}

async function refreshMap() {
  const value = await navigationRepository.map(sessionId.value);
  navigationStore.setMap(value);
  return value;
}
...
onMounted(async () => {
  ...
  try {
    let runtime = snapshot.value;
    const requests: Promise<unknown>[] = [];
    if (!hasCurrentSnapshot) {
      requests.push(sessionRepository.current(sessionId.value).then((value) => {
        runtimeStore.applySnapshot(value);
        runtime = value;
      }));
    }
    requests.push(refreshMap());
    await Promise.all(requests);
    if (runtime) synchronizeView(runtime, { initial: true });
  } catch (cause) {
...
async function dispatch(
  ...
  channel: InteractionChannel = "button",
  input: unknown = null,
) {
  ...
  try {
    ...
    const shouldRefreshMap = ["progress", "navigation"].includes(action.family)
      || ["LOCATION_CONFIRM", "LOCATION_CORRECT", "VISIT_STOP_SELECT"].includes(action.type);
    if (shouldRefreshMap) {
      navigationStore.setNavigation(null);
      await refreshMap().catch(() => {});
    }

    if (action.family === "presentation" || action.family === "semantic") activeView.value = "content";
    synchronizeView(response.runtime);
    ...
    try {
      const current = await sessionRepository.current(sessionId.value);
      runtimeStore.applySnapshot(current);
      synchronizeView(current);
      await refreshMap().catch(() => {});
    } catch {
      runtimeStore.clear();
    }
...
async function selectLocation(location: SelectableLocationProjection["locationRef"]) {
  ...
  if (!action) return;
  await dispatch(action, "button", { location });
  locationSelectionMode.value = null;
}

async function selectStop(visitAnchorId: string) {
  const action = stopSelectAction.value;
  if (!action) return;
  await dispatch(action, "button", { visitAnchorId });
}
...
<template>
  <main class="session-experience">
    ...
    <div v-if="snapshot && !isCompleted" class="view-tabs" role="tablist" aria-label="Vista visita">
      <button type="button" role="tab" :aria-selected="activeView === 'content'" aria-controls="session-content-panel" @click="activeView = 'content'">Visita</button>
      ...
      <button type="button" role="tab" :aria-selected="activeView === 'stops'" aria-controls="session-stops-panel" :disabled="!map?.venues.length" @click="activeView = 'stops'">Tappe</button>
    </div>
    ...
    <div class="session-scroll">
      ...
      <template v-else-if="snapshot">
        <section v-show="activeView === 'content'" id="session-content-panel" class="content-panel" role="tabpanel">
          <template v-if="snapshot.current">
            ...
            <section class="audio-panel" aria-label="Lettura del contenuto">
              ...
              <div><strong>{{ audioTitle }}</strong><span>{{ audioDetail }}</span></div>
        ...
        <section v-show="activeView === 'stops'" id="session-stops-panel" class="stops-panel-wrapper" role="tabpanel">
          <SessionStopsPanel
            v-if="map"
            :map="map"
            :can-select="Boolean(stopSelectAction)"
            :busy="interactionBusy"
            @select="selectStop"
          />
          <p v-else>Le tappe della visita non sono disponibili.</p>
        </section>
      </template>
    </div>
    ...
    <div v-if="mediaOpen && currentMedia" class="modal-overlay media-overlay" @click.self="mediaOpen = false">
      <figure class="media-lightbox" role="dialog" aria-modal="true" aria-label="Immagine del contenuto">
        ...
        <figcaption><span>{{ currentMedia.altText }}</span><small v-if="mediaAttribution">{{ mediaAttribution }}</small></figcaption>
      </figure>
    </div>
    ...
    <div v-if="voiceSheetOpen" class="modal-overlay" @click.self="cancelVoice">
      <section class="voice-sheet" role="dialog" aria-modal="true" aria-labelledby="voice-title">
        <div class="voice-orb" aria-hidden="true"><svg viewBox="0 0 24 24" width="34" height="34"><rect x="8" y="3" width="8" height="12" rx="4" fill="none" stroke="currentColor" stroke-width="2"/><path d="M5 11a7 7 0 0 0 14 0M12 18v3" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round"/></svg></div>
        <h2 id="voice-title">Ti ascolto…</h2>
        <p>Pronuncia uno dei comandi disponibili</p>
        <div class="voice-examples" aria-label="Esempi di comandi disponibili"><span v-for="action in voiceExamples" :key="action.actionId">“{{ action.controlledVoiceAliases[0] || action.label }}”</span></div>
