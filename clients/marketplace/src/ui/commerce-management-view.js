...
export class ArtAroundCommerceManagementView extends HTMLElement {
  ...
  renderListing(listing) {
    const offers = listing.offers || []; const activeOffers = offers.filter((offer) => offer.status === "active"); const hasActiveOffer = activeOffers.length > 0; const isPublic = listing.status === "published" && hasActiveOffer; const renderedOffers = activeOffers.map((offer) => this.renderOffer(offer)).join(""); const canCreateOffer = hasOperation(listing.availableOperations, "create_offer"); const publishedDate = isPublic && listing.publishedAt ? `Pubblicata ${formatDate(listing.publishedAt)}` : "Contenuto privato"; const visibilityNotice = !isPublic ? `<div class="seller-visibility-notice"><strong>Contenuto privato</strong><p>Formula e pubblica almeno un’offerta: la scheda verrà pubblicata automaticamente insieme all’offerta.</p></div>` : "";
