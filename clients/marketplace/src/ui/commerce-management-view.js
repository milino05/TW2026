...
function listingStatusLabel(status) { if (status === "published") return "Nel catalogo"; if (status === "withdrawn") return "Ritirata"; return "Bozza"; }
...
export class ArtAroundCommerceManagementView extends HTMLElement {
  ...
  renderListing(listing) {
    const offers = listing.offers || []; const renderedOffers = offers.map((offer) => this.renderOffer(offer)).join(""); const canCreateOffer = hasOperation(listing.availableOperations, "create_offer"); const publishedDate = listing.publishedAt ? `Pubblicata ${formatDate(listing.publishedAt)}` : "Non ancora pubblicata";
