...
export class ArtAroundListingDetailView extends HTMLElement {
  ...
  renderScope() { const venues = this.detail?.asset?.physicalScope || []; if (!venues.length) return ""; return `<section class="consumer-detail-section"><div class="section-heading"><div><span class="eyebrow">Dove si svolge</span><h2>Sedi della visita</h2></div></div><div class="consumer-scope-list">${venues.map((venue) => { const href = publicVenueHref(venue); return href ? `<a class="chip" data-route href="${href}">${icon("museum", { size: 14 })}${escapeHtml(venue.name)}</a>` : `<span class="chip">${icon("museum", { size: 14 })}${escapeHtml(venue.name)}</span>`; }).join("")}</div></section>`; }
