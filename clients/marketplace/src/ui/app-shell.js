...
export class MarketplaceAppShell extends HTMLElement {
  ...
  onSubmit = async (event) => {
    ...
    if (form.matches("form[data-login]")) {
      ...
      try {
        ...
        this.venueSelector = await marketplaceRepository.venueSelector();
        await this.loadCatalogIfNeeded();
      } catch (error) { this.error = error instanceof Error ? error.message : "Accesso non riuscito"; }
    ...
    if (form.matches("form[data-catalog-filter]")) {
      ...
      catch (error) { this.error = error instanceof Error ? error.message : "Impossibile aggiornare il catalogo"; }
      finally { this.busy = false; this.render(); }
    }
  };
  ...
  onClick = async (event) => {
    ...
    if (acquireButton) {
      this.busy = true; this.error = null; this.render();
      try { await marketplaceRepository.acquire(acquireButton.dataset.acquire); await this.loadCatalogIfNeeded(); }
    ...
    if (logoutButton) {
      ...
      this.user = null; this.catalog = null; this.render();
    }
  };
  ...
  renderVenueSelector() {
    ...
    const groups = (this.venueSelector?.organizations || []).map((organization) => `
      <fieldset><legend>${escapeHtml(organization.name)}</legend>
        ${(organization.venues || []).map((venue) => `<div class="venue-row"><label class="venue-choice"><input type="checkbox" name="selectedVenueIds" value="${escapeHtml(venue.id)}" ${selected.has(String(venue.id)) ? "checked" : ""}> <span><strong>${escapeHtml(venue.name)}</strong>${venue.description ? `<small>${escapeHtml(venue.description)}</small>` : ""}</span></label><a data-route href="/workspace/venue-targets?venueId=${encodeURIComponent(venue.id)}">Crea contenuto per un oggetto</a></div>`).join("")}
      </fieldset>`).join("");
  ...
  renderCatalogFilters() {
    ...
    return `<form data-catalog-filter class="catalog-filter" role="search">${this.renderVenueSelector()}
      ...
      <label>Tipo di asset <select name="resourceType">${options}</select></label><button type="submit" ${this.busy ? "disabled" : ""}>Applica filtri</button></div></form>`;
  }
  ...
  renderCatalog() {
    ...
    const cards = results.map((entry) => {
      ...
      const relevanceText = relevance?.venueNeutral ? "Asset indipendente dalla Venue" : this.selectedVenueIds.length ? "Pertinente ad almeno una sede selezionata" : "";
      const visitInfo = physicalScope.length ? `<p><strong>PhysicalScope completo:</strong> ${escapeHtml(venues)}</p>` : "";
      return `<article class="card"><p class="asset-type">${escapeHtml(RESOURCE_LABELS[entry.asset.type] || entry.asset.type)}</p><h2>${escapeHtml(entry.asset.title)}</h2><p>${escapeHtml(entry.asset.summary)}</p>${visitInfo}${relevanceText ? `<p class="hint">${escapeHtml(relevanceText)}</p>` : ""}<p>Pubblicato da ${escapeHtml(entry.asset.publisher?.name || "")}${entry.asset.version ? ` · versione ${escapeHtml(entry.asset.version)}` : ""}</p>${offers || "<p>Nessuna offerta disponibile.</p>"}</article>`;
    }).join("");
    ...
    return `<main><h1>Catalogo</h1>${this.selectedVenueIds.length ? `<p>${this.selectedVenueIds.length} sede/i selezionata/e. Le Visit mostrano sempre l'intero PhysicalScope.</p>` : "<p>Nessun filtro Venue: catalogo globale.</p>"}${this.renderCatalogFilters()}${this.error ? `<p role="alert">${escapeHtml(this.error)}</p>` : ""}<p>${total} asset trovati</p>${cards || "<p>Nessun asset disponibile nel catalogo corrente.</p>"}<nav class="pagination" aria-label="Pagine catalogo"><button type="button" data-page="${page - 1}" ${page <= 1 || this.busy ? "disabled" : ""}>Precedente</button><span>Pagina ${page}</span><button type="button" data-page="${page + 1}" ${page * pageSize >= total || this.busy ? "disabled" : ""}>Successiva</button></nav></main>`;
  }
