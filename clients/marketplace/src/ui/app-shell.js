...
const TITLES = {
  "/": "Catalogo", "/catalog": "Catalogo", "/catalog/detail": "Dettaglio Marketplace", "/acquisitions": "Acquisizioni e licenze", "/workspace": "Workspace", "/workspace/resource": "Risorsa Workspace", "/workspace/commerce": "Gestione offerte",
  "/workspace/item-authoring": "Editor contenuto", "/workspace/visit-authoring": "Editor visita", "/workspace/venue-targets": "Oggetti della sede",
  "/workspace/context-compose": "Componi release", "/profile": "Profilo", "/organizations/detail": "Organization",
  "/namespaces/editor": "Editor Namespace", "/venues/editor": "Editor Venue", "/404": "Pagina non trovata",
};
const RESOURCE_TYPES = [["", "Tutti gli asset"], ["visit", "Visite"], ["visit_revision", "Revisioni visita"], ["item_edition", "Contenuti"], ["item_revision", "Revisioni contenuto"], ["editorial_context", "Contesti editoriali"], ["editorial_release", "Release editoriali"], ["namespace", "Namespace"], ["namespace_revision", "Revisioni Namespace"]];
...
export class MarketplaceAppShell extends HTMLElement {
  ...
  onClick = async (event) => {
    ...
    const pageButton = target?.closest("button[data-page]"); if (pageButton) { this.catalogPage = Math.max(1, Number(pageButton.dataset.page) || 1); this.busy = true; this.error = null; this.render(); try { await this.loadCatalogIfNeeded(); } catch (error) { this.error = error instanceof Error ? error.message : "Impossibile cambiare pagina"; } finally { this.busy = false; this.render(); } return; }
  ...
  renderVenueSelector() {
    ...
    return `<div class="venue-selector"><div class="filter-heading">${icon("museum")}<div><strong>Musei e sedi</strong><small>Puoi selezionarne più di una</small></div></div>${groups || `<div class="empty-state"><span>${icon("museum", { size: 26 })}</span><p>Nessuna Venue disponibile.</p></div>`}</div>`;
  }
  ...
  renderCatalogFilters() {
    ...
    return `<aside class="catalog-sidebar surface"><form data-catalog-filter class="catalog-filter" role="search"><div class="catalog-query"><label>Cerca nel catalogo <span class="input-icon">${icon("search")}<input name="q" value="${escapeHtml(this.catalogQuery)}" placeholder="Titolo o descrizione"></span></label><label>Tipo di asset <select name="resourceType">${options}</select></label></div>${this.renderVenueSelector()}<button type="submit" ${this.busy ? "disabled" : ""}>${this.busy ? "Aggiornamento…" : "Applica filtri"}</button></form></aside>`;
  }
