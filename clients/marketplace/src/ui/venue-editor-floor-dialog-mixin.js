...
export const venueFloorDialogMixin = {
  async handleMapAuthoringClick(event) {
    ...
    if (target.closest("[data-add-floor-shortcut], [data-open-floor-create]")) {
  ...
  renderMapAndPlaces(editable) {
    const base = venueSpatialMixin.renderMapAndPlaces.call(this, editable);
    const noFloors = !(this.data.layout?.floors || []).length;
    const createAction = editable && noFloors && this.activeSpatialTab === "map"
      ? `<div class="venue-map-empty-action"><button type="button" data-open-floor-create>+ Aggiungi il primo piano</button></div>`
      : "";
    return `${base}${createAction}${this.renderFloorDialog(editable)}`;
  },
};
