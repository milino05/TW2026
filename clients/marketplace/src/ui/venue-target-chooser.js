...
function principalParams() {
  ...
  const params = new URLSearchParams({ principalType: current.get("principalType") || "user" });
  if (current.get("principalId")) params.set("principalId", current.get("principalId"));
  return params;
}
...
export class VenueTargetChooser extends HTMLElement {
  ...
  async load() {
    ...
    if (!venueId) {
      this.error = "Sede non specificata.";
      this.render();
      return;
    }
    try { this.data = await authoringRepository.venueTargets(venueId); }
    catch (error) { this.error = error instanceof Error ? error.message : "Impossibile caricare gli oggetti della sede"; }
    this.render();
  }

  render() {
    const principal = principalParams();
    const targets = (this.data?.targets || []).map((target) => {
      const params = new URLSearchParams(principal);
      params.set("venueTargetId", target.id);
      return `
      <article class="target-choice">
        <div class="target-choice__icon">${icon("museum", { size: 22 })}</div>
        <div class="target-choice__copy"><span class="eyebrow">${escapeHtml(target.subject?.preferredLabel || "Soggetto non disponibile")}</span><h2>${escapeHtml(target.label)}</h2>
