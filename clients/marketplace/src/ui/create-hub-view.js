...
export class ArtAroundCreateHubView extends HTMLElement {
  ...
  async load() {
    ...
    try {
      ...
      const venues = preflight?.capabilities?.venueObjectContentCreate
  ...
  renderVenueCard() {
    if (!this.preflight?.capabilities?.venueObjectContentCreate) return "";
    if (!this.preflight?.content?.allowed) return "";
    const groups = this.venues?.organizations || [];
    const options = groups.flatMap((organization) => (organization.venues || []).map((venue) => ({ id: venue.id, name: venue.name, organization: organization.name })));
    if (!options.length) {
      const href = this.context?.type === "organization" && this.context?.id ? `/organizations/detail?organizationId=${encodeURIComponent(this.context.id)}&section=venues` : "/catalog";
      return `<article class="panel create-choice"><span class="resource-mark">${icon("museum", { size: 21 })}</span><div><span class="eyebrow">Entità della sede</span><h2>Crea contenuto per un’entità fisica</h2><p>Non ci sono ancora sedi disponibili da cui scegliere un’entità.</p></div><a class="button-link secondary" data-route href="${escapeHtml(href)}">${this.context?.type === "organization" ? "Gestisci sedi" : "Esplora il catalogo"} ${icon("chevron", { size: 15 })}</a></article>`;
    }
    return `<article class="panel create-choice"><span class="resource-mark">${icon("museum", { size: 21 })}</span><div><span class="eyebrow">Entità della sede</span><h2>Crea contenuto per un’entità fisica</h2><p>Scegli prima la sede. Nel passaggio successivo selezionerai l’entità e il suo soggetto verrà precompilato.</p><form data-venue-content class="inline-form"><label>Sede<select name="venueId">${options.map((venue) => `<option value="${escapeHtml(venue.id)}">${escapeHtml(venue.organization)} · ${escapeHtml(venue.name)}</option>`).join("")}</select></label><button type="submit">Scegli entità ${icon("chevron", { size: 15 })}</button></form></div></article>`;
  }
