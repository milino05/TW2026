...
export class ArtAroundCreateHubView extends HTMLElement {
  ...
  currentEditorialSpace() { return resolveEditorialSpacePreference(this.principal(), this.workspaceContext?.contentSpaces || []); }
  ...
  renderContentCard() {
    ...
    const space = this.currentEditorialSpace();
    if (!space) {
      return `<article class="panel create-choice blocked"><span class="resource-mark">${icon("workspace", { size: 21 })}</span><div><span class="eyebrow">Contenuto</span><h2>Prima crea uno spazio editoriale</h2><p>Ogni contenuto posseduto deve appartenere ad almeno uno spazio editoriale attivo.</p></div><a class="button-link secondary" data-route href="/workspace">Apri la Libreria ${icon("chevron", { size: 15 })}</a></article>`;
    }
    const href = `/workspace/item-authoring?contentSpaceId=${encodeURIComponent(id(space))}`;
    return `<article class="panel create-choice"><span class="resource-mark">${icon("book", { size: 21 })}</span><div><span class="eyebrow">Contenuto</span><h2>Crea un contenuto</h2><p>Parti dal Subject di cui vuoi parlare. Il nuovo Item verrà inserito nello spazio editoriale corrente.</p><p class="note">Destinazione: <strong>${escapeHtml(space.name || "Spazio editoriale")}</strong> · ${content.usableNamespaceCount} ${content.usableNamespaceCount === 1 ? "insieme di regole disponibile" : "insiemi di regole disponibili"}.</p></div><a class="button-link" data-route href="${escapeHtml(href)}">Crea contenuto ${icon("chevron", { size: 15 })}</a></article>`;
  }
