...
export const venueSpatialMixin = {
  ...
  renderConnections(editable) {
    ...
    return `<section class="venue-command-block"><div class="section-heading compact"><div><h3>Collegamenti</h3><p>Gli estremi e la geometria si definiscono sulla mappa; metrica e caratteristiche restano validate dal backend.</p></div><span class="count">${connections.length}</span></div>${this.renderConnectionComposer(editable)}<div class="venue-command-grid">${cards || `<div class="empty-state compact"><h4>Nessun collegamento</h4><p>Usa “Collega luoghi” sopra la mappa.</p></div>`}</div></section>`;
  },
  ...
  renderExhibitSlots(editable) {
    ...
    const cards = slots.map((slot) => {
      ...
      const incomingConnections = (layout.connections || []).filter((connection) => (
        id(connection.toPlaceId) === id(place?._id)
        || (connection.directionality === "bidirectional" && id(connection.fromPlaceId) === id(place?._id))
      ));
      const connectionOptions = incomingConnections.map((connection) => {
        const sourcePlaceId = id(connection.toPlaceId) === id(place?._id) ? connection.fromPlaceId : connection.toPlaceId;
        const source = placeById.get(id(sourcePlaceId));
        return `<option value="${escapeHtml(id(connection._id))}">${escapeHtml(source?.label || "Luogo")} → ${escapeHtml(place?.label || "Luogo")}</option>`;
      }).join("");
      const sourceSlotOptions = slots
        .filter((entry) => id(entry.exhibitSlotId) !== slotId && id(entry.placeId) === id(slot.placeId))
        .map((entry) => `<option value="${escapeHtml(id(entry.exhibitSlotId))}">${escapeHtml(entry.label)}</option>`)
        .join("");
      const approachOverrides = (slot.approachGuidance?.overrides || []).map((override) => {
        ...
        if (override.sourceKind === "incoming_connection") {
          const connection = incomingConnections.find((entry) => id(entry._id) === sourceId);
          const sourcePlaceId = id(connection?.toPlaceId) === id(place?._id) ? connection?.fromPlaceId : connection?.toPlaceId;
          sourceLabel = `Dal collegamento proveniente da ${placeById.get(id(sourcePlaceId))?.label || "un altro luogo"}`;
        } else sourceLabel = `Dallo slot ${slots.find((entry) => id(entry.exhibitSlotId) === sourceId)?.label || "vicino"}`;
        return `<li><span><strong>${escapeHtml(sourceLabel)}</strong><small>${escapeHtml(override.instruction)}</small></span><button class="button-secondary small" type="button" data-remove-slot-override="${escapeHtml(slotId)}" data-override-source-kind="${escapeHtml(override.sourceKind)}" data-override-source-id="${escapeHtml(sourceId)}">Rimuovi</button></li>`;
      }).join("");
      const targetOptions = unassignedTargets.map((target) => `<option value="${escapeHtml(id(target.id))}" ${selected(target.id, assigned?.id)}>${escapeHtml(target.label)}</option>`).join("");
      const placeOptions = places.map((entry) => `<option value="${escapeHtml(id(entry._id))}" ${selected(entry._id, slot.placeId)}>${escapeHtml(entry.label || "Luogo")}</option>`).join("");
      const assignment = `<form data-slot-assignment="${escapeHtml(slotId)}" class="venue-inline-form"><label>Entità esposta<select name="venueTargetId"><option value="">Nessuna entità</option>${targetOptions}</select></label><button>${assigned ? "Cambia entità" : "Assegna entità"}</button></form>`;
      const editor = `<details><summary>Dettagli e istruzioni di avvicinamento</summary>${approachOverrides ? `<div class="venue-approach-overrides"><strong>Istruzioni specifiche attive</strong><ul>${approachOverrides}</ul></div>` : `<p class="note">Nessuna istruzione specifica: verrà usata quella predefinita.</p>`}<form data-slot-editor="${escapeHtml(slotId)}" class="venue-inline-form"><label>Etichetta slot<input name="label" value="${escapeHtml(slot.label)}" required></label><label>Luogo<select name="placeId">${placeOptions}</select></label><label>Ordine<input name="order" type="number" min="0" value="${slot.order ?? ""}"></label><label class="wide">Istruzione predefinita<textarea name="defaultInstruction">${escapeHtml(slot.approachGuidance?.defaultInstruction || "")}</textarea></label><div class="venue-form-intro wide"><strong>Aggiungi o aggiorna un’istruzione specifica</strong><small>Seleziona una sola provenienza. Le istruzioni già presenti vengono conservate.</small></div><label>Da collegamento<select name="sourceConnectionId"><option value="">Nessun collegamento</option>${connectionOptions}</select></label><label>Da slot vicino<select name="sourceExhibitSlotId"><option value="">Nessuno slot</option>${sourceSlotOptions}</select></label><label class="wide">Istruzione specifica<input name="overrideInstruction" placeholder="Es. L’opera è alla tua destra"></label><button>Salva slot</button></form><button class="danger small" type="button" data-remove-slot="${escapeHtml(slotId)}">Rimuovi slot</button></details>`;
      return `<article class="venue-command-card exhibit-slot-card${id(this.selectedExhibitSlotId) === slotId ? " selected locate-highlight" : ""}" data-exhibit-slot-card="${escapeHtml(slotId)}"><header><div><span class="eyebrow">ExhibitSlot</span><h3>${escapeHtml(slot.label)}</h3></div><span class="chip">${assigned ? "Assegnato" : "Libero"}</span></header><p>${escapeHtml(place?.label || "Luogo mancante")} · ${escapeHtml(floor?.label || "Piano")}</p><code class="slot-public-code">${escapeHtml(slot.publicCode || "Codice non disponibile")}</code><div class="button-row"><button class="button-secondary small" type="button" data-copy-slot-code="${escapeHtml(slot.publicCode || "")}" ${slot.publicCode ? "" : "disabled"}>Copia codice</button><button class="button-secondary small" type="button" data-locate-slot="${escapeHtml(slotId)}">Localizza</button></div>${editable ? `${assignment}${editor}` : ""}</article>`;
    }).join("");
    const create = editable && places.length ? `<form data-create-slot class="venue-inline-create"><div class="venue-form-intro"><strong>Nuovo slot espositivo</strong><small>Definisci l’etichetta, poi scegli il luogo direttamente sulla mappa.</small></div><label>Etichetta<input name="label" required placeholder="Es. Parete nord · Slot 1"></label><label>Ordine facoltativo<input name="order" type="number" min="0"></label><button>${icon("pin", { size: 15 })} Scegli luogo</button></form>` : "";
    return `<section class="venue-arrangement-panel"><div class="section-heading compact"><div><h3>Slot espositivi</h3><p>Gli slot sono posizioni stabili; le entità possono essere assegnate o scollegate senza perdere disponibilità e media.</p></div><span class="count">${slots.length}</span></div>${create}<div class="venue-command-grid">${cards || `<div class="empty-state compact"><h4>Nessuno slot</h4><p>Crea il primo slot e associalo a un luogo.</p></div>`}</div></section>`;
  },
  ...
  renderArrangement(editable) {
    const tabs = [["slots", "Slot espositivi"], ["entities", "Entità della sede"]].map(([key, label]) => `<button type="button" data-arrangement-tab="${key}" aria-selected="${this.activeArrangementTab === key}">${label}</button>`).join("");
  ...
  renderMapAndPlaces(editable) {
    ...
    const spatialTabs = [["map", "Mappa"], ["places", "Luoghi"], ["connections", "Collegamenti"], ["arrangement", "Allestimento"]].map(([key, label]) => `<button type="button" id="venue-spatial-tab-${key}" role="tab" data-spatial-tab="${key}" aria-controls="venue-spatial-panel-${key}" aria-selected="${this.activeSpatialTab === key}">${label}</button>`).join("");
