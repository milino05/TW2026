...
export class ArtAroundVisitAuthoringView extends HTMLElement {
  ...
  renderCreate() {
    return `<main class="page visit-authoring-page"><nav class="breadcrumb"><button type="button" data-back>${icon("arrowLeft", { size: 15 })} Libreria</button><span>/</span><span>Nuova visita</span></nav><header class="page-header"><div><span class="eyebrow">Crea visita</span><h1>Nuova visita</h1><p>Definisci le informazioni essenziali; ArtAround collegherà i contenuti alle occorrenze fisiche quando il legame è univoco.</p></div></header><section class="wizard-step panel"><header class="step-heading"><span class="step-number">1</span><div><h2>Come si presenta questa visita?</h2></div></header><form data-create-visit class="editor-form"><label>Titolo<input name="title" required maxlength="160"></label><label>Descrizione<textarea name="description" rows="5"></textarea></label><button type="submit" ${this.busy ? "disabled" : ""}>Crea la bozza e scegli i contenuti ${icon("chevron", { size: 15 })}</button></form></section>${this.error ? `<p role="alert">${escapeHtml(this.error)}</p>` : ""}</main>`;
  }
  renderStepOne() {
