...
export class ArtAroundVisitAuthoringView extends HTMLElement {
  ...
  renderStepSix() {
    ...
    const state = published ? `<div class="readiness success">${icon("check", { size: 20 })}<div><strong>Versione pubblicata</strong><p>Questa versione della visita è uno snapshot editoriale immutabile. La pubblicazione nel Catalogo è un passaggio commerciale separato.</p></div></div>` : integrity === "valid" ? `<div class="readiness success">${icon("check", { size: 20 })}<div><strong>Controllo superato</strong><p>La visita è coerente e può passare alla pubblicazione.</p></div></div>` : `<div class="readiness warning">${icon("warning", { size: 20 })}<div><strong>Serve un controllo</strong><p>Esegui il controllo dopo le modifiche.</p></div></div>`;
    return `<section class="wizard-step panel"><header class="step-heading"><span class="step-number">6</span><div><span class="eyebrow">Riepilogo e pubblicazione</span><h2>Controlla la visita prima di pubblicarla</h2></div></header>${this.reviewSummary()}${state}${issues.length ? `<div class="issue-panel"><strong>Problemi da risolvere</strong><ul>${issues.map((issue) => `<li>${escapeHtml(userFacingIssueMessage(issue))}</li>`).join("")}</ul></div>` : ""}<div class="workflow-panel"><div><h3>Azioni disponibili</h3><p class="note">Pubblicare editorialmente non crea automaticamente una scheda nel Marketplace.</p></div><div class="workflow-actions">${operations.map((operation) => this.renderWorkflowOperation(operation)).join("") || `<p>Nessuna azione editoriale disponibile.</p>`}</div></div></section>`;
  }
