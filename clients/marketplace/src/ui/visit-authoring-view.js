...
export class ArtAroundVisitAuthoringView extends HTMLElement {
  ...
  renderManualStopBrowser() {
    ...
    return `<details class="advanced-panel manual-stops"><summary>Opzioni avanzate</summary><h4>Aggiungi una tappa senza contenuto</h4><p class="note">Utile solo quando vuoi prevedere una sosta che non deriva automaticamente dai contenuti.</p><label>Sede<select data-venue>${venueOptions || "<option value=''>Nessuna sede disponibile</option>"}</select></label><div class="target-grid">${targets.map((entry) => `<article class="target-card"><div><strong>${escapeHtml(entry.label)}</strong><small>${escapeHtml(entry.subject?.preferredLabel || entry.description || "Entità fisica")}</small></div><button type="button" data-add-stop="${escapeHtml(id(entry.id))}">Aggiungi tappa</button></article>`).join("") || `<p class="note">Nessun’altra entità pubblicata disponibile.</p>`}</div></details>`;
  }
  ...
  styles() {
    return `<style>
      ...
      .advanced-panel{padding:.8rem;border:1px dashed var(--line-strong);border-radius:var(--radius-md);background:var(--surface)}.manual-stops{margin-top:.8rem}.target-grid{grid-template-columns:repeat(2,minmax(0,1fr));margin-top:.7rem}.target-card{grid-template-columns:1fr auto}.occurrence-choice{display:grid;gap:.75rem;padding:1rem;border:1px solid var(--amber-500);border-radius:var(--radius-md);background:var(--amber-100)}.occurrence-grid{display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:.6rem}.occurrence-card{display:grid;text-align:left;padding:.8rem}
