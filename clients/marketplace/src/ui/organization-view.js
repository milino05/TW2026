...
const SECTIONS = new Set(["overview", "people", "roles", "venues", "rules", "settings"]);
...
export class ArtAroundOrganizationView extends HTMLElement {
  ...
  renderRoleEditor() {
    ...
    const role = this.roleForEditor();
    const selected = new Set((role?.permissionCodes || []).map(String));
    const groups = this.data.permissionCatalog?.groups || [];
  ...
  tabLabel(section) {
    const counts = { people: this.data.members.total, roles: this.data.roles.length, venues: this.data.venues.total, rules: this.data.namespaces.total };
