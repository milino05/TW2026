...
function normalizedSpatialTab(value) {
  return SPATIAL_TABS.has(value) ? value : "map";
}
function ids(entries, selector) {
  return new Set((entries || []).map(selector).map(id).filter(Boolean));
}
...
export const venueContextualWorkspaceMixin = {
  locateExhibitSlot(exhibitSlotId) {
    ...
    requestAnimationFrame(() => {
      const slotCard = this.querySelector(`[data-inspector-slot="${CSS.escape(id(slot.exhibitSlotId))}"]`);
      slotCard?.scrollIntoView({ block: "nearest" });
      slotCard?.querySelector("input, select, textarea, button")?.focus();
    });
  },
  ...
  async handleMapAuthoringClick(event) {
    ...
    const openPlace = target.closest("[data-open-place-inspector]");
    if (openPlace) {
      const place = (this.data.layout?.places || []).find((entry) => id(entry._id) === id(openPlace.dataset.openPlaceInspector));
    ...
    const openConnection = target.closest("[data-open-connection-inspector]");
    if (openConnection) {
      const connection = (this.data.layout?.connections || []).find((entry) => id(entry._id) === id(openConnection.dataset.openConnectionInspector));
      if (!connection) return true;
      const from = (this.data.layout?.places || []).find((entry) => id(entry._id) === id(connection.fromPlaceId));
      this.selectedConnectionId = id(connection._id);
    ...
    const openSlot = target.closest("[data-open-slot-inspector]");
    if (openSlot) {
      this.locateExhibitSlot(openSlot.dataset.openSlotInspector);
      return true;
    }
    ...
    if (actionType === "create-place") {
      ...
      if (exhibitSlotId) this.locateExhibitSlot(exhibitSlotId);
    }
    return true;
  },
  ...
  async handleMapAuthoringSubmit(form, data) {
    ...
    if (connectionId) {
      const connection = (this.data.layout?.connections || []).find((entry) => id(entry._id) === id(connectionId));
      const from = (this.data.layout?.places || []).find((entry) => id(entry._id) === id(connection?.fromPlaceId));
  ...
  renderPlaces(editable) {
    ...
    const cards = places.map((place) => {
      ...
      return `<article class="venue-command-card${id(this.selectedMapPlaceId) === placeId ? " selected" : ""}"><header><div><span class="eyebrow">${escapeHtml(typeById.get(id(place.placeTypeDefinitionId))?.label || "Tipo non disponibile")}</span><h3>${escapeHtml(place.label || "Luogo senza nome")}</h3></div><span class="chip">${escapeHtml(floorById.get(id(place.floorId))?.label || "Piano")}</span></header><dl class="venue-command-facts"><div><dt>Slot espositivi</dt><dd>${slotCountByPlace.get(placeId) || 0}</dd></div><div><dt>Collegamenti</dt><dd>${connectionCountByPlace.get(placeId) || 0}</dd></div></dl><div class="button-row"><button class="button-secondary small" type="button" data-open-place-inspector="${escapeHtml(placeId)}">Apri sulla mappa</button></div></article>`;
    }).join("");
  ...
  renderConnections(editable) {
    ...
    return `<section class="venue-command-block"><div class="section-heading compact"><div><h3>Collegamenti</h3><p>Vista globale degli archi del grafo. Tipo, metrica, istruzioni e geometria si modificano nell’inspector della mappa.</p></div><div class="button-row"><span class="count">${connections.length}</span>${create}</div></div><div class="venue-command-grid">${cards || `<div class="empty-state compact"><h4>Nessun collegamento</h4><p>Apri la mappa e collega due luoghi.</p></div>`}</div></section>`;
  },
  ...
  renderExhibitSlots(editable) {
    ...
    const cards = slots.map((slot) => {
      ...
      return `<article class="venue-command-card exhibit-slot-card${id(this.selectedExhibitSlotId) === slotId ? " selected locate-highlight" : ""}"><header><div><span class="eyebrow">Slot espositivo</span><h3>${escapeHtml(slot.label)}</h3></div><span class="chip">${assigned ? "Assegnato" : "Libero"}</span></header><p>${escapeHtml(place?.label || "Luogo mancante")} · ${escapeHtml(floor?.label || "Piano")}</p><dl class="venue-command-facts"><div><dt>Entità esposta</dt><dd>${escapeHtml(assigned?.label || "Nessuna")}</dd></div><div><dt>Indicazione</dt><dd>${escapeHtml(slot.approachGuidance?.defaultInstruction || "Da completare")}</dd></div></dl><code class="slot-public-code">${escapeHtml(slot.publicCode || "Codice non disponibile")}</code><div class="button-row"><button class="button-secondary small" type="button" data-open-slot-inspector="${escapeHtml(slotId)}">Apri nell’inspector</button>${slot.publicCode ? `<button class="button-secondary small" type="button" data-copy-slot-code="${escapeHtml(slot.publicCode)}">Copia codice</button>` : ""}</div></article>`;
    }).join("");
    const create = editable && (layout.places || []).length ? `<button type="button" data-start-slot>${icon("pin", { size: 15 })} Nuovo slot</button>` : "";
    return `<section class="venue-command-block"><div class="section-heading compact"><div><h3>Slot espositivi</h3><p>Posizioni stabili dell’allestimento. Assegnazioni, istruzioni e dettagli si modificano nel Place inspector.</p></div><div class="button-row"><span class="count">${slots.length}</span>${create}</div></div><div class="venue-command-grid">${cards || `<div class="empty-state compact"><h4>Nessuno slot</h4><p>Crea uno slot dalla toolbar o dal Place inspector.</p></div>`}</div></section>`;
  },
      ...
      : activeTab === "connections" ? this.renderConnections(editable)
        : activeTab === "slots" ? this.renderExhibitSlots(editable)
          : activeTab === "inventory" ? this.renderTargets(editable)
            : `<div class="venue-canvas-layout">${this.renderMapPreview(editable)}${this.renderMapInspector(editable)}</div>${this.renderCalibrationComposer(editable)}${this.renderGeometryComposer(editable)}`;
