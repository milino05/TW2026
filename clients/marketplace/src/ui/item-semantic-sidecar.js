...
export class ArtAroundItemSemanticSidecar extends HTMLElement {
  ...
  onClick = (event) => {
    ...
    if (target.closest("[data-open-sidecar-collection-content]")) {
  ...
  renderChoice(choice) {
    return `<button type="button" class="workspace-sidecar-choice" data-sidecar-editorial-context="${escapeHtml(id(choice.id))}"><span class="workspace-sidecar-choice__icon">${icon("link", { size: 18 })}</span><span class="workspace-sidecar-choice__copy"><strong>${escapeHtml(choice.name || "Raccolta editoriale")}</strong><small>${escapeHtml(choice.contentSpace?.name || "Spazio editoriale")} · ${escapeHtml(choice.namespace?.name || "Regole editoriali")}</small><span>${Number(choice.itemCount || 0)} contenuti · ${Number(choice.relationCount || 0)} relazioni · grafo locale</span></span>${icon("chevron", { size: 15 })}</button>`;
  }

  renderChooser() {
    const results = this.choices?.results || [];
    const pagination = this.choices?.pagination || { page: this.page, total: 0, totalPages: 0 };
    return `<section class="workspace-sidecar__chooser"><div><span class="eyebrow">Grafo della Raccolta</span><h2>Scegli dove lavorare</h2><p>Scegli la Raccolta che fornisce il contesto semantico per <strong>${escapeHtml(this.subject?.preferredLabel || "il Subject del contenuto")}</strong>. Ogni Raccolta modifica soltanto il proprio grafo locale.</p></div><form class="inline-form" data-sidecar-collection-search role="search"><label>Cerca raccolta<input name="q" value="${escapeHtml(this.query)}" placeholder="Nome o descrizione"></label><button type="submit" class="button-secondary" ${this.busy ? "disabled" : ""}>${icon("search", { size: 15 })} Cerca</button></form>${results.length ? `<div class="workspace-sidecar-choice-list">${results.map((choice) => this.renderChoice(choice)).join("")}</div>` : this.busy ? `<div class="empty-state compact"><p>Ricerca delle raccolte…</p></div>` : `<div class="empty-state compact"><h3>Nessuna raccolta modificabile</h3><p>${this.query ? "Nessuna raccolta corrisponde alla ricerca." : "Per aggiungere collegamenti serve una Raccolta il cui grafo locale sia modificabile nella tua area di lavoro."}</p><div class="button-row"><button type="button" class="button-secondary" data-open-editorial-spaces>Apri gli spazi editoriali</button><button type="button" class="button-secondary" data-open-relation-hub>Collega soggetti</button></div></div>`}${Number(pagination.totalPages || 0) > 1 ? `<nav class="pagination" aria-label="Pagine delle raccolte"><button type="button" data-sidecar-page="${Number(pagination.page || 1) - 1}" ${Number(pagination.page || 1) <= 1 || this.busy ? "disabled" : ""}>← Precedente</button><span>Pagina ${Number(pagination.page || 1)} di ${Number(pagination.totalPages || 1)}</span><button type="button" data-sidecar-page="${Number(pagination.page || 1) + 1}" ${Number(pagination.page || 1) >= Number(pagination.totalPages || 0) || this.busy ? "disabled" : ""}>Successiva →</button></nav>` : ""}</section>`;
  }

  renderMembershipPrompt() {
    const graph = this.studio?.semanticGraph || {};
    return `<section class="workspace-sidecar__chooser"><div><span class="eyebrow">Relazioni · ${escapeHtml(this.studio?.context?.name || "Raccolta")}</span><h2>${escapeHtml(this.subject?.preferredLabel || "Subject")}</h2><p>Questo Subject non è ancora materializzato nel grafo locale <strong>${escapeHtml(graph.name || "della Raccolta")}</strong>. Può essere aggiunto al grafo soltanto se almeno un contenuto che lo rappresenta appartiene già alla Raccolta.</p><p class="note">Se questo Item è già nei Contenuti della Raccolta, puoi materializzare il Subject ora. Altrimenti aggiungi prima il contenuto: il grafo non può contenere Subject privi di contenuto nella Raccolta.</p></div><div class="button-row"><button type="button" data-add-sidecar-subject ${this.busy ? "disabled" : ""}>${icon("plus", { size: 15 })} Aggiungi al grafo</button><button type="button" class="button-secondary" data-open-sidecar-collection-content>Apri contenuti Raccolta</button>${this.contextualCollectionId() ? "" : `<button type="button" class="button-secondary" data-change-sidecar-context>Scegli un'altra raccolta</button>`}</div></section>`;
  }

  renderGraph() {
    const graph = this.studio?.semanticGraph || {};
    return `<section class="workspace-sidecar__graph"><header class="workspace-sidecar__context"><div><span class="eyebrow">Relazioni · ${escapeHtml(this.studio?.context?.name || "Raccolta")}</span><h2>${escapeHtml(this.subject?.preferredLabel || "Collegamenti")}</h2><p>Grafo locale: <strong>${escapeHtml(graph.name || "Grafo della Raccolta")}</strong></p></div>${this.contextualCollectionId() ? "" : `<button type="button" class="button-secondary small" data-change-sidecar-context>Scegli un altro contesto</button>`}</header><artaround-semantic-graph-editor></artaround-semantic-graph-editor></section>`;
  }
