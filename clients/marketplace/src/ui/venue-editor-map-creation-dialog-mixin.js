...
export const venueMapCreationDialogMixin = {
  closeMapCreationDialog() {
    if (this.mapCreationDialog?.type === "connection" && this.pendingMapAction?.type === "connect") this.pendingMapAction = null;
  ...
  renderMapActionStatus() {
    ...
    if (action?.type === "connect" && action.fromPlaceId && action.toPlaceId) {
      return `<p class="venue-map-instruction active"><strong>Collega luoghi</strong> · completa tipo, direzione e metrica nel dialog aperto.</p>`;
    }
  ...
  renderMapCreationDialog(editable) {
    ...
    if (this.mapCreationDialog.type === "connection") {
      ...
      return `<div class="venue-modal-backdrop" role="presentation"><section class="venue-modal-card venue-connection-create-dialog" role="dialog" aria-modal="true" aria-labelledby="venue-connection-create-title"><header><div><span class="eyebrow">Nuovo collegamento</span><h3 id="venue-connection-create-title">Configura il percorso selezionato</h3></div><button class="button-secondary small" type="button" data-close-map-creation-dialog aria-label="Chiudi">×</button></header>${composer}<p class="note">Indicazioni direzionali, geometria e caratteristiche fisiche possono essere rifinite subito dopo nell’editor dettagliato del collegamento.</p></section></div>`;
    }
