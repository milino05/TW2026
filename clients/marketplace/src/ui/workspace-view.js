...
export class ArtAroundWorkspaceView extends HTMLElement {
  ...
  renderRemoval(asset) {
    ...
    const graphLifecycle = collection
      ? `<div class="resource-removal-critical"><span>${icon("link", { size: 24 })}</span><div><strong>Anche il grafo locale verrà ritirato</strong><p>${relationCount > 0 ? `La revisione corrente contiene <strong>${relationCount} ${relationCount === 1 ? "relazione" : "relazioni"}</strong>: non resterà come grafo modificabile indipendente.` : "Il grafo locale non resterà come risorsa modificabile indipendente."}</p><p>Le revisioni immutabili del grafo e le release già pubblicate restano conservate e continuano a supportare snapshot, acquisizioni e visite già pinzate.</p></div></div>`
      : "";
    const confirmation = this.pendingRemoval
      ? `<section class="confirmation-panel resource-removal-confirmation" role="alert"><div><span class="eyebrow">Conferma richiesta</span><strong>Eliminare ${subject} “${escapeHtml(asset.title)}”?</strong><p>${consequence} Le pubblicazioni verranno ritirate e le offerte rese inattive. Acquisizioni, diritti già concessi e adozioni resteranno validi.</p>${graphLifecycle}</div><div class="button-row"><button class="danger" type="button" data-confirm-removal ${this.busy ? "disabled" : ""}>Elimina ${subject}</button><button class="button-secondary" type="button" data-cancel-removal ${this.busy ? "disabled" : ""}>Annulla</button></div></section>`
      : "";
