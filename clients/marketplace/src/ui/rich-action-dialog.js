...
export function openRichActionDialog({
  ...
  return new Promise((resolve) => {
    ...
    dialog = createTaskDialog({
      ...
      renderFooter: () => `<button type="button" class="button-secondary" data-modal-dismiss data-rich-action-cancel>${cancelLabel}</button><button type="button" class="${tone === "danger" ? "danger" : ""}" data-modal-confirm data-rich-action-confirm>${confirmLabel}</button>`,
      isDirty: () => false,
