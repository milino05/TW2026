...
export class ArtAroundSubjectPresence extends HTMLElement {
  ...
  renderPublicPlacement(row) {
    return `<article class="subject-venue-card subject-venue-card--public"><div class="subject-venue-card-main"><div><h4>${escapeHtml(row.venue.name)}</h4><p>${row.inventory?.place ? `${escapeHtml(row.inventory.place.floorLabel || "Piano")} · ${escapeHtml(row.inventory.place.label || "Posizione")}` : "Configurazione pubblicata"}</p></div><span class="chip" data-tone="success">Esposto</span></div><div class="button-row"><button class="button-secondary" type="button" data-map-venue="${escapeHtml(row.venue.id)}" data-focus-target="${escapeHtml(row.mapTargetId || "")}">${icon("map", { size: 15 })} Mostra sulla mappa</button></div></article>`;
  }
  ...
  render() {
    ...
    const media = usage.previewMedia;
