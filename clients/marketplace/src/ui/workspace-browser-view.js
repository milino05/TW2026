...
const OWNED_TYPES = [["", "Tutte le risorse"], ["item_edition", "Contenuti"], ["visit", "Visite"], ["editorial_context", "Raccolte editoriali"], ["namespace", "Regole editoriali"]];
const LICENSED_TYPES = [["", "Tutte le risorse"], ["item_edition", "Contenuti"], ["item_revision", "Versioni dei contenuti"], ["visit", "Visite"], ["visit_revision", "Versioni delle visite"], ["editorial_context", "Raccolte editoriali"], ["editorial_release", "Versioni delle raccolte"], ["namespace", "Regole editoriali"], ["namespace_revision", "Versioni delle regole editoriali"]];
function escapeHtml(value = "") { return String(value).replaceAll("&", "&amp;").replaceAll("<", "&lt;").replaceAll(">", "&gt;").replaceAll('"', "&quot;").replaceAll("'", "&#039;"); }
function initialState() { const p = new URLSearchParams(window.location.search); return { ownership: p.get("ownership") === "licensed" ? "licensed" : "owned", q: p.get("q") || "", resourceType: p.get("resourceType") || "", page: Math.max(1, Number(p.get("page")) || 1), removed: ["content", "namespace"].includes(p.get("removed")) ? p.get("removed") : "" }; }
...
export class ArtAroundWorkspaceBrowserView extends HTMLElement {
  ...
  render() {
    ...
    const removedMessage = this.state.removed ? `<p class="status success" role="status">${this.state.removed === "namespace" ? "Regole editoriali eliminate dall’account." : "Contenuto eliminato dall’account."} Le acquisizioni e i diritti già concessi restano validi.</p>` : "";
