...
export class ArtAroundProfileView extends HTMLElement {
  ...
  onSubmit = async (event) => {
    ...
    if (form.matches("[data-create-namespace]")) {
      ...
      await this.execute(() => accountRepository.createNamespace({ ownerType: "user", ownerId: this.workspace.account.id, name: String(data.get("name") || ""), description: String(data.get("description") || "") }), "Regole editoriali personali create.");
  ...
  render() {
    ...
    this.innerHTML = `<main class="page account-page" aria-busy="${this.busy}"><header class="account-header"><div><span class="eyebrow">Account ArtAround</span><h1>${escapeHtml(account.username)}</h1><p>Profilo, preferenze personali, accessi alle organizzazioni e strumenti editoriali avanzati.</p></div><div class="account-avatar" aria-hidden="true">${escapeHtml(account.username.slice(0, 2).toUpperCase())}</div></header><nav class="account-tabs" aria-label="Sezioni account"><a href="#account-overview">Profilo</a><a href="#account-preferences">Preferenze visita</a><a href="#account-organizations">Organizzazioni <span>${organizations.length}</span></a><a href="#account-rules">Strumenti avanzati <span>${personalNamespaces.length}</span></a></nav>${this.busy ? `<p role="status">Aggiornamento…</p>` : ""}${this.message ? `<p role="status">${icon("check", { size: 16 })} ${escapeHtml(this.message)}</p>` : ""}${this.error ? `<p role="alert">${icon("warning", { size: 16 })} ${escapeHtml(this.error)}</p>` : ""}<section class="account-overview" id="account-overview"><div><span class="eyebrow">Profilo</span><h2>Il tuo account</h2><p>Sei sempre autenticato con questo account. L'area di lavoro selezionata determina soltanto dove vengono create, acquisite e pubblicate le risorse.</p></div><dl class="account-summary"><div><dt>Organizzazioni</dt><dd>${organizations.length}</dd></div><div><dt>Regole editoriali personali</dt><dd>${personalNamespaces.length}</dd></div></dl></section><section class="account-section" id="account-preferences"><div class="section-heading"><div><span class="eyebrow">Preferenze visita</span><h2>Come vuoi vivere le visite</h2><p>Queste impostazioni aiutano il Navigator a personalizzare l'esperienza.</p></div></div><div class="account-preferences"><form class="panel" data-presentation-preference><div class="preference-heading"><span>${icon("book", { size: 20 })}</span><div><h3>Presentazione</h3><p>Regola profondità e linguaggio.</p></div></div><label>Profondità <input type="range" min="0" max="1" step="0.05" name="depthPreference" value="${number(presentation.depthPreference)}"><span class="range-labels"><small>Essenziale</small><small>Approfondita</small></span></label><label>Complessità linguistica <input type="range" min="0" max="1" step="0.05" name="languageComplexityPreference" value="${number(presentation.languageComplexityPreference)}"><span class="range-labels"><small>Semplice</small><small>Specialistica</small></span></label><button>${icon("check", { size: 16 })} Salva</button></form><form class="panel" data-navigation-preference><div class="preference-heading"><span>${icon("route", { size: 20 })}</span><div><h3>Movimento</h3><p>Definisci il ritmo della visita.</p></div></div><label>Ritmo preferito <input type="range" min="0" max="1" step="0.05" name="movementPacePreference" value="${number(navigation.movementPacePreference)}"><span class="range-labels"><small>Rilassato</small><small>Sostenuto</small></span></label><button>${icon("check", { size: 16 })} Salva</button></form><form class="panel" data-learning-preference><div class="preference-heading"><span>${icon("user", { size: 20 })}</span><div><h3>Adattamento</h3><p>Controlla l'uso dei segnali personali.</p></div></div><label class="check"><input type="checkbox" name="personalHistory" ${checked(learning.personalHistory)}> <span>Usa la mia cronologia</span></label><label class="check"><input type="checkbox" name="collectiveContribution" ${checked(learning.collectiveContribution)}> <span>Contribuisci in forma pseudonima</span></label><button>${icon("check", { size: 16 })} Salva</button></form></div></section><section class="account-section" id="account-organizations"><div class="section-heading"><div><span class="eyebrow">Accessi</span><h2>Organizzazioni</h2><p>Qui controlli le organizzazioni di cui fai parte. Per lavorare in una di esse, cambiala dalla schermata delle aree di lavoro.</p></div><button type="button" data-context-hub>${icon("workspace", { size: 16 })} Cambia o crea area</button></div><div class="account-resource-grid">${this.renderOrganizations(organizations)}</div></section><section class="account-section" id="account-rules"><div class="section-heading"><div><span class="eyebrow">Strumenti avanzati</span><h2>Regole editoriali personali</h2><p>Vocabolari e criteri editoriali di tua proprietà, separati da quelli delle organizzazioni.</p></div><span class="count">${personalNamespaces.length}</span></div><div class="account-resource-grid">${this.renderPersonalRules(personalNamespaces)}</div><details class="account-create"><summary>${icon("plus", { size: 16 })} Crea regole editoriali personali</summary><form data-create-namespace><label>Nome<input name="name" required placeholder="Nome delle regole editoriali"></label><label>Descrizione<textarea name="description" placeholder="Ambito e finalità editoriali"></textarea></label><button>${icon("plus", { size: 16 })} Crea</button></form></details></section></main>`;
  }
}
