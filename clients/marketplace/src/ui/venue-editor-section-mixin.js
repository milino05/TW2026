...
export const venueSectionMixin = {
  ...
  renderPhysicalOnboarding() {
    ...
    if (!choices.length && !onboarding.canCreate) {
      return `<main class="page venue-editor-page"><nav class="breadcrumb"><button type="button" data-back>${icon("arrowLeft", { size: 16 })} Organizzazione</button></nav><section class="venue-onboarding venue-onboarding--blocked"><span class="eyebrow">Configurazione fisica</span><h1>${escapeHtml(venue.name)}</h1><h2>Serve un PhysicalVocabulary utilizzabile</h2><p>Questa organizzazione non dispone di un vocabolario fisico utilizzabile e il tuo ruolo non può crearne uno. Chiedi a un responsabile di pubblicarne o assegnarne uno.</p></section></main>`;
    }
