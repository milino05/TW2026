...
export const venueSectionMixin = {
  ...
  renderVenueRemoval() {
    ...
    const confirmation = this.pendingVenueRemoval
      ? `<section class="confirmation-panel resource-removal-confirmation" role="alert"><div><span class="eyebrow">Conferma richiesta</span><strong>Spostare “${escapeHtml(this.data.venue.name)}” nel cestino?</strong><p>La sede sparirà dalle superfici attive. VenueRelease, LayoutRevision e ${targetCount} ${targetCount === 1 ? "entità fisica" : "entità fisiche"} resteranno conservati come stato storico e potranno tornare disponibili ripristinando la sede.</p><p>${escapeHtml(visitWarning)}</p></div><div class="button-row"><button class="danger" type="button" data-confirm-venue-removal ${this.busy ? "disabled" : ""}>Sposta sede nel cestino</button><button class="button-secondary" type="button" data-cancel-venue-removal ${this.busy ? "disabled" : ""}>Annulla</button></div></section>`
      : "";
    return `<section class="panel resource-danger-zone"><span class="eyebrow">Operazione sensibile</span><h2>Rimuovi sede</h2><p>La rimozione è logica: non cancella release, layout, entità o storico delle visite.</p><p class="note">${escapeHtml(visitWarning)}</p>${confirmation || `<button class="danger" type="button" data-request-venue-removal>${icon("trash", { size: 15 })} Sposta nel cestino</button>`}</section>`;
  },
  ...
  renderDestructiveActionConfirmation() {
    ...
    if (!action) return "";
    return `<section class="confirmation-panel resource-removal-confirmation" role="alert"><div><span class="eyebrow">Conferma richiesta</span><strong>${escapeHtml(action.title || "Confermare l'operazione?")}</strong><p>${escapeHtml(action.description || "Questa operazione modifica la configurazione fisica di lavoro.")}</p></div><div class="button-row"><button class="danger" type="button" data-confirm-destructive-action ${this.busy ? "disabled" : ""}>${escapeHtml(action.confirmLabel || "Conferma")}</button><button class="button-secondary" type="button" data-cancel-destructive-action ${this.busy ? "disabled" : ""}>Annulla</button></div></section>`;
  },
