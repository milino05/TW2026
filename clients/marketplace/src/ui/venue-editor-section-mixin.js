...
export const venueSectionMixin = {
  ...
  render() {
    ...
    this.innerHTML = `<main class="page venue-editor-page venue-editor-page--workspace" aria-busy="${this.busy}" data-map-mode="${escapeHtml(this.mapMode?.() || "idle")}"><nav class="breadcrumb" aria-label="Percorso"><button type="button" data-back>${icon("arrowLeft", { size: 16 })} Organizzazione</button><span>/</span><span>Sedi e spazi fisici</span></nav><header class="venue-editor-header"><div><span class="eyebrow">Sede dell'organizzazione</span><h1>${escapeHtml(venue.name)}</h1><p>${escapeHtml(venue.description || "Configura inventario, spazi e percorsi fisici della sede.")}</p></div><div class="venue-editor-status"><span class="chip">${escapeHtml(sourceLabel(venue.source))}</span>${release ? `<span class="chip">Versione ${release.version}</span><span class="chip">${escapeHtml(statusLabel(release.status))}</span>` : ""}</div></header>${this.busy ? `<p role="status">Aggiornamento…</p>` : ""}${this.message ? `<p class="feedback-success" role="status">${icon("check", { size: 16 })} ${escapeHtml(this.message)}</p>` : ""}${this.error ? `<p role="alert">${icon("warning", { size: 16 })} ${escapeHtml(this.error)}</p>` : ""}${this.renderDestructiveActionConfirmation()}<div class="venue-editor-layout"><aside class="venue-editor-nav"><nav role="tablist" aria-orientation="vertical" aria-label="Sezioni sede">${nav}</nav><details class="technical-details"><summary>Dettagli tecnici</summary><dl class="definition-list"><div><dt>Venue ID</dt><dd><code>${escapeHtml(venue.id)}</code></dd></div>${release ? `<div><dt>VenueRelease ID</dt><dd><code>${escapeHtml(release.id)}</code></dd></div>` : ""}${layout ? `<div><dt>LayoutRevision ID</dt><dd><code>${escapeHtml(layout.id)}</code></dd></div>` : ""}</dl></details></aside><div class="venue-editor-content">${this.renderOverview()}${this.renderMapAndPlaces(editable)}${this.renderVisitors(editable)}${this.renderPublication()}</div></div></main>`;
