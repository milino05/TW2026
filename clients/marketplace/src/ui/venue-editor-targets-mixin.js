...
function targetCard(entry, context) {
  ...
  const itemHref = `/workspace/item-authoring?venueId=${encodeURIComponent(venueId)}&subjectId=${encodeURIComponent(id(entry.subject?.id))}&physicalIntent=1`;
...
export const venueTargetsMixin = {
  ...
  renderTargets(editable) {
    ...
    const resolverFallback = this.venueSubjectCandidates && !this.venueSubjectCandidates.exact?.length ? `<details class="venue-semantic-fallback"><summary>Estendi la ricerca a Wikidata o crea manualmente</summary><p>Le corrispondenze approssimative non vengono selezionate automaticamente.</p><artaround-semantic-entity-picker mode="subject" entity-kind="item" initial-query="${escapeHtml(this.venueSubjectQuery)}"></artaround-semantic-entity-picker></details>` : "";
    return `<section class="venue-arrangement-panel"><div class="venue-inventory-toolbar"><div><h3>Venue entities</h3><p>Inventario interno della sede, distinto dagli Item e dagli slot.</p></div><div class="venue-filter-group" role="group" aria-label="Filtra inventario">${[["all", "Tutte"], ["exposed", "Esposte"], ["unplaced", "Non collocate"], ["unavailable", "Non disponibili"]].map(([value, label]) => `<button class="button-secondary small" type="button" data-inventory-filter="${value}" aria-pressed="${this.inventoryFilter === value}">${label}</button>`).join("")}</div></div><div class="venue-target-grid">${cards || `<div class="empty-state compact"><h4>Nessuna entità in questo filtro</h4></div>`}</div>${editable ? `<details class="venue-create"><summary>${icon("plus", { size: 16 })} Aggiungi entità all’inventario</summary><form data-venue-subject-search class="venue-subject-search"><label>Cerca un’opera, una persona o un luogo<input name="query" minlength="2" value="${escapeHtml(this.venueSubjectQuery)}" required></label><button>Cerca</button></form>${exact}${suggestions}${resolverFallback}${selectedSubject}</details>` : ""}</section>`;
  },
