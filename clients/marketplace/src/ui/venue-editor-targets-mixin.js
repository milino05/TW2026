...
function renderConfigurationState(target, editable, hasRelease) {
  ...
  const publishedNote = configuration.publishedReferenced
...
export const venueTargetsMixin = {
  ...
  renderTargets(editable) {
    ...
    const cards = this.data.targets.map((entry) => {
      const trash = has(entry.availableOperations, "venue.target.trash")
        ? `<button class="danger" type="button" data-trash-target="${escapeHtml(entry.id)}" data-label="${escapeHtml(entry.label)}">Sposta nel cestino</button>`
        : "";
      const bindingDetails = hasRelease && entry.binding
        ...
        : "";
      return `<article class="venue-target-card"><header><div><span class="eyebrow">${escapeHtml(entry.subject?.label || "Identità non disponibile")}</span><h3>${escapeHtml(entry.label)}</h3></div><span class="chip">${entry.binding ? escapeHtml(availabilityLabel(entry.binding.availability)) : "Fuori dalla bozza"}</span></header><p>${escapeHtml(entry.description || "Nessuna descrizione fisica.")}</p>${renderConfigurationState(entry, editable, hasRelease)}<details><summary>Dettagli oggetto</summary><form data-target-metadata="${escapeHtml(entry.id)}"><label>Nome nella sede<input name="label" value="${escapeHtml(entry.label)}" required></label><label>Descrizione fisica<textarea name="description">${escapeHtml(entry.description || "")}</textarea></label><div class="button-row"><button type="submit">Salva oggetto</button>${trash}</div></form></details>${bindingDetails}</article>`;
    }).join("");
