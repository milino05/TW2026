...
export class ArtAroundPublicVenueView extends HTMLElement {
  ...
  render() {
    ...
    const targetCards = targets.map((target) => `<article class="public-target-card"><span class="resource-mark">${icon("museum", { size: 18 })}</span><div><strong>${escapeHtml(target.label)}</strong><p>${escapeHtml(target.description || "Entità della sede")}</p></div></article>`).join("");
