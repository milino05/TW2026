...
function newDraft() {
  return { namespaceId: "", label: "", author: "", license: "", text: "", tags: "", durationTypeDefinitionId: "", languageLevelDefinitionId: "", locale: "it-IT" };
}
...
export class ItemAuthoringView extends HTMLElement {
  ...
  async prepareNewEdition() {
    ...
    this.newEditionMode = true; this.namespaceControls = null; this.draft = newDraft();
  ...
  async selectNamespace(namespaceId) {
    ...
    this.draft.durationTypeDefinitionId = ""; this.draft.languageLevelDefinitionId = "";
  }
  ...
  onInput = (event) => { this.updateDraftField(event.target); };
  ...
  onSubmit = async (event) => {
    ...
    try {
      ...
      } else if (form.matches("[data-add-representation]")) await this.addRepresentation(data);
      else if (form.matches("[data-update-representation]")) await this.updateRepresentation(data);
  ...
  async createEditionFromDraft() {
    ...
    const created = await authoringRepository.createEdition(this.itemId, {
      ...
      revision: {
        label: this.draft.label, authorCredits: [this.draft.author].filter(Boolean), metadata: { license: this.draft.license }, relatedSubjectIds: [], tags: parseTags(this.draft.tags), illustrativeMedia: [], selectionSignals: [],
        presentationVariants: [{ key: "standard", label: "Standard", semanticFocus: [], presentationAspects: [], knowledgeRequirements: [], representations: [{ durationTypeDefinitionId: this.draft.durationTypeDefinitionId, languageLevelDefinitionId: this.draft.languageLevelDefinitionId, locale: this.draft.locale, text: this.draft.text }] }],
  ...
  async updateEditionFromDraft() {
    ...
    payload.label = this.draft.label; payload.authorCredits = [this.draft.author].filter(Boolean); payload.tags = parseTags(this.draft.tags); payload.metadata = { license: this.draft.license };
    const first = payload.presentationVariants?.[0]?.representations?.[0];
    if (first) { first.durationTypeDefinitionId = this.draft.durationTypeDefinitionId || first.durationTypeDefinitionId; first.languageLevelDefinitionId = this.draft.languageLevelDefinitionId || first.languageLevelDefinitionId; first.locale = this.draft.locale || first.locale; first.text = this.draft.text; }
    await authoringRepository.updateEdition(editionId, payload); await this.reloadProjection(editionId); this.activeStep = 4;
  ...
  async addRepresentation(data) {
    if (!this.availableOperation("item.edit")) throw new Error("Il contenuto non è modificabile nello stato corrente");
    const revision = this.selectedRevision(); const editionId = id(this.selectedEdition()?.id); if (!revision || !editionId) throw new Error("Nessuna versione modificabile");
    const payload = projectedRevisionToWrite(revision); const variant = payload.presentationVariants?.[0]; if (!variant) throw new Error("La variante di presentazione principale non è disponibile");
    variant.representations = variant.representations || [];
    variant.representations.push({ durationTypeDefinitionId: String(data.get("durationTypeDefinitionId") || ""), languageLevelDefinitionId: String(data.get("languageLevelDefinitionId") || ""), locale: String(data.get("locale") || "").trim(), text: String(data.get("text") || "").trim() });
    await authoringRepository.updateEdition(editionId, payload); await this.reloadProjection(editionId); this.activeStep = 3; this.notice = "Testo alternativo aggiunto. Riesegui il controllo prima della pubblicazione.";
  }

  async updateRepresentation(data) {
    if (!this.availableOperation("item.edit")) throw new Error("Il contenuto non è modificabile nello stato corrente");
    const revision = this.selectedRevision(); const editionId = id(this.selectedEdition()?.id); const representationId = String(data.get("representationId") || "");
    ...
    const representation = variant?.representations?.find((entry) => id(entry._id) === representationId); if (!representation) throw new Error("Testo alternativo non trovato");
    ...
    representation.locale = String(data.get("locale") || representation.locale).trim(); representation.text = String(data.get("text") || representation.text).trim();
    await authoringRepository.updateEdition(editionId, payload); await this.reloadProjection(editionId); this.activeStep = 3; this.notice = "Testo alternativo aggiornato. Riesegui il controllo prima della pubblicazione.";
  }
  ...
  renderStepTwo() {
    ...
    const revision = this.selectedRevision(); const editing = Boolean(revision && !this.newEditionMode); const namespaceName = editing ? this.selectedNamespace()?.name : this.namespaceControls?.namespace?.name;
    return `<section class="wizard-step panel"><header class="step-heading"><span class="step-number">2</span><div><span class="eyebrow">Contenuto</span><h2>Scrivi il contenuto essenziale</h2><p>Titolo, testo, autore e licenza sono le informazioni necessarie per una bozza pubblicabile.</p></div></header>${editing ? `<div class="selection-summary"><span>Regole editoriali</span><strong>${escapeHtml(namespaceName || "Non disponibili")}</strong></div>` : this.renderNamespaceSelector()}${editing || this.namespaceControls ? `<form data-content-draft class="editor-form"><label>Titolo del contenuto<input name="label" required value="${escapeHtml(this.draft.label)}"></label><label>Testo<textarea name="text" rows="10" required>${escapeHtml(this.draft.text)}</textarea></label><div class="two-columns"><label>Autore<input name="author" required value="${escapeHtml(this.draft.author)}"></label><label>Licenza<input name="license" required value="${escapeHtml(this.draft.license)}"></label></div><details class="technical-details"><summary>Metadati facoltativi</summary><label>Tag<input name="tags" value="${escapeHtml(this.draft.tags)}"></label></details><div class="step-actions"><button class="button-secondary" type="button" data-back-step="1">Indietro</button><button type="submit">Continua alla personalizzazione ${icon("chevron", { size: 15 })}</button></div></form>` : ""}</section>`;
  }
  ...
  renderTechnicalPresentation() {
    ...
    if (this.newEditionMode) return `<details class="technical-details"><summary>Dettagli tecnici</summary><p>Namespace ${escapeHtml(this.draft.namespaceId || "-")} · NamespaceRevision ${escapeHtml(this.namespaceControls?.revision?.id || "-")} · al salvataggio verranno creati ItemEdition, ItemRevision, PresentationVariant e Representation.</p></details>`;
    if (!revision) return "";
    return `<details class="technical-details"><summary>Dettagli tecnici</summary><p>ItemEdition ${escapeHtml(this.selectedEdition()?.id || "-")} · ItemRevision ${escapeHtml(revision.id || "-")} · NamespaceRevision ${escapeHtml(this.selectedNamespace()?.revision?.id || "-")} · PresentationVariant ${escapeHtml(variant?.id || "-")} · Representation ${escapeHtml(representation?.id || "-")}</p></details>`;
  }

  renderAdditionalRepresentations(controls) {
    if (this.newEditionMode || !this.selectedRevision()) return `<details class="technical-details"><summary>Altri testi e livelli</summary><p>Salva prima la bozza principale. Potrai poi aggiungere altri testi con durata, livello linguistico o lingua differenti.</p></details>`;
    const representations = this.firstVariant()?.representations || []; const extra = representations.slice(1); const canEdit = Boolean(this.availableOperation("item.edit"));
    ...
    const existing = extra.map((representation, index) => `<details class="representation-card"><summary><strong>Testo alternativo ${index + 1}</strong></summary>${canEdit ? `<form data-update-representation class="editor-form"><input type="hidden" name="representationId" value="${escapeHtml(id(representation.id))}"><label>Durata<select name="durationTypeDefinitionId">${durationOptions(representation.duration?.definitionId)}</select></label><label>Complessità<select name="languageLevelDefinitionId">${languageOptions(representation.languageComplexity?.definitionId)}</select></label><label>Lingua<input name="locale" value="${escapeHtml(representation.locale || "it-IT")}"></label><label>Testo<textarea name="text" rows="6">${escapeHtml(representation.text || "")}</textarea></label><button type="submit">Salva testo alternativo</button></form>` : `<p>${escapeHtml(representation.text || "")}</p>`}</details>`).join("");
    const add = canEdit ? `<details class="technical-details"><summary>${icon("plus", { size: 15 })} Aggiungi un altro testo</summary><form data-add-representation class="editor-form"><label>Durata<select name="durationTypeDefinitionId" required><option value="">Scegli</option>${durationOptions()}</select></label><label>Complessità<select name="languageLevelDefinitionId" required><option value="">Scegli</option>${languageOptions()}</select></label><label>Lingua<input name="locale" required value="it-IT"></label><label>Testo<textarea name="text" rows="6" required></textarea></label><button type="submit">Aggiungi testo</button></form></details>` : "";
    return `<section class="alternate-texts"><header><span class="eyebrow">Testi multipli</span><h3>Altri testi e livelli</h3></header>${existing || `<p class="note">Nessun testo alternativo.</p>`}${add}</section>`;
  }

  renderStepThree() {
    if (this.activeStep !== 3 || !this.itemId) return "";
    const controls = this.personalizationControls(); if (!controls) return `<section class="wizard-step panel"><p role="alert">Le opzioni di personalizzazione non sono disponibili.</p></section>`;
    ...
    return `<section class="wizard-step panel"><header class="step-heading"><span class="step-number">3</span><div><span class="eyebrow">Personalizzazione</span><h2>Adatta il contenuto al pubblico</h2></div></header><form data-personalization-draft class="editor-form"><div class="two-columns"><label>Durata<select name="durationTypeDefinitionId" required>${durationOptions}</select></label><label>Complessità linguistica<select name="languageLevelDefinitionId" required>${languageOptions}</select></label></div><label>Lingua e locale<input name="locale" required value="${escapeHtml(this.draft.locale)}"></label><div class="step-actions"><button class="button-secondary" type="button" data-back-step="2">Indietro</button><button type="submit">${this.newEditionMode ? "Salva bozza" : "Salva modifiche"}</button></div></form>${this.renderAdditionalRepresentations(controls)}${this.renderMemberships()}${this.renderTechnicalPresentation()}</section>`;
  }

  reviewSummary() {
    const revision = this.selectedRevision(); const first = this.firstRepresentation(); if (!revision) return "";
    return `<div class="review-grid"><article><span>Di cosa parla</span><strong>${escapeHtml(this.selectedSubject?.preferredLabel || "-")}</strong></article><article><span>Titolo</span><strong>${escapeHtml(revision.label || "-")}</strong></article><article><span>Durata</span><strong>${escapeHtml(first?.duration?.label || "-")}</strong></article><article><span>Linguaggio</span><strong>${escapeHtml(first?.languageComplexity?.label || "-")}</strong></article><article><span>Autore</span><strong>${escapeHtml(revision.authorCredits?.[0] || "-")}</strong></article><article><span>Licenza</span><strong>${escapeHtml(revision.license || "-")}</strong></article></div>`;
  }
  ...
  styles() {
    return `<style>:host{display:block}.authoring-page{display:grid;gap:1rem;max-width:68rem;margin:auto;padding:2rem 1rem 5rem}.authoring-progress ol{display:grid;grid-template-columns:repeat(4,minmax(9rem,1fr));gap:.55rem;min-width:42rem;margin:0;padding:0;list-style:none}.authoring-progress__summary{display:none}.authoring-progress button{display:flex;width:100%;align-items:center;gap:.5rem;padding:.65rem;border:1px solid #ccd6d1;border-radius:.7rem;background:white}.authoring-progress button>span{display:grid;place-items:center;flex:0 0 1.7rem;height:1.7rem;border-radius:999px;background:#edf2ef}.wizard-step{padding:1.35rem}.step-heading{display:flex;gap:.85rem}.step-number{display:grid;place-items:center;flex:0 0 2rem;height:2rem;border-radius:999px;background:#173e35;color:white}.editor-form{display:grid;gap:.9rem;max-width:48rem;margin-top:1rem}.two-columns,.review-grid{display:grid;grid-template-columns:1fr 1fr;gap:1rem}.step-actions,.workflow-actions{display:flex;gap:.65rem;align-items:center;flex-wrap:wrap;margin-top:1rem}.subject-summary,.context-box,.selection-summary,.readiness,.issue-panel,.workflow-panel{margin-top:1rem;padding:1rem;border:1px solid #d4ddd8;border-radius:.8rem;background:#f8faf8}.membership-fieldset,.technical-details,.representation-card{margin-top:1rem;padding:.75rem;border:1px dashed #91a39b;border-radius:.7rem}.membership-grid,.alternate-texts{display:grid;gap:.65rem}.membership{display:grid;grid-template-columns:auto 1fr;gap:.5rem}.edition-tabs{display:flex;gap:.5rem;flex-wrap:wrap}.note{color:#60706a}@media(max-width:48rem){.authoring-progress ol{grid-template-columns:repeat(4,minmax(0,1fr));min-width:0}.authoring-progress button strong{font-size:.72rem}.two-columns,.review-grid{grid-template-columns:1fr}}@media(max-width:32rem){.authoring-progress__summary{display:grid;gap:.1rem}.authoring-progress button strong{display:none}.step-actions>*{width:100%}}</style>`;
  }
}
