...
export class ItemAuthoringView extends HTMLElement {
  ...
  renderProgress() {
    ...
    return `<nav class="authoring-progress" aria-label="Passaggi di creazione"><ol>${stages.map(([step, label]) => {
      ...
      return `<li data-current="${current}" data-complete="${complete}"><button type="button" data-step="${step}" ${enabled ? "" : "disabled"} aria-current="${current ? "step" : "false"}"><span>${complete ? icon("check", { size: 14 }) : step}</span><strong>${escapeHtml(label)}</strong></button></li>`;
    }).join("")}</ol></nav>`;
  }
