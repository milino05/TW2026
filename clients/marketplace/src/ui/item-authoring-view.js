...
export class ItemAuthoringView extends HTMLElement {
  ...
  renderRepresentationEditors(controls) {
    ...
    return `<div class="representation-list">${this.draft.representations.map((representation, index) => {
      ...
      const languageLevel = (controls.languageLevels || []).find((entry) => entry.definitionId === representation.languageLevelDefinitionId);
      const durationLabel = duration ? `${duration.label}${duration.targetSeconds ? ` · ${duration.targetSeconds}s` : ""}` : "Da indicare";
      const textPreview = String(representation.text || "").trim();
      if (!active) return `<article class="representation-card collapsed"><div class="representation-card__summary"><strong>${index === 0 ? "Testo principale" : `Testo ${index + 1}`}</strong><dl class="representation-card__metadata"><div><dt>Durata</dt><dd>${escapeHtml(durationLabel)}</dd></div><div><dt>Livello linguistico</dt><dd>${escapeHtml(languageLevel?.label || "Da indicare")}</dd></div><div><dt>Lingua</dt><dd>${escapeHtml(representation.locale || "Da indicare")}</dd></div></dl><p>${escapeHtml(textPreview || "Testo ancora da compilare")}</p></div><div class="button-row"><button class="button-secondary small" type="button" data-edit-text="${index}">Modifica</button><button class="button-secondary small" type="button" data-remove-text="${index}">Rimuovi</button></div></article>`;
  ...
  styles() {
    return `<style>
      ...
      .representation-list{display:grid;gap:.75rem}.representation-card{display:grid;gap:.75rem;padding:1rem;border:1px solid var(--border);border-radius:var(--radius);background:var(--surface)}.representation-card>header,.representation-card.collapsed{display:flex;justify-content:space-between;gap:1rem;align-items:flex-start}.representation-card__summary{display:grid;gap:.65rem;min-width:0;flex:1}.representation-card__metadata{display:grid;grid-template-columns:repeat(3,minmax(0,1fr));gap:.45rem;margin:0}.representation-card__metadata>div{display:grid;gap:.12rem;padding:.5rem .6rem;border-radius:.55rem;background:var(--surface-soft,#f4f7f5)}.representation-card__metadata dt{color:var(--muted);font-size:.68rem;font-weight:750}.representation-card__metadata dd{margin:0;font-size:.8rem;font-weight:750}.representation-card__summary p{display:-webkit-box;overflow:hidden;margin:0;color:var(--muted);font-size:.82rem;line-height:1.45;-webkit-box-orient:vertical;-webkit-line-clamp:2}.representation-card small{color:var(--muted)}.representation-settings{display:grid;grid-template-columns:1fr 1fr .7fr;gap:.7rem}.add-text{justify-self:start}
      ...
      @media(max-width:48rem){.authoring-progress button strong{font-size:.72rem}.representation-settings,.review-grid,.signal-grid{grid-template-columns:1fr}.representation-card__metadata{grid-template-columns:1fr 1fr}.media-card{grid-template-columns:1fr}.media-card figure{max-width:12rem}}
      @media(max-width:32rem){.authoring-progress button strong{display:none}.step-actions>*{width:100%}.representation-card>header,.representation-card.collapsed{align-items:flex-start;flex-direction:column}.representation-card__metadata{grid-template-columns:1fr}.representation-card.collapsed>.button-row{width:100%}}
    </style>`;
  }
}
