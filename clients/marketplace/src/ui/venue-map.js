...
export class ArtAroundVenueMap extends HTMLElement {
  ...
  renderPlaceDetail(place) {
    ...
    const cards = entries.map(({ slot, target }) => {
      const subjectId = id(target.subjectId);
      const createContent = subjectId ? `<a class="button-link small secondary venue-map-create-content" data-route href="/workspace/item-authoring?subjectId=${encodeURIComponent(subjectId)}">${icon("plus", { size: 14 })} Crea contenuto</a>` : "";
      return `<article data-focused="${id(target.id) === focusedId}"><div><strong>${escapeHtml(target.label || "Entità")}</strong><small>${escapeHtml(slot.label || place.label || "Collocazione pubblicata")}</small>${createContent}</div>${target.recognitionMedia?.[0]?.url ? `<img src="${escapeHtml(target.recognitionMedia[0].url)}" alt="${escapeHtml(target.recognitionMedia[0].altText || target.label || "")}" loading="lazy">` : ""}</article>`;
    }).join("");
  ...
  render() {
    ...
    this.innerHTML = `<style>
      ...
      .venue-map-detail{display:grid;gap:.6rem;padding:1rem;border:1px solid var(--line);border-radius:var(--radius-lg);background:var(--surface)}.venue-map-detail h3,.venue-map-detail p{margin:0}.venue-map-detail-targets{display:grid;gap:.55rem}.venue-map-detail-targets article{display:grid;grid-template-columns:minmax(0,1fr) auto;gap:.6rem;align-items:center;padding:.65rem;border:1px solid var(--line);border-radius:var(--radius-sm);background:var(--sage-50)}.venue-map-detail-targets article[data-focused="true"]{border-color:var(--amber-400);background:var(--amber-100)}.venue-map-detail-targets article>div{display:grid;gap:.3rem;min-width:0}.venue-map-detail-targets small{color:var(--sage-600)}.venue-map-detail-targets img{width:3.5rem;height:3.5rem;border-radius:.45rem;object-fit:cover}.venue-map-create-content{justify-self:start;margin-top:.25rem}
