...
const WORKFLOW_ACTION = {
  ...
  "namespace.revision.request_review": "request-review",
  "namespace.revision.withdraw_review": "withdraw-review",
  "namespace.revision.request_changes": "request-changes",
  "namespace.revision.publish": "publish",
};
const WORKFLOW_LABEL = {
  "namespace.revision.check": "Controlla se è tutto pronto",
  "namespace.revision.request_review": "Invia in revisione",
  "namespace.revision.withdraw_review": "Ritira dalla revisione",
  "namespace.revision.request_changes": "Richiedi modifiche",
  "namespace.revision.publish": "Pubblica",
};
...
function emptyDefinition(field) {
  ...
  if (field === "durationTypes") base.targetSeconds = 60;
  if (field === "relationTypes") Object.assign(base, { domainDefinitionIds: [], rangeDefinitionIds: [], category: "semantic", strength: "medium", directionality: "directed", userIntents: [], reverse: { label: "", description: "", userIntents: [] }, validationRules: { allowMultiple: true, targetRequired: true } });
  return base;
}
...
export class ArtAroundNamespaceEditorView extends HTMLElement {
  ...
  async saveAll({ continueWorkflow = null } = {}) { if (!this.data) return; const { metadata, definitions } = this.snapshotDraft(); await this.execute(async () => { if (metadata) await accountRepository.updateNamespace(this.id, metadata); if (definitions) await managementRepository.updateNamespaceRevision(this.id, definitions); if (continueWorkflow) await this.runWorkflowRequest(continueWorkflow); }, continueWorkflow ? "Modifiche salvate e workflow aggiornato." : "Regole editoriali salvate."); }
  ...
  async runWorkflowRequest(code) { const action = WORKFLOW_ACTION[code]; if (!action) return; const payload = {}; if (code === "namespace.revision.request_changes") { const message = this.workflowMessage.trim(); if (!message) throw new Error("Inserisci una motivazione per le modifiche richieste."); payload.message = message; } await managementRepository.namespaceWorkflow(this.id, action, payload); }
  async performWorkflow(code) { if (!WORKFLOW_ACTION[code]) return; if (code === "namespace.revision.request_changes") { this.pendingWorkflow = code; this.workflowMessage = ""; this.render(); return; } await this.execute(() => this.runWorkflowRequest(code), "Workflow delle regole editoriali aggiornato."); }
  ...
  onClick = async (event) => {
    ...
    if (add) { const { definitions } = this.snapshotDraft(); definitions[add.dataset.addDefinition].push(emptyDefinition(add.dataset.addDefinition)); this.data.revision.definitions = definitions; this.dirty = true; this.render(); requestAnimationFrame(() => this.querySelector(`[data-collection="${add.dataset.addDefinition}"] [data-definition-row]:last-of-type input[name="label"]`)?.focus()); return; }
    const remove = target.closest("[data-remove-definition]");
    if (remove) { const { definitions } = this.snapshotDraft(); definitions[remove.dataset.collection].splice(Number(remove.dataset.removeDefinition), 1); this.data.revision.definitions = definitions; this.dirty = true; this.render(); return; }
