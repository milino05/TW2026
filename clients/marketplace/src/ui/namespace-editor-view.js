...
export class ArtAroundNamespaceEditorView extends HTMLElement {
  ...
  onClick = async (event) => {
    ...
    if (add) { const { definitions } = this.snapshotDraft(); const field = add.dataset.addDefinition; definitions[field].push(emptyDefinition(field, definitions[field])); this.data.revision.definitions = definitions; this.dirty = true; this.render(); requestAnimationFrame(() => this.querySelector(`[data-collection="${field}"] [data-definition-row]:last-of-type input[name="label"]`)?.focus()); return; }
