...
export class ArtAroundNamespaceEditorView extends HTMLElement {
  data = null; busy = false; error = null; message = null; dirty = false; id = namespaceId(); activeSection = initialNamespaceSection(); editingDefinitionKey = null; tutorialOpen = false; tutorialStep = 0; tutorialReturnSection = "general"; starterDialogOpen = false; privateSuccessOpen = false;
  ...
  onClick = async (event) => {
    ...
    if (target.closest("[data-back]")) {
      if (!this.dirty) { navigate(ownerBackUrl(this.data?.namespace.owner)); return; }
      this.snapshotDraft();
    ...
    if (this.dirty) {
      this.snapshotDraft();
      const confirmed = await openActionDialog({
        title: "Salvare le modifiche e continuare?",
        message: "Il controllo finale deve usare la versione effettivamente salvata delle regole editoriali.",
        confirmLabel: "Salva e controlla",
        cancelLabel: "Continua a modificare",
        tone: "warning",
      });
      if (!confirmed) return;
      await this.saveAll({ continueWorkflow: code });
      return;
    }
