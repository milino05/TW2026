...
test("item authoring crea, proietta e rimuove collegamenti tramite revisioni immutabili del grafo", { skip: !mongoUri }, async () => {
  await withFreshDatabase(async () => {
    ...
    await assert.rejects(
      ...
      (error) => error?.status === 400 && error?.details?.some((entry) => entry.code === "UNKNOWN_FIELD" && entry.field === "unexpected"),
    );
    ...
    const [concurrentSubjectA, concurrentSubjectB] = await Subject.create([
      { preferredLabel: "Contenuto concorrente A", createdBy: owner._id },
      { preferredLabel: "Contenuto concorrente B", createdBy: owner._id },
    ]);
    const [concurrentItemA, concurrentItemB] = await ItemV2.create([
      { primarySubjectId: concurrentSubjectA._id, ownerType: "user", ownerId: owner._id, createdBy: owner._id },
      { primarySubjectId: concurrentSubjectB._id, ownerType: "user", ownerId: owner._id, createdBy: owner._id },
    ]);
    await Promise.all([
      edition(concurrentItemA, "Contenuto concorrente A"),
      edition(concurrentItemB, "Contenuto concorrente B"),
    ]);
    await Promise.all([
      connections.createItemConnection({
        itemId: sourceItem._id,
    ...
    const afterConcurrentWrites = await connections.getItemConnectionAuthoring({
      itemId: sourceItem._id,
      editionId: sourceEdition._id,
      actorUserId: owner._id,
    });
    assert.equal(afterConcurrentWrites.connections.length, 3, "le scritture concorrenti non devono sovrascriversi");
    const currentContext = await EditorialContext.findById(context._id).lean();
    assert.equal(await SemanticEdgeV2.countDocuments({ graphRevisionId: currentContext.workingGraphRevisionId }), 3);
  });
});
