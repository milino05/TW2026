...
function quizFor(plan) {
  if (plan.key === "pinacoteca") return [
    ...
    { question: "La logistica della visita è modellata come Item?", options: ["No", "Sì", "Solo nelle visite di gruppo"], correctOptionIndex: 0, points: 2 },
  ];
  return [
...
async function createVisits(plan, organization, owner, editorial, venueData) {
  ...
  for (const definition of plan.visits) {
    ...
    const revision = await VisitRevisionV2.create({ _id: demoId(`visit-revision:${plan.key}:${definition.key}`), visitId: visit._id, version: 1, title: definition.title, description: definition.description, editorialSources: [{ _id: sourceId, editorialReleaseId: editorial.editorialRelease._id }], contentEntries, visitAnchors, groupSessionDefaults: definition.groupSessionDefaults || {}, quiz: definition.quiz ? { questions: quizFor(plan) } : { questions: [] }, presentationBaseline: { depthPreference: definition.depth, languageComplexityPreference: definition.complexity, locale: "it-IT" }, logistics: { preVisitNotes: [`Percorso nella sede ${plan.venueName}.`], routeHints: [] }, status: "published", integrity: { status: "valid", issues: [], checkedAt: FIXED_NOW, checkedBy: owner._id }, review: reviewApproved(owner._id), publication: { publishedAt: FIXED_NOW, publishedBy: owner._id }, createdBy: owner._id, updatedBy: owner._id });
    ...
    const listing = await MarketplaceListing.create({ _id: demoId(`listing:${plan.key}:${definition.key}`), sellerType: "organization", sellerId: organization._id, resourceType: "visit", resourceId: visit._id, title: definition.title, summary: definition.description, catalogMetadata: { demoVenueId: venueData.venue._id, demoDataset: "v3" }, status: "published", createdBy: owner._id, publishedAt: FIXED_NOW });
...
async function verifyExamDatasetV3() {
  ...
  let totalVisits = 0; let groupPreparedVisits = 0; let totalTargets = 0; let totalItems = 0;
  for (const plan of MUSEUM_PLANS) {
    ...
    for (const visit of visits) {
      ...
      const preferredJoinAlias = String(revision.groupSessionDefaults?.preferredJoinAlias || "").trim();
      const quizQuestionCount = revision.quiz?.questions?.length || 0;
      if (preferredJoinAlias || quizQuestionCount) {
        groupPreparedVisits += 1;
        if (!preferredJoinAlias || quizQuestionCount < 3) add("GROUP_PREPARED_VISIT_INCOMPLETE", `Visita demo predisposta per il gruppo incompleta: ${plan.key}`, { visitId: visit._id });
      }
      if (revision.deliveryMode !== undefined || revision.synchronization !== undefined) add("VISIT_EXECUTION_MODE_LEAK", `VisitRevision contiene ancora configurazione runtime: ${plan.key}`, { visitId: visit._id });
  ...
  if (pinacotecaVisits.length < 3) add("PINACOTECA_THREE_VISITS_REQUIRED", "La Pinacoteca deve avere almeno tre visite demo"); if (totalVisits !== 6) add("TOTAL_VISITS_MISMATCH", "Il dataset deve contenere sei visite demo", { totalVisits }); if (groupPreparedVisits !== 2) add("GROUP_PREPARED_VISITS_MISMATCH", "Il dataset deve contenere esattamente due visite predisposte per l'uso di gruppo", { groupPreparedVisits });
  return { ok: failures.length === 0, failures, summary: { requiredUsers: Object.keys(users).length, organizations: MUSEUM_PLANS.length, venues: MUSEUM_PLANS.length, publishedItems: totalItems, activeVenueTargets: totalTargets, publishedVisits: totalVisits, groupPreparedVisits } };
}
