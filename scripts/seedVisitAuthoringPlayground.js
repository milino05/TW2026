...
async function seedVisitAuthoringPlayground({ username = "visitatore1", floorPlanRoot = undefined } = {}) {
  ...
  const organization = await Organization.findOneAndUpdate(
    ...
    {
      $set: {
        name: `Museo Playground Visite (${normalizedUsername})`,
...
async function main() {
  ...
  try {
    const result = await seedVisitAuthoringPlayground({ username });
...
module.exports = { WORKS, seedVisitAuthoringPlayground };