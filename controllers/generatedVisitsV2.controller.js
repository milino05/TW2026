...
} = require("../services/generatedPlanLifecycleV2.service");
...
async function get(req, res, next) {
  try {
    const plan = await getGeneratedPlanForUserV2({ planId: req.params.planId, userId: req.user._id });
    res.status(200).json(await projectGeneratedPlanV2(plan));
  } catch (error) { next(error); }
}
async function accept(req, res, next) {
  try {
    const plan = await acceptGeneratedPlanForUserV2({ planId: req.params.planId, userId: req.user._id });
    res.status(200).json(await projectGeneratedPlanV2(plan));
  } catch (error) { next(error); }
}
