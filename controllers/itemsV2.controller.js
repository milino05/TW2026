...
async function checkEdition(req, res, next) { try { res.status(200).json(await service.checkEditionConsistency({ editionId: req.params.editionId, actorUserId: req.user._id })); } catch (error) { next(error); } }
...
module.exports = { create, list, get, createEdition, updateEdition, checkEdition, publishEdition, fork };
