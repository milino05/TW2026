...
const instantiation = require("../services/itemInstantiationV2.service");
const itemCreation = require("../services/itemCreationV2.service");
const itemRecognitionMedia = require("../services/itemRecognitionMedia.service");
const authoring = require("../services/itemAuthoringV2.service");
const mediaUpload = require("../services/itemMediaUpload.service");
async function create(req, res, next) { try { res.status(201).json(await itemCreation.createItem({ payload: req.body || {}, actorUserId: req.user._id })); } catch (error) { next(error); } }
...
async function updateRecognitionMedia(req, res, next) { try { res.status(200).json(await itemRecognitionMedia.updateItemRecognitionMedia({ itemId: req.params.itemId, payload: req.body || {}, actorUserId: req.user._id })); } catch (error) { next(error); } }
...
async function fork(req, res, next) { try { res.status(201).json(await instantiation.forkItem({ sourceItemId: req.params.itemId, sourceEditionId: req.body?.sourceEditionId, ownerType: req.body?.ownerType, ownerId: req.body?.ownerId, contentSpaceId: req.body?.contentSpaceId, actorUserId: req.user._id })); } catch (error) { next(error); } }
