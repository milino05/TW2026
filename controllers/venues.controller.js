...
async function ensureWorkingRelease(req, res, next) { try { res.status(200).json(await venueReleaseService.ensureWorkingVenueRelease({ venueId: req.params.venueId, actorUserId: req.user._id })); } catch (error) { next(error); } }
