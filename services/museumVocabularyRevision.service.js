...
async function createInitialVocabularyForMuseum({ museumId, actorUserId, initialPayload = null }) {
  ...
  if (existing) throw new AppError("Il museo possiede gia un vocabolario semantico", 409);
  const museum = await Museum.findById(museumId).select("_id").lean();
  if (!museum) throw new AppError("Museo non trovato", 404);

  const normalized = initialPayload
    ? { ...EMPTY_VOCABULARY, ...normalizeVocabularyPayload(initialPayload) }
    : { ...EMPTY_VOCABULARY };
...
async function loadStableOrFail(museumId) {
  ...
  if (!stable) throw new AppError("Vocabolario semantico non inizializzato", 404);
  return stable;
}
...
async function getVocabularyRevision({ museumId, userId = null, view = "published" }) {
  const museum = await Museum.findById(museumId).select("_id").lean();
  if (!museum) throw new AppError("Museo non trovato", 404);
  const stable = await loadStableOrFail(museumId);
  if (view === "working") await assertMuseumRole({ userId, museumId, minimumRole: "operator" });
  const revisionId = view === "working"
    ? stable.workingRevisionId || (await createWorkingFromPublished(stable, userId))._id
    : stable.publishedRevisionId;
  ...
  return {
    stable,
    revision,
    vocabulary: materializeVocabulary({ museumId, version: revision.version, revisionId: revision._id, source: revision }),
  };
}
...
async function updateVocabularyDraft({ museumId, payload, userId }) {
  ...
  // Le working revision possono essere incomplete durante l'authoring.
  // La validazione completa e bloccante avviene con check-consistency/review/publish.
...
function removedKeys(previous = [], next = []) {
  const nextSet = new Set((next || []).map((entry) => entry.key));
  return (previous || []).map((entry) => entry.key).filter((key) => !nextSet.has(key));
}
async function dependencyIssues({ museumId, published, working }) {
  ...
  const [variantLanguages, variantDurations, relationKeys, aspectKeys, itemTypes, visitLanguages, visitDurations] = await Promise.all([
  ...
  const used = {
    languageLevels: new Set([...variantLanguages, ...visitLanguages]),
    durationTypes: new Set([...variantDurations, ...visitDurations]),
  ...
  for (const [group, keys] of Object.entries(removed)) {
    for (const key of keys) {
      if (used[group].has(key)) issues.push({
        field: group,
...
async function publishVocabulary({ museumId, userId }) {
  ...
  await revision.save();
  const pointer = await MuseumVocabulary.updateOne(
    { _id: stable._id, workingRevisionId: revision._id },
