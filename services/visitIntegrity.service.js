...
async function loadWorking(visitId) {
  const visit = await findVisitOrFail(visitId);
  if (!visit.workingRevisionId) throw new AppError("La visita non ha una revisione di lavoro", 409);
...
async function evaluateVisitConsistency({ visitId, actorUserId, allowInReview = false }) {
  const { visit, revision } = await loadWorking(visitId);
  await assertVisitEditor({ visit, actorUserId });
  if (revision.status === "in_review" && !allowInReview) {
    throw new AppError("Ritirare la richiesta prima di ricontrollare", 409);
  }
  ...
  revision.estimatedContentSeconds = result.estimatedContentSeconds;
...
async function checkVisitConsistency({ visitId, actorUserId }) {
  return evaluateVisitConsistency({ visitId, actorUserId, allowInReview: false });
}

async function requestVisitReview({ visitId, actorUserId }) {
  const consistency = await checkVisitConsistency({ visitId, actorUserId });
  if (consistency.visit.kind !== "official") throw new AppError("Le visite community non usano la revisione manageriale", 409);
  if (consistency.issues.some((entry) => entry.severity !== "warning")) {
    throw new AppError("Impossibile richiedere la revisione con problemi di integrita", 400, consistency.issues);
  }
  try {
    requestReview(consistency.revision, actorUserId);
  } catch (error) {
...
async function withdrawVisitReview({ visitId, actorUserId }) {
  const { visit, revision } = await loadWorking(visitId);
  await assertVisitEditor({ visit, actorUserId });
  if (visit.kind !== "official") throw new AppError("Operazione non applicabile alle visite community", 409);
  try {
    withdrawReview(revision, actorUserId);
  } catch (error) {
    throw new AppError(error.message, 409, [{ code: error.code }]);
  }
  await revision.save();
...
async function requestVisitChanges({ visitId, actorUserId, message }) {
  const { visit, revision } = await loadWorking(visitId);
  if (visit.kind !== "official") throw new AppError("Operazione non applicabile alle visite community", 409);
  await assertMuseumRole({ userId: actorUserId, museumId: visit.ownerMuseumId, minimumRole: "manager" });
  try {
    requestChanges(revision, actorUserId, message);
  } catch (error) {
    throw new AppError(error.message, 409, [{ code: error.code }]);
  }
  await revision.save();
...
async function publishVisit({ visitId, actorUserId }) {
  ...
  if (initial.visit.kind === "official") {
    await assertMuseumRole({
      userId: actorUserId,
      museumId: initial.visit.ownerMuseumId,
      minimumRole: "manager",
    });
  } else {
    await assertVisitEditor({ visit: initial.visit, actorUserId });
  }

  const consistency = await evaluateVisitConsistency({
    visitId,
    actorUserId,
    allowInReview: initial.visit.kind === "official",
  });
  const { visit, revision, issues } = consistency;
  if (issues.some((entry) => entry.severity !== "warning")) {
    throw new AppError("Impossibile pubblicare una visita con problemi di integrita", 400, issues);
  }
  if (visit.kind === "community" && revision.status === "in_review") {
    throw new AppError("Stato non valido per una visita community", 409);
  }
  ...
  try {
    markPublished(revision, actorUserId);
  } catch (error) {
    throw new AppError(error.message, 409, [{ code: error.code }]);
  }
  await revision.save();

  const update = await Visit.updateOne(
  ...
  if (previousPublishedId) {
    await VisitRevision.updateOne({ _id: previousPublishedId, status: "published" }, { $set: { status: "superseded" } });
  }
