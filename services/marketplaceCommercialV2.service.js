...
const { availablePrincipalProjection } = require("./marketplaceWorkspaceV2.service");
...
function versionPolicyOptions(resourceType) {
  if (LIVE_RESOURCE_TYPES.has(resourceType)) {
    return [
      { code: "follow_current", label: "Segui gli aggiornamenti futuri" },
      { code: "pin_at_acquisition", label: "Fissa la versione al momento dell'acquisizione" },
    ];
  }
...
async function getCommercialManagement({ actorUserId, principalType = "user", principalId = actorUserId, page = 1, limit = 10 }) {
  ...
  for (const listing of listings) {
    let asset = {
      ...
      title: listing.title || "Asset Marketplace",
  ...
  return {
    principal: selected || { type: principalType, id: principalId, name: principalType === "user" ? "Profilo personale" : "Organization" },
    availablePrincipals,
...
module.exports = { getCommercialManagement, capabilityOptions, versionPolicyOptions };
