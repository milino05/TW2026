...
const { validateSemanticGraphSnapshot, writeSemanticGraphSnapshot } = require("./semanticGraphSnapshotWriter.service");
const { loadSemanticGraphRevision } = require("./semanticGraphV2.service");
const { clearLocalEdgeSuppression } = require("./editorialGraphImport.service");
...
function normalizeSubjectItemSelections(payload) {
  ...
  if (selections !== undefined && !Array.isArray(selections)) throw new AppError("subjectItemSelections deve essere un array", 400, [{ field: "subjectItemSelections", code: "INVALID_TYPE" }]);
...
function applyClassAssignments(snapshot, assignments) {
  for (const assignment of assignments) ensureBinding(snapshot, assignment.subjectId).subjectClassDefinitionIds = assignment.subjectClassDefinitionIds;
}
function relationDefinition(namespaceRevision, relationTypeDefinitionId) {
  return (namespaceRevision?.relationTypes || []).find((entry) => String(entry.definitionId) === String(relationTypeDefinitionId)) || null;
}
function sameRelationEdge(edge, sourceSubjectId, targetSubjectId, relationTypeDefinitionId, relation) {
  ...
  return relation?.directionality === "symmetric" && sameId(edge.sourceSubjectId, targetSubjectId) && sameId(edge.targetSubjectId, sourceSubjectId);
}
async function collectionSubjectIds(editorialContextId, session) {
  const memberships = await CollectionItemMembership.find({ editorialContextId }).select("itemId").session(session).lean();
  if (!memberships.length) return new Set();
  const items = await ItemV2.find({ _id: { $in: memberships.map((entry) => entry.itemId) }, lifecycleStatus: "active" }).select("primarySubjectId").session(session).lean();
...
async function itemCandidatesForSubject({ context, subjectId, session }) {
  const items = await ItemV2.find({ primarySubjectId: subjectId, lifecycleStatus: "active" }).select("_id primarySubjectId").session(session).lean();
  if (!items.length) return [];
  const memberships = await ContentSpaceItemMembership.find({ contentSpaceId: context.contentSpaceId, itemId: { $in: items.map((item) => item._id) } }).select("itemId").session(session).lean();
...
async function resolveCollectionItemForSubject({ state, context, subjectId, itemId, actorUserId, session }) {
  ...
  if (!candidates.length) throw new AppError("Il Subject selezionato non ha contenuti utilizzabili nello Spazio editoriale", 409, [{ field: "subjectId", code: "COLLECTION_GRAPH_SUBJECT_CONTENT_REQUIRED", context: { subjectId } }]);
  let selected = null;
  if (itemId) {
    assertObjectId(itemId, "itemId");
    selected = candidates.find((item) => sameId(item._id, itemId)) || null;
    if (!selected) throw new AppError("Il contenuto scelto non rappresenta il Subject o non appartiene allo Spazio editoriale", 409, [{ field: "itemId", code: "COLLECTION_GRAPH_SUBJECT_ITEM_INVALID", context: { subjectId, itemId } }]);
  } else if (candidates.length === 1) [selected] = candidates;
  else throw new AppError("Scegli quale contenuto aggiungere alla Raccolta per questo Subject", 409, [{ field: "itemId", code: "COLLECTION_GRAPH_SUBJECT_ITEM_SELECTION_REQUIRED", context: { subjectId, itemIds: candidates.map((item) => item._id) } }]);
  await assertCanReferenceItemInEditorialSpace({ itemId: selected._id, actorUserId, principalType: state.contentSpace.ownerType, principalId: state.contentSpace.ownerId });
  return selected;
}
...
async function addCollectionGraphEdge({ editorialContextId, payload, actorUserId }) {
  ...
  state.snapshot.edges.push(newEdge);

  const graphIssues = await validateSemanticGraphSnapshot({ snapshot: state.snapshot, namespaceRevision: state.namespaceRevision });
  if (graphIssues.length) throw new AppError("Il grafo non rispetta le regole editoriali", 409, graphIssues);
  ...
  try {
    await mongoose.connection.transaction(async (session) => {
      const context = await EditorialContext.findOne({ _id: state.context._id, lifecycleStatus: "active", semanticGraphId: state.semanticGraph._id }).session(session);
      ...
      if (missingSubjects.length === endpoints.length) throw new AppError("Almeno uno dei Subject collegati deve essere già rappresentato nella Raccolta", 409, [{ code: "COLLECTION_GRAPH_ANCHOR_REQUIRED" }]);
      if (missingSubjects.length && context.activeReviewRevisionId) throw new AppError("La Raccolta è in revisione: ritira la richiesta prima di modificarne i contenuti", 409, [{ code: "EDITORIAL_CONTEXT_REVIEW_LOCKED" }]);

      const selectedItems = [];
      for (const subjectId of missingSubjects) {
        const selectedItem = await resolveCollectionItemForSubject({ state, context, subjectId, itemId: subjectItemSelections.get(subjectId) || null, actorUserId, session });
      ...
      if (selectedItems.length) {
        ...
        if (contextPointer.modifiedCount !== 1) throw new AppError("La Raccolta è stata modificata da un'altra operazione", 409, [{ code: "EDITORIAL_CONTEXT_WORKING_CONFLICT" }]);
        allowedSubjects = new Set([...allowedSubjects, ...missingSubjects]);
      }

      const unsupported = state.snapshot.subjectBindings.map((binding) => id(binding.subjectId)).filter((subjectId) => !allowedSubjects.has(subjectId));
      if (unsupported.length) throw new AppError("Il grafo non rispetta i contenuti della Raccolta", 409, unsupported.map((subjectId) => ({ field: "subjectId", code: "GRAPH_SUBJECT_WITHOUT_COLLECTION_CONTENT", context: { subjectId } })));
      ...
      const revision = await writeSemanticGraphSnapshot({
        ...
        actorUserId,
        session,
        afterPersist: ({ session: activeSession }) => clearLocalEdgeSuppression({
          editorialContextId: context._id,
          edge: newEdge,
          namespaceRevision: state.namespaceRevision,
    ...
    if (error?.code === 11000) throw new AppError("Il contenuto è già entrato nella Raccolta durante questa operazione: riprova", 409, [{ code: "COLLECTION_ITEM_MEMBERSHIP_CONFLICT" }]);
    throw error;
  }
  return loadSemanticGraphRevision(revisionId, { bypassCache: true });
}

module.exports = { addCollectionGraphEdge };
