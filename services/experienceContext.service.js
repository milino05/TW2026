...
async function resolveExperienceContext({ userId, museumId, request = {} }) {
  const [user, profile, declaredGenerator, population] = await Promise.all([
  ...
  if (request.visitDensity === undefined && declaredGenerator?.visitDensity == null && (learnedMore !== null || learnedSkip !== null)) { visitDensity.source = "learned_history"; visitDensity.confidence = Math.max(profile?.behavior?.moreDetailRequestRate?.confidence || 0, profile?.behavior?.optionalStopSkipRate?.confidence || 0); }
