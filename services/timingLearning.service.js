...
async function updateItemObservationProfiles({ session, personalObservationBase }) {
  const valid = (session.physicalTargetObservations || []).filter((entry) => (entry.reliability || 0) >= policy.learning.minimumReliability);
  const groups = groupBy(valid, (entry) => String(entry.itemId));
  for (const observations of groups.values()) {
    const first = observations[0], observed = robustMedian(observations.map((entry) => Number(entry.observedSeconds)));
    if (!Number.isFinite(observed)) continue;
    const factor = Number.isFinite(personalObservationBase) && personalObservationBase > 0 ? observed / personalObservationBase : 1;
    const factorScope = `item:${first.itemId}:factor`, secondsScope = `item:${first.itemId}:seconds`;
...
async function updateVisitTimingProfile({ session, summary }) {
  ...
  const residualScope = `visit:${session.visitRevisionId}:residual`, totalScope = `visit:${session.visitRevisionId}:total`;
  const sampleCount = Math.max(1, summary.validTransitionCount + summary.validContentEntryCount + summary.validTargetObservationCount);
