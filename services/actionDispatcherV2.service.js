...
function normalizeExpectedRuntimeVersion(value) {
  ...
  if (!Number.isInteger(version) || version < 1) {
    throw new AppError("expectedRuntimeVersion e obbligatoria", 400, [{
      field: "expectedRuntimeVersion",
      code: "REQUIRED",
    }]);
  }
  return version;
...
function normalizeInteractionChannel(value) {
  ...
  if (!INTERACTION_CHANNELS.has(channel)) {
    throw new AppError("interactionChannel non valido", 400, [{
      field: "interactionChannel",
      code: "INVALID_INTERACTION_CHANNEL",
    }]);
  }
  return channel;
}
...
async function recordRejectedUnavailable({ sessionId, userId, actionId, interactionChannel }) {
  await VisitSessionV2.updateOne(
    ...
    { $push: { interactionEvents: interactionEvent({
      userId,
      actionId,
      interactionChannel,
      status: "rejected",
...
async function claimRuntimeVersion({ sessionId, userId, expectedRuntimeVersion }) {
  ...
  throw new AppError("La Session e stata modificata", 409, [{
    ...
    context: { currentRuntimeVersion: current.runtimeVersion },
  }]);
}
...
async function executeDescriptor({ sessionId, userId, descriptor }) {
  switch (descriptor.type) {
    case "PROGRESS_NEXT":
      await advanceSession({ sessionId, userId, direction: "next" });
      return null;
    case "PROGRESS_PREVIOUS":
      await advanceSession({ sessionId, userId, direction: "previous" });
      return null;
    case "PRESENTATION_DEPTH_INCREASE":
      await changePresentationDepthV2({ sessionId, userId, direction: "up" });
      return null;
    case "PRESENTATION_DEPTH_DECREASE":
      await changePresentationDepthV2({ sessionId, userId, direction: "down" });
      return null;
    case "PRESENTATION_COMPLEXITY_INCREASE":
      await changePresentationComplexityV2({ sessionId, userId, direction: "up" });
      return null;
    ...
    case "PAUSE":
      ...
      return null;
    case "RESUME":
      await resumeSessionV2({ sessionId, userId });
      return null;
    case "COMPLETE": {
    ...
    case "NAVIGATE_TO_PLACE_INTENT":
      await routeToIntentV2({ sessionId, userId, intent: descriptor.serverInput?.intent });
      return { type: "navigation_requested", label: descriptor.label };
    default:
...
async function dispatchAction({ sessionId, userId, payload = {} }) {
  ...
  if (!actionId) {
    throw new AppError("actionId e obbligatorio", 400, [{ field: "actionId", code: "REQUIRED" }]);
  }
  ...
  if (derived.session.runtimeVersion !== expectedRuntimeVersion) {
    throw new AppError("La Session e stata modificata", 409, [{
      code: "RUNTIME_VERSION_CONFLICT",
      context: { currentRuntimeVersion: derived.session.runtimeVersion },
    }]);
  }
