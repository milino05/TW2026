...
function validatePresentationAgainstNamespace(revision, namespaceRevision) {
  const issues = []; const add = (field, code, message) => issues.push({ field, code, message, severity: "error" }); const defs = definitionSets(namespaceRevision); const variantIds = new Set(); const representationIds = new Set(); const variantKeys = new Set(); let representationCount = 0;
  ...
  else if (!representationCount) add("presentationVariants[0].representations", "EMPTY_REPRESENTATIONS", "Aggiungi almeno un testo completo prima di pubblicare");
