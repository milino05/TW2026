...
function buildSignature({ vocabulary, items }) { const pointers = items.map((item) => `${id(item._id)}:${id(item.publishedRevisionId)}`).sort().join("|"); return `${id(vocabulary.vocabularyRevisionId)}::published::${pointers}`; }
...
async function materializeGraph({ museumId, view, vocabulary, items, revisionIds }) {
  ...
  for (const item of items) {
    const revisionId = selectedRevisionId(item, view), revision = revisionById.get(id(revisionId));
...
async function loadMuseumSemanticGraph(museumId, { view = "published", bypassCache = false } = {}) {
  ...
  // Le working revision sono mutabili mentre mantengono lo stesso pointer: per
  // correttezza editoriale non vengono mai cacheate. La cache e riservata ai
  // published graph, i cui edge set sono immutabili per revision ID.
  if (view === "working") return materializeGraph({ museumId, view, vocabulary, items, revisionIds });
  const signature = buildSignature({ vocabulary, items }), key = cacheKey(museumId), cached = graphCache.get(key);
  if (!bypassCache && cached?.signature === signature) { touchCache(key, cached); return cached.graph; }
...
async function loadRevisionSemanticEdges({ museumId, sourceItemId, sourceItemRevisionId }) {
  const [vocabulary, persistedEdges] = await Promise.all([
    getMuseumVocabulary(museumId),
    SemanticEdge.find({ museumId, sourceItemId, sourceItemRevisionId }).lean(),
  ]);
  const typeByKey = new Map((vocabulary.relationTypes || []).map((type) => [type.key, type]));
  const result = [];
  for (const edge of persistedEdges) {
    const type = typeByKey.get(edge.relationTypeKey);
    if (type) result.push(materializeDirectEdge(edge, type));
  }
  return result;
}
...
module.exports = {
  id, semanticRefKey, featureKey, relationStrength, edgeWeight,
  loadMuseumSemanticGraph, loadRevisionSemanticEdges, invalidateMuseumSemanticGraphCache,
  resolveFeatureToItemIds, neighbors, outgoingEdges, incomingEdges, shortestSemanticPath, featureMatchesNode,
};
