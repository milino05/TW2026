...
function appendIssue(revision, issue) {
  ...
  const duplicate = issues.some((entry) => entry.code === issue.code && String(entry.context?.layoutRevisionId || "") === String(issue.context?.layoutRevisionId || ""));
  revision.integrity = { status: "needs_review", issues: duplicate ? issues : [...issues, issue], checkedAt: null, checkedBy: null };
}
...
async function ensureRepairDraft({ visit, publishedRevision, issue }) {
  if (visit.workingRevisionId) {
    const working = await VisitRevision.findById(visit.workingRevisionId);
    if (working) { appendIssue(working, issue); working.baselineTiming = {}; await working.save(); return working; }
  }
  const working = new VisitRevision({ visitId: visit._id, version: await nextVersion(visit._id), basedOnRevisionId: publishedRevision._id, ...cloneRevisionData(publishedRevision), status: "draft", integrity: { status: "needs_review", issues: [issue] }, createdBy: publishedRevision.updatedBy || publishedRevision.createdBy, updatedBy: publishedRevision.updatedBy || publishedRevision.createdBy });
  await working.save();
...
async function checkAgainstLayout({ revision, museumId, layoutRevision }) {
  ...
  for (let index = 0; index < stopItems.length; index += 1) {
    ...
    if (!item || String(item.museumId) !== String(museumId)) continue;
    if (!placements.has(String(item._id))) problems.push({ code: "ITEM_PLACEMENT_MISSING", stopIndex: index, itemId: item._id });
  }
  for (let index = 0; index < stopItems.length - 1; index += 1) {
    ...
    if (!route.reachable) problems.push({ code: "ROUTE_NOT_AVAILABLE", fromStopIndex: index, toStopIndex: index + 1 });
  }
  return problems;
}
...
async function propagateLayoutPublication({ museumId, newLayoutRevisionId, previousLayoutRevisionId = null }) {
  ...
  for (const revision of revisions) {
    ...
    if (blocking && revision.status === "published") {
      ...
      if (visit) { await ensureRepairDraft({ visit, publishedRevision: revision, issue }); visit.publishedRevisionId = null; await visit.save(); unpublishedCount += 1; }
    }
  }
