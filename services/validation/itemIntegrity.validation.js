...
function validateRepresentationIntegrity({ representation, index, vocabulary, issues }) {
  ...
  if (!representation.languageLevelKey || typeof representation.languageLevelKey !== "string") {
    pushIssue(issues, `${basePath}.languageLevelKey`, "MISSING_LANGUAGE_LEVEL_KEY", "languageLevelKey e obbligatoria per pubblicare");
  } else if (!allowedLanguageLevelKeys.has(representation.languageLevelKey)) {
    pushIssue(issues, `${basePath}.languageLevelKey`, "INVALID_LANGUAGE_LEVEL_KEY", `languageLevelKey non valida: ${representation.languageLevelKey}`, {
  ...
  if (!representation.durationKey || typeof representation.durationKey !== "string") {
    pushIssue(issues, `${basePath}.durationKey`, "MISSING_DURATION_KEY", "durationKey e obbligatoria per pubblicare");
  } else if (!allowedDurationKeys.has(representation.durationKey)) {
    pushIssue(issues, `${basePath}.durationKey`, "INVALID_DURATION_KEY", `durationKey non valida: ${representation.durationKey}`, {
  ...
  if (!representation.text || typeof representation.text !== "string") {
    pushIssue(issues, `${basePath}.text`, "MISSING_TEXT", "Il testo della representation e obbligatorio per pubblicare");
  }
}
...
async function computeItemIntegrityIssues({ item, museumId, vocabulary }) {
  ...
  if (!item.itemType || typeof item.itemType !== "string") {
    ...
    pushIssue(issues, "itemType", "INVALID_ITEM_TYPE", `itemType non presente nel config: ${item.itemType}`);
  }

  if (!Array.isArray(item.representations) || item.representations.length === 0) {
    pushIssue(issues, "representations", "MISSING_REPRESENTATIONS", "Almeno una representation completa e obbligatoria per pubblicare");
  } else {
    ...
    item.representations.forEach((representation, index) => {
      ...
      if (representation.languageLevelKey && representation.durationKey) {
        ...
        if (seenPairs.has(pairKey)) {
          pushIssue(issues, `representations[${index}]`, "DUPLICATE_REPRESENTATION", `Coppia languageLevelKey/durationKey duplicata: ${pairKey}`);
        } else {
    ...
    if (defaultCount > 1) {
      pushIssue(issues, "representations", "MULTIPLE_DEFAULTS", "E consentita una sola representation di default");
    }
  }

  const relations = Array.isArray(item.relations) ? item.relations : [];
  const targetIds = [...new Set(relations.map((relation) => relation?.target).filter((target) => mongoose.isValidObjectId(target)).map(String))];
  const targetItems = await Item.find({ _id: { $in: targetIds } }).select("_id museumId itemType").lean();
  ...
  relations.forEach((relation, index) => {
    ...
    if (!relation.relationTypeKey || !relationType) {
      pushIssue(issues, `${basePath}.relationTypeKey`, "INVALID_RELATION_TYPE", `relationTypeKey non valida: ${relation.relationTypeKey}`);
      return;
    }

    if (relationType.domain?.length && !relationType.domain.includes(item.itemType)) {
      pushIssue(issues, `${basePath}.relationTypeKey`, "INVALID_RELATION_DOMAIN", `La relazione ${relation.relationTypeKey} non e compatibile con itemType ${item.itemType}`);
    }

    const count = occurrences.get(relation.relationTypeKey) || 0;
    if (relationType.validationRules?.allowMultiple === false && count >= 1) {
      pushIssue(issues, basePath, "MULTIPLE_RELATIONS_NOT_ALLOWED", `La relazione ${relation.relationTypeKey} ammette un solo target`);
    }
    occurrences.set(relation.relationTypeKey, count + 1);

    if (!relation.target || !mongoose.isValidObjectId(relation.target)) {
      pushIssue(issues, `${basePath}.target`, "INVALID_OBJECT_ID", "target deve essere un ObjectId valido");
      return;
    }

    if (String(relation.target) === String(item._id)) {
      pushIssue(issues, `${basePath}.target`, "SELF_RELATION", "Un item non puo essere in relazione con se stesso");
      return;
    }

    const targetItem = targetsById.get(String(relation.target));
    if (!targetItem) {
      pushIssue(issues, `${basePath}.target`, "TARGET_NOT_FOUND", "L'item target non esiste piu");
      return;
    }

    if (String(targetItem.museumId) !== String(museumId)) {
      pushIssue(issues, `${basePath}.target`, "CROSS_MUSEUM_TARGET", "Il target appartiene a un museo diverso");
      return;
    }

    if (relationType.range?.length && !relationType.range.includes(targetItem.itemType)) {
      pushIssue(issues, `${basePath}.target`, "INVALID_RELATION_RANGE", `Il target di tipo ${targetItem.itemType} non e compatibile con la relazione ${relation.relationTypeKey}`);
    }

    if (relation.weight !== undefined) {
      const weight = Number(relation.weight);
      if (!Number.isFinite(weight) || weight < 0 || weight > 10) {
        pushIssue(issues, `${basePath}.weight`, "INVALID_WEIGHT", "weight deve essere compreso tra 0 e 10");
      }
    }
