...
function normalizeEditorialContextPayload(payload = {}) {
  ...
  if (hasOwn(payload, "semanticGraphId")) normalized.semanticGraphId = payload.semanticGraphId;
...
function validateEditorialContextPayload({ payload = {}, rawPayload = payload, creating = false }) {
  ...
  const allowed = creating
    ? ["contentSpaceId", "namespaceId", "semanticGraphId", "displayName", "shortDescription", "description"]
  ...
  if (creating) {
    ...
    if (!mongoose.isValidObjectId(payload.semanticGraphId)) issues.push({ field: "semanticGraphId", code: "INVALID_OBJECT_ID", message: "semanticGraphId non valido" });
  }
