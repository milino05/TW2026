...
async function validateLogistics({ revision, stopItems, issues }) {
  const transitions = revision.logistics?.transitions || []; const transitionKeys = new Set(); const layoutCache = new Map();
  ...
  for (let index = 0; index < transitions.length; index += 1) {
    ...
    const fromItem = stopItems[transition.fromStopIndex]; const toItem = stopItems[transition.toStopIndex]; if (!fromItem || !toItem) continue; const sameMuseum = String(fromItem.museumId) === String(toItem.museumId);
    ...
    if (transition.type === "inter_venue") { if (sameMuseum) issues.push(issue(`${field}.type`, "INTER_VENUE_SAME_MUSEUM", "Una transizione inter_venue e superflua tra item dello stesso museo", "warning")); if (!transition.instructionOverride) issues.push(issue(`${field}.instructionOverride`, "TRANSFER_INSTRUCTION_RECOMMENDED", "E consigliata un'indicazione per il trasferimento tra musei", "warning")); continue; }
    const current = await currentLayout(fromItem.museumId); if (!current) { issues.push(issue(`${field}.layoutRevisionId`, "LAYOUT_NOT_AVAILABLE", "Nessun layout pubblicato disponibile per questa transizione")); continue; }
    const layoutRevision = current.revision; if (transition.layoutRevisionId && String(transition.layoutRevisionId) !== String(layoutRevision._id)) issues.push(issue(`${field}.layoutRevisionId`, "STALE_LAYOUT_REVISION", "La transizione riferisce una revisione layout non piu pubblicata", "warning"));
    const placements = new Map((layoutRevision.itemPlacements || []).map((placement) => [String(placement.itemId), placement])); const fromPlacement = placements.get(String(fromItem._id)); const toPlacement = placements.get(String(toItem._id)); if (!fromPlacement || !toPlacement) { issues.push(issue(field, "ITEM_PLACEMENT_MISSING", "Una delle tappe non ha una posizione nel layout pubblicato corrente")); continue; }
    if (transition.plannedPath?.length) { const planned = resolvePlannedPath({ connections: layoutRevision.connections, pathConnectionIds: transition.plannedPath, fromPlaceId: fromPlacement.primaryPlaceId, toPlaceId: toPlacement.primaryPlaceId }); if (!planned.reachable) issues.push(issue(`${field}.plannedPath`, "INVALID_PLANNED_PATH", "Il percorso pianificato non e valido sul layout pubblicato corrente")); }
  }
  for (let index = 0; index < stopItems.length - 1; index += 1) {
    const fromItem = stopItems[index]; const toItem = stopItems[index + 1]; if (!fromItem || !toItem || String(fromItem.museumId) !== String(toItem.museumId)) continue; const current = await currentLayout(fromItem.museumId); if (!current) { issues.push(issue(`stops[${index}]`, "LAYOUT_NOT_AVAILABLE", "Il museo non dispone di un layout pubblicato per il routing")); continue; }
    const placements = new Map((current.revision.itemPlacements || []).map((placement) => [String(placement.itemId), placement])); const fromPlacement = placements.get(String(fromItem._id)); const toPlacement = placements.get(String(toItem._id)); if (!fromPlacement || !toPlacement) { issues.push(issue(`stops[${index}]`, "ITEM_PLACEMENT_MISSING", "Le tappe consecutive devono essere localizzate nel layout pubblicato corrente")); continue; }
    const route = resolveRoute({ connections: current.revision.connections, fromPlaceId: fromPlacement.primaryPlaceId, toPlaceId: toPlacement.primaryPlaceId }); if (!route.reachable) issues.push(issue(`stops[${index}]`, "ROUTE_NOT_AVAILABLE", "Non esiste alcun percorso nel grafo corrente tra le due tappe consecutive"));
  }
}

async function computeVisitIntegrity({ visit, revision }) {
  const issues = []; const museumIds = new Set(); let estimatedContentSeconds = 0;
  if (!revision.title) issues.push(issue("title", "REQUIRED", "title e obbligatorio")); if (!Array.isArray(revision.stops) || !revision.stops.length) { issues.push(issue("stops", "EMPTY_ARRAY", "La visita deve contenere almeno una tappa")); return { issues, museumIds: [], estimatedContentSeconds }; }
  ...
  for (let index = 0; index < revision.stops.length; index += 1) {
    ...
    if (visit.kind === "official") { selected = findRepresentationByPolicy(itemRevision, revision.defaultPresentationPolicy || {}); if (!selected) { issues.push(issue(`stops[${index}]`, "DEFAULT_POLICY_NOT_AVAILABLE", "L'item non supporta la policy ufficiale della visita")); continue; } }
    else { vocabulary = await getMuseumVocabulary(item.museumId); selected = findDefaultRepresentation(itemRevision); if (!selected) { issues.push(issue(`stops[${index}]`, "ITEM_DEFAULT_NOT_AVAILABLE", "Un item community deve avere una presentazione di default")); continue; } }
  ...
  await validateLogistics({ revision, stopItems, issues }); return { issues, museumIds: [...museumIds], estimatedContentSeconds };
}
