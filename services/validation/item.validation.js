...
function normalizeItemPayload(payload = {}) {
  return {
    externalId:
      typeof payload.externalId === "string" ? payload.externalId.trim() : payload.externalId,

    itemType:
      typeof payload.itemType === "string" ? payload.itemType.trim() : payload.itemType,
    ...
    representations: Array.isArray(payload.representations)
      ? payload.representations.map((rep) => ({
        languageLevel:
          typeof rep.languageLevel === "string"
            ? rep.languageLevel.trim()
            : rep.languageLevel,
        duration:
          typeof rep.duration === "string" ? rep.duration.trim() : rep.duration,
    ...
    relations: Array.isArray(payload.relations)
      ? payload.relations.map((rel) => ({
        ...
        target: rel.target,
        weight:
          typeof rel.weight === "number" ? rel.weight : 1,
      }))
      : [],
  };
}
...
function validateTopLevelFields(payload, vocabulary, errors) {
  ...
  if (!vocabulary.itemTypes.includes(payload.itemType)) {
    pushError(
      ...
      `itemType non valido: ${payload.itemType}`,
...
function validateRepresentations(representations, vocabulary, errors) {
  if (!Array.isArray(representations)) {
    pushError(
      ...
      "representations",
      "INVALID_TYPE",
      "representations deve essere un array"
    );
    return;
  }
  ...
  representations.forEach((rep, index) => {
    ...
    if (!rep.languageLevel || typeof rep.languageLevel !== "string") {
      ...
      pushError(
        ...
        `${basePath}.languageLevel`,
        "INVALID_CONTROLLED_VALUE",
        `languageLevel non valido: ${rep.languageLevel}`,
    ...
    if (!rep.duration || typeof rep.duration !== "string") {
      ...
      pushError(
        ...
        `${basePath}.duration`,
        "INVALID_CONTROLLED_VALUE",
        `duration non valida: ${rep.duration}`,
        { allowedValues: vocabulary.durationTypes }
      );
    }
    ...
    if (!rep.text || typeof rep.text !== "string") {
      pushError(
        ...
        `${basePath}.text`,
        "REQUIRED",
        "Il testo della representation è obbligatorio"
      );
    }
    ...
    if (
      ...
      pushError(
        ...
        `Coppia languageLevel/duration duplicata: ${pairKey}`
      );
    } else {
  ...
  if (defaultCount > 1) {
    pushError(
      ...
      "representations",
      "MULTIPLE_DEFAULTS",
      "È consentita una sola representation di default"
    );
  }
}
...
async function validateRelations({ museumId, itemType, relations, vocabulary, errors, currentItemId = null }) {
  ...
  const relationTypesById = new Map(
    vocabulary.relationTypes.map((rt) => [String(rt._id), rt])
  );
  ...
  for (let index = 0; index < relations.length; index += 1) {
    ...
    if (!mongoose.isValidObjectId(rel.relationType)) {
      pushError(
        errors,
        `${basePath}.relationType`,
        "INVALID_OBJECT_ID",
        "relationType non è un ObjectId valido"
      );
      continue;
    }
    ...
    if (!relationType) {
      pushError(
        ...
        "Il tipo di relazione non appartiene al vocabolario del museo"
      );
      continue;
    }
    ...
    if (
      ...
      !relationType.domain.includes(itemType)
    ) {
      pushError(
        errors,
        `${basePath}.relationType`,
        "INVALID_DOMAIN",
        `La relazione ${relationType.label} non è applicabile a itemType ${itemType}`
      );
    }
    ...
    if (!mongoose.isValidObjectId(rel.target)) {
      pushError(
        ...
        `${basePath}.target`,
        "INVALID_OBJECT_ID",
        "target non è un ObjectId valido"
      );
      continue;
    }
    ...
    if (currentItemId && String(rel.target) === String(currentItemId)) {
      pushError(
        ...
        `${basePath}.target`,
        "SELF_RELATION",
        "Un item non può essere in relazione con sé stesso"
      );
      continue;
    }

    const targetItem = await Item.findById(rel.target)
      .select("_id itemType museumId")
      .lean();
    ...
    if (
      ...
      pushError(
        ...
        `${basePath}.target`,
        "INVALID_RANGE",
        `Il target di tipo ${targetItem.itemType} non è compatibile con la relazione ${relationType.label}`
      );
    }
...
async function validateItemPayload({
  museumId,
  payload,
  vocabulary,
  currentItemId = null,
