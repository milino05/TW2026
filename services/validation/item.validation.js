...
function normalizeRepresentation(value) {
  ...
  return {
    ...
    ...(hasOwn(value, "isDefault") ? { isDefault: normalizeBoolean(value.isDefault) === true } : {}),
  };
}
...
function normalizeItemPayload(payload = {}) {
  ...
  if (hasOwn(payload, "representations")) normalized.representations = Array.isArray(payload.representations) ? payload.representations.map(normalizeRepresentation) : payload.representations;
  ...
  if (hasOwn(payload, "relations")) normalized.relations = Array.isArray(payload.relations) ? payload.relations.map((relation) => isPlainObject(relation) ? { relationTypeKey: normalizeKey(relation.relationTypeKey), target: relation.target, weight: relation.weight === undefined ? 1 : Number(relation.weight) } : relation) : payload.relations;
  return normalized;
}

function validateRepresentationArray(representations, vocabulary, errors, field, { legacyDefaults = false } = {}) {
  ...
  const pairs = new Set(); let defaultCount = 0;
  representations.forEach((representation, index) => {
    const path = `${field}[${index}]`;
    if (!isPlainObject(representation)) { pushError(errors, path, "INVALID_TYPE", "Ogni representation deve essere un oggetto"); return; }
  ...
  if (legacyDefaults && defaultCount > 1) pushError(errors, field, "MULTIPLE_DEFAULTS", "E consentita una sola representation legacy di default");
}

function validateRepresentations(representations, vocabulary, errors, { requireDefault = false } = {}) {
  validateRepresentationArray(representations, vocabulary, errors, "representations", { legacyDefaults: true });
  if (requireDefault && Array.isArray(representations) && representations.filter((entry) => entry?.isDefault).length !== 1) pushError(errors, "representations", "DEFAULT_REQUIRED", "Per pubblicare una revisione legacy serve esattamente una representation di default");
}
...
async function validatePresentationVariants({ museumId, variants, defaultPresentation, vocabulary, errors, requireDefault = false, requirePublishedTargets = false }) {
  ...
  for (let index = 0; index < variants.length; index += 1) {
    const variant = variants[index]; const path = `presentationVariants[${index}]`;
    if (!isPlainObject(variant)) { pushError(errors, path, "INVALID_TYPE", "Ogni variante deve essere un oggetto"); continue; }
    ...
    for (let a = 0; a < (variant.presentationAspects || []).length; a += 1) {
      const aspect = variant.presentationAspects[a]; const aspectPath = `${path}.presentationAspects[${a}]`;
      if (!isPlainObject(aspect) || !aspect.key) { pushError(errors, aspectPath, "INVALID_TYPE", "PresentationAspect non valido"); continue; }
    ...
    for (let f = 0; f < (variant.semanticFocus || []).length; f += 1) {
      const focus = variant.semanticFocus[f]; const focusPath = `${path}.semanticFocus[${f}]`;
      if (!isPlainObject(focus) || !["item", "relation_type", "item_type", "canonical"].includes(focus.kind)) { pushError(errors, focusPath, "INVALID_SEMANTIC_FOCUS", "semanticFocus non valido"); continue; }
      ...
      if (focus.kind === "item") {
        if (!mongoose.isValidObjectId(focus.itemId)) { pushError(errors, `${focusPath}.itemId`, "INVALID_OBJECT_ID", "itemId non valido"); continue; }
...
async function validateRelations({ museumId, itemType, itemId, relations, vocabulary, errors, requirePublishedTargets = false }) {
  if (!Array.isArray(relations)) { pushError(errors, "relations", "INVALID_TYPE", "relations deve essere un array"); return; }
  const types = new Map(vocabulary.relationTypes.map((entry) => [entry.key, entry])); const seen = new Set(); const counts = new Map();
  for (let index = 0; index < relations.length; index += 1) {
    const relation = relations[index]; const path = `relations[${index}]`;
    if (!isPlainObject(relation)) { pushError(errors, path, "INVALID_TYPE", "Ogni relation deve essere un oggetto"); continue; }
    ...
    if (!mongoose.isValidObjectId(relation.target)) { pushError(errors, `${path}.target`, "INVALID_OBJECT_ID", "target deve essere un ObjectId valido"); continue; }
    if (String(relation.target) === String(itemId)) { pushError(errors, `${path}.target`, "SELF_RELATION", "Un item non puo riferirsi a se stesso"); continue; }
    const target = await Item.findOne({ _id: relation.target, lifecycleStatus: "active" }).lean();
    if (!target) { pushError(errors, `${path}.target`, "TARGET_NOT_FOUND", "L'item target non esiste o e nel cestino"); continue; }
    ...
    if (seen.has(duplicate)) pushError(errors, path, "DUPLICATE_RELATION", "Relazione duplicata"); seen.add(duplicate);
...
module.exports = { normalizeItemPayload, validateItemDraftPayload, validateRepresentations, validatePresentationVariants, validateRelations };
