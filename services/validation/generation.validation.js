...
function validateInterest(interest, field, errors) {
  if (!interest || typeof interest !== "object" || !INTEREST_KINDS.includes(interest.kind)) { errors.push(issue(field, "INVALID_INTEREST", "Interest non valido")); return; }
  ...
  if (interest.weight !== undefined && (!Number.isFinite(Number(interest.weight)) || Number(interest.weight) < -1 || Number(interest.weight) > 1)) errors.push(issue(`${field}.weight`, "INVALID_NUMBER", "weight deve essere tra -1 e 1"));
}
