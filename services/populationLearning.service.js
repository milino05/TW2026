...
async function refreshGlobalProfile() { const [movement, observation, calm, normal, fast, depth, language, density] = await Promise.all([aggregate("population_speed", "global"), aggregate("population_observation", "global"), aggregate("pace_factor", "global:calm"), aggregate("pace_factor", "global:normal"), aggregate("pace_factor", "global:fast"), aggregate("population_depth_preference", "global"), aggregate("population_language_complexity_preference", "global"), aggregate("population_visit_density", "global")]); return GlobalAdaptiveProfile.findOneAndUpdate({ key: "global" }, { $set: { movementSpeedMps: estimateFromAggregate(movement), observationSeconds: estimateFromAggregate(observation), "paceFactors.calm": estimateFromAggregate(calm), "paceFactors.normal": estimateFromAggregate(normal), "paceFactors.fast": estimateFromAggregate(fast), "presentation.depthPreference": estimateFromAggregate(depth), "presentation.languageComplexityPreference": estimateFromAggregate(language), "presentation.visitDensity": estimateFromAggregate(density) } }, { upsert: true, new: true, setDefaultsOnInsert: true }); }
...
function presentationSignals(session) { const events = session.interactionEvents || [], experiences = session.contentEntryExperiences || [], den = Math.max(1, experiences.length), count = (type) => events.filter((event) => event.type === type).length, depthUp = count("presentation_depth_increased"), depthDown = count("presentation_depth_decreased"), languageUp = count("presentation_language_increased"), languageDown = count("presentation_language_decreased"), optionalSkip = events.filter((event) => event.type === "content_entry_skipped" && event.metadata?.entryRole === "optional" && !["time_pressure", "accessibility", "interrupted"].includes(event.metadata?.skipReason)).length; const depthEvidence = depthUp + depthDown, languageEvidence = languageUp + languageDown, densityEvidence = depthUp + optionalSkip; return { depth: depthEvidence ? clamp01(0.5 + ((depthUp - depthDown) / den) * 0.35) : null, depthEvidence, language: languageEvidence ? clamp01(0.5 + ((languageUp - languageDown) / den) * 0.35) : null, languageEvidence, density: densityEvidence ? clamp01(0.5 + (depthUp / den) * 0.2 - (optionalSkip / den) * 0.3) : null, densityEvidence }; }
...
async function updatePopulationFromSession({ session, summary, personalObservationBase }) {
  ...
  const presentation = presentationSignals(session);
  if (presentation.depth !== null) await updateContributor({ userId, metricType: "population_depth_preference", scopeKey: "global", value: presentation.depth, sampleCount: presentation.depthEvidence });
  if (presentation.language !== null) await updateContributor({ userId, metricType: "population_language_complexity_preference", scopeKey: "global", value: presentation.language, sampleCount: presentation.languageEvidence });
  if (presentation.density !== null) await updateContributor({ userId, metricType: "population_visit_density", scopeKey: "global", value: presentation.density, sampleCount: presentation.densityEvidence });
...
module.exports = { refreshGlobalProfile, refreshMuseumProfile, presentationSignals, updatePopulationFromSession };
