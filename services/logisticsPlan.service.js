...
async function buildLogisticsPlan({ userId, visitId }) {
  ...
  for (let index = 0; index < revision.stops.length - 1; index += 1) {
    ...
    let route;
    let source = "dynamic";
    if (plannedIds.length && routeCompatible({ connections: layoutRevision.connections, pathConnectionIds: plannedIds, requirements: navigation.requirements })) {
      ...
      route = resolveRoute({ connections: layoutRevision.connections, fromPlaceId: fromPlacement.primaryPlaceId, toPlaceId: toPlacement.primaryPlaceId, requirements: navigation.requirements, speedMps: effectiveSpeed, learnedResidualByConnection, userCorrectionFactor: correctionFactor });
    }
...
module.exports = { mergeNavigationPreference, buildLogisticsPlan };
