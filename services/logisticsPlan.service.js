...
function mergeNavigationPreference(user, visitPreference, override = {}) { const global = user?.defaultNavigationPreference || {}; const local = visitPreference?.navigation || {}; return { movementPacePreference: Number.isFinite(Number(override.movementPacePreference)) ? Math.max(0, Math.min(1, Number(override.movementPacePreference))) : (Number.isFinite(local.movementPacePreference) ? local.movementPacePreference : (global.movementPacePreference ?? 0.5)), requirements: Array.isArray(override.requirements) ? override.requirements : (Array.isArray(local.requirements) && local.requirements.length ? local.requirements : (global.requirements || [])) }; }
function translateRequirements(layoutRevision, requirements = []) { const attributes = layoutRevision.routingAttributes || []; const byLocal = new Map(attributes.map((entry) => [entry.key, entry])); const byCanonical = new Map(attributes.filter((entry) => entry.canonicalKey).map((entry) => [entry.canonicalKey, entry])); const translated = []; const warnings = []; const unsupportedRequired = []; for (const requirement of requirements) { const local = byLocal.get(requirement.attributeKey) || byCanonical.get(requirement.attributeKey); if (!local) { if (requirement.priority === "required") unsupportedRequired.push(requirement.attributeKey); else warnings.push({ code: "PREFERRED_ATTRIBUTE_UNSUPPORTED", attributeKey: requirement.attributeKey }); continue; } translated.push({ ...requirement, attributeKey: local.key }); } return { requirements: translated, warnings, unsupportedRequired }; }
...
async function buildLogisticsPlan({ userId, visitId, navigationOverride = {} }) {
  const visit = await Visit.findOne({ _id: visitId, lifecycleStatus: "active", publishedRevisionId: { $ne: null } }).lean(); if (!visit) throw new AppError("Visita pubblicata non trovata", 404);
  const revision = await VisitRevision.findById(visit.publishedRevisionId).lean(); if (!revision) throw new AppError("Revisione pubblicata della visita non trovata", 404);
  ...
  for (const item of items) { if (!item) continue; const [itemProfile, population] = await Promise.all([ItemObservationProfile.findOne({ itemId: item._id }).lean(), populationForMuseum(item.museumId)]); observationSeconds += resolveObservationSeconds({ userProfile: adaptiveProfile, globalProfile: population.globalProfile, museumProfile: population.museumProfile, itemProfile }); }
  for (let index = 0; index < revision.stops.length - 1; index += 1) {
    const fromItem = items[index]; const toItem = items[index + 1]; if (!fromItem || !toItem) continue;
    ...
    const layoutRevision = await loadLayout(fromItem.museumId, configured?.layoutRevisionId); if (!layoutRevision) throw new AppError("Layout pubblicato non disponibile per una transizione indoor", 409);
    const translated = translateRequirements(layoutRevision, navigation.requirements); warnings.push(...translated.warnings.map((warning) => ({ ...warning, museumId: fromItem.museumId, transitionIndex: index })));
    ...
    const placements = placementMap(layoutRevision); const fromPlacement = placements.get(String(fromItem._id)); const toPlacement = placements.get(String(toItem._id)); if (!fromPlacement || !toPlacement) throw new AppError("Una tappa non ha una posizione nel layout pubblicato", 409);
  ...
  const estimatedBaseTotalSeconds = Math.max(0, contentSeconds + observationSeconds + logisticsSeconds);
  ...
  return { visitId: visit._id, visitRevisionId: revision._id, adaptivePolicyVersion: policy.version, presentationPlan, navigation, movementBaselineMps, paceFactor, effectiveMovementSpeedMps, observationBaselineSeconds, estimatedContentSeconds: Math.round(contentSeconds), estimatedObservationSeconds: Math.round(observationSeconds), estimatedLogisticsSeconds: Math.round(logisticsSeconds), estimatedBaseTotalSeconds: Math.round(estimatedBaseTotalSeconds), estimatedVisitResidualSeconds: Math.round(learnedVisitResidual), estimatedTotalSeconds: Math.round(Math.max(0, estimatedBaseTotalSeconds + learnedVisitResidual)), typicalVisitRange: timingProfile?.confidence >= policy.confidence.usableThreshold ? { lowerSeconds: timingProfile.lowerTypicalSeconds, upperSeconds: timingProfile.upperTypicalSeconds, confidence: timingProfile.confidence } : null, warnings, transitions };
}
