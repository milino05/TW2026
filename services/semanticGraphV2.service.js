...
function semanticRefKey(ref) {
  return `${normalizeKey(ref?.scheme)}::${String(ref?.id || ref?.refId || "").trim()}`;
}
...
function validateGraphSnapshotAgainstNamespace({ subjectBindings = [], edges = [] }, namespaceRevision) {
  ...
  const classesBySubject = new Map(subjectBindings.map((binding) => [
    String(binding.subjectId),
    new Set(binding.subjectClassDefinitionIds || []),
  ]));
  ...
  subjectBindings.forEach((binding, index) => {
    (binding.subjectClassDefinitionIds || []).forEach((definitionId, classIndex) => {
      if (!classIds.has(String(definitionId))) {
        issues.push({
          field: `subjectBindings[${index}].subjectClassDefinitionIds[${classIndex}]`,
          code: "UNKNOWN_SUBJECT_CLASS",
          message: `SubjectClass non disponibile: ${definitionId}`,
        });
      }
    });
  });
  ...
  edges.forEach((edge, index) => {
    if (!boundSubjectIds.has(String(edge.sourceSubjectId))) {
      issues.push({
        field: `edges[${index}].sourceSubjectId`,
        code: "GRAPH_SUBJECT_BINDING_REQUIRED",
        message: "Il Subject sorgente deve appartenere al grafo",
      });
    }
    if (!boundSubjectIds.has(String(edge.targetSubjectId))) {
      issues.push({
        field: `edges[${index}].targetSubjectId`,
        code: "GRAPH_SUBJECT_BINDING_REQUIRED",
        message: "Il Subject destinazione deve appartenere al grafo",
      });
    }
    const relation = relationById.get(String(edge.relationTypeDefinitionId));
    if (!relation) {
      issues.push({
        field: `edges[${index}].relationTypeDefinitionId`,
        code: "UNKNOWN_RELATION_TYPE",
        message: `RelationType non disponibile: ${edge.relationTypeDefinitionId}`,
      });
      return;
    }
    ...
    if (domain.length && !domain.some((definitionId) => sourceClasses.has(String(definitionId)))) {
      issues.push({
        ...
        message: `Il Subject sorgente non soddisfa il domain di ${relation.key}`,
      });
    }
    if (range.length && !range.some((definitionId) => targetClasses.has(String(definitionId)))) {
      issues.push({
        ...
        message: `Il Subject destinazione non soddisfa il range di ${relation.key}`,
      });
    }
  });
...
async function loadSemanticGraphRevision(graphRevisionId, { namespaceRevisionId = null, bypassCache = false } = {}) {
  ...
  persistedEdges.forEach((edge) => {
    subjectIds.add(id(edge.sourceSubjectId));
    subjectIds.add(id(edge.targetSubjectId));
  });
  ...
  const relationTypeByDefinitionId = new Map((namespaceRevision.relationTypes || []).map((entry) => [
...
function resolveFeatureToSubjectIds(graph, feature = {}) {
  ...
  for (const [subjectId, node] of graph.nodes) {
    if (feature.kind === "subject_class" && (node.binding?.subjectClassDefinitionIds || []).includes(String(feature.definitionId))) {
      result.push(subjectId);
    } else if (feature.kind === "relation_type"
      && (graph.edgesFrom.get(subjectId) || []).some((edge) => edge.relationTypeDefinitionId === String(feature.definitionId))) {
      result.push(subjectId);
    }
  }
  return result;
...
function neighbors(graph, subjectId, { relationTypeDefinitionId = null } = {}) {
  return (graph?.edgesFrom.get(id(subjectId)) || []).filter((edge) => (
    !relationTypeDefinitionId || edge.relationTypeDefinitionId === String(relationTypeDefinitionId)
  ));
}

function outgoingEdges(graph, subjectId) {
  return neighbors(graph, subjectId).filter((edge) => !edge.generated);
}

function incomingEdges(graph, subjectId) {
  return neighbors(graph, subjectId).filter((edge) => edge.generated);
}
...
function shortestSemanticPath(graph, { from, to, relationTypeDefinitionId = null, maxDepth = 3 } = {}) {
  ...
  for (const start of starts) {
    if (goals.has(start)) return { subjectIds: [start], edges: [], depth: 0, strength: 1 };
  }
  ...
  while (queue.length) {
    ...
    for (const edge of neighbors(graph, current.subjectId, { relationTypeDefinitionId })) {
      ...
      const next = {
        ...
        strength: current.strength * Math.max(0.05, edge.traversalWeight || 0.05),
      };
      if (goals.has(nextId)) {
        return {
