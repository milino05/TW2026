...
function explicitPresentationPreference(payload = {}) {
  if (!payload.presentationPreference) return null;
  return payload.presentationPreference;
}
...
async function startFromSource({ userId, source, payload = {} }) {
  ...
  const session = await VisitSessionV2.create({
    userId,
    sourceType: source.origin.sourceType,
    visitId: source.visitId || null,
    visitRevisionId: source.visitRevisionId || null,
    generatedVisitPlanId: source.generatedVisitPlanId || null,
...
async function startVisitSessionV2({ userId, visitId, payload = {} }) {
  return startFromSource({ userId, source: await visitSourceSnapshotV2({ userId, visitId }), payload });
}
async function startGeneratedPlanSessionV2({ userId, planId, payload = {} }) {
  return startFromSource({ userId, source: await generatedSourceSnapshotV2({ userId, planId }), payload });
}
...
function effectivePresentation(session, entry) {
  ...
  return override ? {
    ...
    estimatedContentSeconds: entry.baselinePresentation.estimatedContentSeconds,
...
async function currentSessionProjection({ sessionId, userId }) {
  ...
  const index = Math.min(Math.max(0, Number(session.currentEntryIndex) || 0), entries.length - 1);
  const entry = entries[index];
  const runtime = await entryRuntimeData(session, entry);
  const anchor = effectiveAnchorForIndex(plan, index);
  const nextEntry = entries[index + 1] || null;
  const nextAnchorId = nextEntry?.deliveryAnchorId || null;
  ...
  return {
    ...
    current: {
      ...
      anchor: anchor ? { id: anchor._id, venueTargetId: anchor.venueTargetId, venueId: anchor.venueId, placeId: anchor.placeId } : null,
      nextPhysicalLeg: nextLeg,
    },
...
async function changePresentationAxis({ sessionId, userId, axis, direction }) {
  ...
  const value = { contentEntryId: entry._id, variantId: next.variantId, representationId: next.representationId, durationTypeDefinitionId: next.durationTypeDefinitionId, languageLevelDefinitionId: next.languageLevelDefinitionId, locale: next.locale, updatedAt: new Date() };
...
async function recordContentEntryExperience({ sessionId, userId, payload = {} }) {
  ...
  const current = effectivePresentation(session, entry);
  const contentSeconds = Number(payload.contentSeconds ?? current.estimatedContentSeconds ?? entry.baselinePresentation.estimatedContentSeconds);
...
async function routeToIntentV2({ sessionId, userId, payload = {} }) {
  ...
  const currentAnchor = effectiveAnchorForIndex(plan, session.currentEntryIndex);
  const venueId = payload.venueId || currentAnchor?.venueId;
  const fromPlaceId = payload.fromPlaceId || currentAnchor?.placeId;
