...
const ORIGINS = new Set(["human", "ai_assisted", "ai_generated", "imported", "forked"]);
...
async function ensureScope({ scopeKey, item, namespace, actorUserId }) {
  ...
  if (!context) {
    context = await EditorialContext.findOne({
    ...
    if (context) {
      ...
      context.trashedAt = null;
      context.trashedBy = null;
      context.workingGraphRevisionId = null;
      context.publishedReleaseId = null;
      await context.save();
    } else {
...
async function createItemConnection({ itemId, editionId, payload, actorUserId }) {
  ...
  if (!Number.isFinite(weight) || weight < 0 || weight > 10) throw new AppError("L’importanza deve essere compresa fra 0 e 10", 400);
  const origin = String(payload?.provenanceOrigin || "human");
  if (!ORIGINS.has(origin)) throw new AppError("La provenienza del collegamento non è valida", 400);
  const note = String(payload?.note || "").trim();
  if (note.length > 1000) throw new AppError("La nota può contenere al massimo 1000 caratteri", 400);
  snapshot.edges.push({
    sourceSubjectId,
    targetSubjectId,
...
async function removeItemConnection({ itemId, editionId, connectionId, contextId, actorUserId }) {
  const { item, edition, namespace, namespaceRevision } = await loadEditionContext({ itemId, editionId, actorUserId });
  const scopes = await ownerScopes({ item, namespace });
