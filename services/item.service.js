...
async function createWorkingRevisionFromPublished(item, actorUserId) {
  ...
  try {
    await cloneRevisionSemanticEdges({ museumId: item.museumId, sourceItemId: item._id, fromRevisionId: published._id, toRevisionId: revision._id });
    item.workingRevisionId = revision._id;
    await item.save();
    return revision;
  } catch (error) {
    await SemanticEdge.deleteMany({ sourceItemRevisionId: revision._id }).catch(() => {});
...
async function createItem({ museumId, payload, userId }) {
  ...
  const vocabulary = await getMuseumVocabulary(museumId), normalized = normalizedPayload(payload), errors = await validateDraft({ museumId, itemType: normalized.itemType, payload: normalized, vocabulary, mode: "create" });
  if (errors.length) throw new AppError("Payload non valido", 400, errors);
  ...
  try {
    ...
    revision = await ItemRevision.create({ itemId: item._id, version: 1, ...revisionData, status: "draft", integrity: { status: "needs_review", issues: [] }, createdBy: userId, updatedBy: userId });
    const semanticEdges = normalized.semanticEdges || [], edgeErrors = await validateDraft({ museumId, itemId: item._id, itemType: item.itemType, payload: { semanticEdges }, vocabulary, mode: "update" });
    if (edgeErrors.length) throw new AppError("Payload non valido", 400, edgeErrors);
    const edges = await replaceRevisionSemanticEdges({ museumId, sourceItemId: item._id, sourceItemRevisionId: revision._id, semanticEdges });
    item.workingRevisionId = revision._id; await item.save();
    return { item, revision, semanticEdges: edges.map(edgeSnapshot) };
  } catch (error) {
    if (revision) await SemanticEdge.deleteMany({ sourceItemRevisionId: revision._id }).catch(() => {});
    if (revision) await revision.deleteOne().catch(() => {});
    await item.deleteOne().catch(() => {});
    throw error;
  }
}

async function restoreDraftState({ item, revision, itemBefore, revisionBefore, edgesBefore, museumId }) {
  await Promise.all([
    Item.updateOne({ _id: item._id }, { $set: { externalId: itemBefore.externalId ?? null, itemType: itemBefore.itemType } }),
    ItemRevision.updateOne({ _id: revision._id }, { $set: { ...revisionBefore.payload, ...revisionBefore.workflow } }),
  ]);
  await replaceRevisionSemanticEdges({ museumId, sourceItemId: item._id, sourceItemRevisionId: revision._id, semanticEdges: edgesBefore });
}

async function updateItem({ museumId, itemId, payload, userId }) {
  await assertMuseumRole({ userId, museumId, minimumRole: "operator" }); rejectManagedFields(payload);
  const item = await findItemOrFail({ museumId, itemId }), revision = await getWorkingRevision(item, userId);
  const existingEdges = await getRevisionSemanticEdges(revision._id), edgesBefore = existingEdges.map(edgeSnapshot), itemBefore = { externalId: item.externalId, itemType: item.itemType }, revisionBefore = { payload: revisionSnapshot(revision), workflow: workflowSnapshot(revision) };
  ...
  const effectiveItemType = hasOwn(payload, "itemType") ? normalized.itemType : item.itemType, merged = mergeRevisionPayload(revision, payload, normalized), effectiveEdges = hasOwn(payload, "semanticEdges") ? normalized.semanticEdges : edgesBefore, vocabulary = await getMuseumVocabulary(museumId);
  const errors = await validateDraft({ museumId, itemId, itemType: effectiveItemType, payload: { ...merged, itemType: effectiveItemType, semanticEdges: effectiveEdges }, vocabulary, mode: "create" });
  if (errors.length) throw new AppError("Payload non valido", 400, errors);
  ...
  Object.assign(revision, merged); revision.updatedBy = userId;
  try {
    await Promise.all([revision.save(), item.save()]);
    const edges = hasOwn(payload, "semanticEdges") ? await replaceRevisionSemanticEdges({ museumId, sourceItemId: item._id, sourceItemRevisionId: revision._id, semanticEdges: effectiveEdges }) : existingEdges;
    invalidateMuseumSemanticGraphCache(museumId);
    return { item, revision, semanticEdges: edges.map(edgeSnapshot) };
  } catch (error) {
    try { await restoreDraftState({ item, revision, itemBefore, revisionBefore, edgesBefore, museumId }); }
    catch (rollbackError) { throw new AppError("Aggiornamento Item fallito e rollback incompleto", 500, [{ code: "ITEM_GRAPH_ROLLBACK_FAILED", message: rollbackError.message }, { code: "ORIGINAL_ERROR", message: error.message }]); }
    throw error;
  }
}
...
async function getItemById({ museumId, itemId, actorUserId = null, view = "published" }) {
  const item = await findItemOrFail({ museumId, itemId, includeTrashed: view === "working" }), canManage = await actorCanManage(actorUserId, museumId);
  if (view === "working" && !canManage) throw new AppError("Accesso alla revisione di lavoro non autorizzato", 403);
  const revisionId = view === "working" && item.workingRevisionId ? item.workingRevisionId : item.publishedRevisionId;
  if (!revisionId) throw new AppError("Nessuna revisione disponibile", 404);
  const revision = await ItemRevision.findById(revisionId); if (!revision) throw new AppError("Revisione non trovata", 404);
  if (!canManage && revision.integrity.status !== "valid") throw new AppError("Item non disponibile", 404);
  const semanticEdges = await getRevisionSemanticEdges(revision._id);
  return { item, revision, semanticEdges: semanticEdges.map(edgeSnapshot) };
}
async function listItems({ museumId, filters = {}, actorUserId = null, view = "published" }) {
  ...
  const selected = items.map((item) => ({ item, revisionId: view === "working" && item.workingRevisionId ? item.workingRevisionId : item.publishedRevisionId })).filter((entry) => entry.revisionId);
  const revisionIds = selected.map((entry) => entry.revisionId), [revisions, edgesByRevision] = await Promise.all([ItemRevision.find({ _id: { $in: revisionIds } }).lean(), getRevisionSemanticEdgesMap(revisionIds)]), revisionById = new Map(revisions.map((revision) => [String(revision._id), revision])), results = [];
  for (const entry of selected) {
    const revision = revisionById.get(String(entry.revisionId)); if (!revision) continue;
...
async function hardDeleteItem({ museumId, itemId, userId }) {
  ...
  await deleteSemanticEdgesForSourceItem(item._id); await ItemRevision.deleteMany({ itemId: item._id }); await item.deleteOne(); invalidateMuseumSemanticGraphCache(museumId); return item;
}
