...
const REVISION_FIELDS = ["label", "recognitionImage", "tags", "metadata", "semanticRefs", "selectionSignals", "relations", "presentationVariants", "defaultPresentation", "jsonld"];
...
async function validateDraft({ museumId, itemId = null, itemType, payload, vocabulary, mode }) { const errors = await validateItemDraftPayload({ museumId, itemId, itemType, payload, vocabulary, mode }); if (Array.isArray(payload.selectionSignals)) { const keys = new Set((vocabulary.selectionSignals || []).map((entry) => entry.key)); const seen = new Set(); payload.selectionSignals.forEach((signal, index) => { const path = `selectionSignals[${index}]`; if (!signal || typeof signal !== "object" || !keys.has(signal.key)) errors.push({ field: `${path}.key`, code: "UNKNOWN_SELECTION_SIGNAL", message: `SelectionSignal non presente nel vocabolario: ${signal?.key || ""}` }); if (seen.has(signal?.key)) errors.push({ field: `${path}.key`, code: "DUPLICATE_KEY", message: "SelectionSignal duplicato" }); seen.add(signal?.key); if (!Number.isFinite(signal?.weight) || signal.weight < 0 || signal.weight > 1) errors.push({ field: `${path}.weight`, code: "INVALID_NUMBER", message: "weight deve essere tra 0 e 1" }); }); } return errors; }
...
async function createItem({ museumId, payload, userId }) {
  ...
  const errors = await validateDraft({ museumId, itemType: normalized.itemType, payload: normalized, vocabulary, mode: "create" }); if (errors.length) throw new AppError("Payload non valido", 400, errors);
  const item = await Item.create({ externalId: normalized.externalId, museumId, itemType: normalized.itemType, createdBy: userId });
  try { const revisionData = Object.fromEntries(REVISION_FIELDS.filter((field) => normalized[field] !== undefined).map((field) => [field, normalized[field]])); const revision = await ItemRevision.create({ itemId: item._id, version: 1, ...revisionData, status: "draft", integrity: { status: "needs_review", issues: [] }, createdBy: userId, updatedBy: userId }); item.workingRevisionId = revision._id; await item.save(); return { item, revision }; } catch (error) { await item.deleteOne().catch(() => {}); throw error; }
}
async function updateItem({ museumId, itemId, payload, userId }) {
  ...
  const errors = await validateDraft({ museumId, itemId, itemType: effectiveItemType, payload: { ...merged, itemType: effectiveItemType }, vocabulary, mode: "create" }); if (errors.length) throw new AppError("Payload non valido", 400, errors);
  if (hasOwn(payload, "externalId")) item.externalId = normalized.externalId; if (hasOwn(payload, "itemType")) item.itemType = normalized.itemType; Object.assign(revision, merged); revision.updatedBy = userId; await Promise.all([revision.save(), item.save()]); return { item, revision };
}
...
async function hardDeleteItem({ museumId, itemId, userId }) { await assertMuseumRole({ userId, museumId, minimumRole: "manager" }); const item = await findItemOrFail({ museumId, itemId, includeTrashed: true }); if (item.lifecycleStatus !== "trashed") throw new AppError("L'item deve essere nel cestino prima della cancellazione definitiva", 409); const [visitDependency, relationDependency, focusDependency] = await Promise.all([VisitRevision.exists({ "contentEntries.itemId": item._id }), ItemRevision.exists({ "relations.target": item._id }), ItemRevision.exists({ "presentationVariants.semanticFocus.itemId": item._id })]); if (visitDependency || relationDependency || focusDependency) throw new AppError("Impossibile eliminare definitivamente: esistono dipendenze attive", 409); await ItemRevision.deleteMany({ itemId: item._id }); await item.deleteOne(); return item; }
