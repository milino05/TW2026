...
function rejectManagedFields(payload = {}) {
  ...
  const errors = forbidden.filter((field) => hasOwn(payload, field)).map((field) => ({
    field,
    code: field === "representations" ? "REMOVED_FIELD" : "FORBIDDEN_FIELD",
    message: field === "representations"
      ? "representations non e piu un campo diretto: usare presentationVariants"
      : `${field} e gestito dal backend`,
  }));
  if (hasOwn(payload, "relationCommands")) errors.push({
    field: "relationCommands",
    code: "REMOVED_FIELD",
    message: "Le relazioni si modificano tramite l'array relations della revisione",
  });
...
async function createItem({ museumId, payload, userId }) {
  ...
  if (errors.length) throw new AppError("Payload non valido", 400, errors);
  const item = new Item({ externalId: normalized.externalId, museumId, itemType: normalized.itemType, createdBy: userId });
  await item.save();
  try {
...
async function updateItem({ museumId, itemId, payload, userId }) {
  ...
  const errors = await validateItemDraftPayload({
    ...
    payload: { ...merged, itemType: effectiveItemType },
    vocabulary,
    mode: "create",
  });
  if (errors.length) throw new AppError("Payload non valido", 400, errors);
...
async function trashItem({ museumId, itemId, userId }) {
  ...
  await item.save();
  await invalidateVisitsUsingItem({ itemId: item._id, code: "VISIT_ITEM_TRASHED", message: "Un item della visita e stato spostato nel cestino", blocking: true });
  return item;
}
...
async function hardDeleteItem({ museumId, itemId, userId }) {
  ...
  const [visitDependency, relationDependency, focusDependency] = await Promise.all([
