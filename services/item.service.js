...
function normalizeSelectionSignals(values) {
  if (!Array.isArray(values)) return values;
  return values.map((signal) => signal && typeof signal === "object" ? { key: String(signal.key || "").trim().toLowerCase(), weight: signal.weight === undefined ? 1 : Number(signal.weight) } : signal);
}
function normalizedPayload(payload) {
  const normalized = normalizeItemPayload(payload);
  if (hasOwn(payload, "selectionSignals")) normalized.selectionSignals = normalizeSelectionSignals(payload.selectionSignals);
  return normalized;
}
...
async function createWorkingRevisionFromPublished(item, actorUserId) {
  ...
  try {
    ...
    await item.save();
    invalidateMuseumSemanticGraphCache(item.museumId, { view: "working" });
    return revision;
  } catch (error) {
...
async function validateDraft({ museumId, itemId = null, itemType, payload, vocabulary, mode }) {
  ...
  if (Array.isArray(payload.selectionSignals)) {
    const keys = new Set((vocabulary.selectionSignals || []).map((entry) => entry.key));
    const seen = new Set();
...
async function createItem({ museumId, payload, userId }) {
  ...
  const errors = await validateDraft({ museumId, itemType: normalized.itemType, payload: normalized, vocabulary, mode: "create" });
  if (errors.length) throw new AppError("Payload non valido", 400, errors);
  ...
  try {
    ...
    const semanticEdges = normalized.semanticEdges || [];
    const edgeErrors = await validateDraft({ museumId, itemId: item._id, itemType: item.itemType, payload: { semanticEdges }, vocabulary, mode: "update" });
    ...
    item.workingRevisionId = revision._id;
    await item.save();
    invalidateMuseumSemanticGraphCache(museumId, { view: "working" });
...
async function updateItem({ museumId, itemId, payload, userId }) {
  ...
  const existingEdges = await getRevisionSemanticEdges(revision._id);
  const effectiveEdges = hasOwn(payload, "semanticEdges") ? normalized.semanticEdges : existingEdges.map(edgeSnapshot);
  const vocabulary = await getMuseumVocabulary(museumId);
  ...
  const edges = hasOwn(payload, "semanticEdges")
    ? await replaceRevisionSemanticEdges({ museumId, sourceItemId: item._id, sourceItemRevisionId: revision._id, semanticEdges: effectiveEdges })
    : existingEdges;
  invalidateMuseumSemanticGraphCache(museumId, { view: "working" });
  return { item, revision, semanticEdges: edges.map(edgeSnapshot) };
}
...
async function listItems({ museumId, filters = {}, actorUserId = null, view = "published" }) {
  ...
  for (const item of items) {
    ...
    results.push({ item, revision, semanticEdges: semanticEdges.map(edgeSnapshot) });
  }
  return results;
}
...
async function hardDeleteItem({ museumId, itemId, userId }) {
  ...
  const [visitDependency, edgeDependency, focusDependency] = await Promise.all([
    VisitRevision.exists({ "contentEntries.itemId": item._id }),
    semanticEdgeTargetDependency(item._id),
    ItemRevision.exists({ "presentationVariants.semanticFocus.itemId": item._id }),
  ]);
