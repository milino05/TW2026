...
function assertPreferenceValue(value, field) {
  if (!Number.isFinite(value) || value < 0 || value > 1) throw new TypeError(`${field} deve essere un numero compreso tra 0 e 1`);
}
...
function findRepresentationByPolicy(source, policy = {}, options = {}) {
  ...
  const preferredVariant = options.variantKey || policy.variantKey || source?.defaultPresentation?.variantKey || null;
  const candidate = candidates.find((entry) =>
    (!preferredVariant || entry.variantKey === preferredVariant) &&
    entry.representation.durationKey === policy.durationKey &&
    entry.representation.languageLevelKey === policy.languageLevelKey,
  ) || null;
  return decorate(candidate);
}
function resolveOfficialRepresentation({ source, defaultPolicy, preference = null, variantKey = null }) {
  const effectivePolicy = preference?.mode === "custom"
    ? { durationKey: preference.durationKey, languageLevelKey: preference.languageLevelKey, variantKey: preference.variantKey }
    : defaultPolicy;
  return findRepresentationByPolicy(source, effectivePolicy || {}, { variantKey }) || findDefaultRepresentation(source);
}
function resolveCommunityRepresentation({ source, durationTypes = [], languageLevels = [], preference, variantScores = new Map() }) {
  const candidates = listRepresentationCandidates(source);
  const fallback = getDefaultCandidate(source);
  ...
  const scored = candidates.map((candidate, sourceIndex) => {
    const depth = durationPositions.get(candidate.representation.durationKey);
    const language = languagePositions.get(candidate.representation.languageLevelKey);
    ...
    const languageDistance = Math.abs(language - preference.languageComplexityPreference);
...
function findAdjacentRepresentation({ source, durationTypes = [], languageLevels = [], currentRepresentation, axis, direction }) {
  if (!currentRepresentation || !["duration", "language"].includes(axis) || !["up", "down"].includes(direction)) return null;
  ...
  const currentIndex = vocabulary.findIndex((entry) => entry.key === currentRepresentation[keyField]);
  if (currentIndex < 0) return null;
  const candidates = listRepresentationCandidates(source).filter((candidate) => candidate.variantKey === currentRepresentation.variantKey);
  const step = direction === "up" ? 1 : -1;
  for (let index = currentIndex + step; index >= 0 && index < vocabulary.length; index += step) {
    const candidate = candidates.find((entry) =>
      entry.representation[keyField] === vocabulary[index].key &&
      entry.representation[fixedField] === currentRepresentation[fixedField],
    );
...
function estimateContentSeconds({ selections = [], vocabularyByMuseumId = new Map() }) {
  return selections.reduce((total, selection) => {
    ...
    const seconds = buildDurationSecondsMap(vocabulary.durationTypes).get(selection.representation?.durationKey);
