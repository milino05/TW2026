...
La navigation preference distingue `movementPacePreference` astratta e routing requirements. Il client non manipola velocità fisica, connections, Dijkstra o path. I `routingProfiles` del `PhysicalVocabularyRevision` pinzato sono l'interfaccia user-facing principale; gli attributi configurabili vengono proiettati soltanto con label, descrizione e controlli tipizzati comprensibili, non come requirement tecnici grezzi.

Key e UUID locali non diventano preference globali persistenti. Le preference globali usano `PhysicalFeatureRef.semantic`; il mapping verso i `definitionId` della revisione adottata resta backend-side. Requirement `required` non supportati o route incompatibili producono blocker/readiness outcome; preference non supportate producono warning quando l’esecuzione resta possibile. Il client non elimina silenziosamente vincoli e non calcola percorsi alternativi.
...
# Punto 15/30 — Navigation multi-Venue e PhysicalFeatureRef

Le navigation preference persistenti/globali usano requirement con `PhysicalFeatureRef.semantic`, cosi da restare provider-neutral e riutilizzabili fra Venue che adottano vocabolari diversi. I riferimenti locali sono validi soltanto nel contesto della `PhysicalVocabularyRevision` pinzata e non diventano default globali. `UserPreferenceService` valida forma, operatori e valori senza dipendere da un catalogo platform-level.

Il backend risolve ogni riferimento semantico separatamente contro il `PhysicalVocabularyRevision` pinzato dal Layout corrente di ciascuna Venue. La corrispondenza richiede lo stesso identificatore provider-neutral; una risoluzione assente o ambigua non viene indovinata per label. Un requirement globale `required` deve essere supportato e soddisfatto in tutte le Venue o segmenti applicabili; l'assenza di supporto produce un readiness blocker. Un requirement soft viene applicato dove supportato e produce warning altrove.

I routing profile restano legati alla revisione del vocabolario e in una Visit multi-Venue non vengono unificati per key o label. Il `PreparationDraft` distingue override globali da selezioni locali per Venue. I requirement globali `required` non possono essere indeboliti da profili locali; conflitti hard vengono rifiutati invece di essere risolti silenziosamente. Una stessa `semanticRef` deve risolversi a una sola definizione applicabile nella revisione, altrimenti la traduzione e ambigua.
...
Per le `ContentSource` dirette di tipo `item_revision`, lo snapshot semantico della Session non dipende da una `EditorialRelease` artificiale e il runtime non consulta un grafo live mutevole. Durante la preparation/start il backend risolve, entro l'authority del principal proprietario della Visit, gli `EditorialContext` applicabili agli Item diretti e pinna esplicitamente le `SemanticGraphRevision` immutabili necessarie nel `SessionPlan` (`semanticGraphPins`). Gli Item owned con revisioni pubblicate che costituiscono destinazioni utilizzabili di tali relazioni vengono pinzati separatamente come `semanticContentPins`: rimangono contenuti di approfondimento e non diventano `ContentEntry`, `VisitAnchor` o tappe della Visit. Una sorgente diretta acquisita non autorizza implicitamente l'accesso al working graph privato del venditore; per relazioni semantiche esterne serve uno snapshot/sorgente esplicitamente autorizzato.
...
# Marketplace/Editor UX — Visit authoring workflow v2

Questa decisione **sostituisce integralmente** la precedente formulazione del Visit authoring workflow che separava `Contenuti` e `Tappe` in due step obbligatori.

- il flusso user-facing canonico è **Informazioni principali → Costruisci la visita → Impostazioni → Percorso → Pubblicazione**. `ContentEntry`, `ContentSource`, `VisitAnchor`, `VenueTarget`, `deliveryAnchorId` e `VisitRevision` restano concetti di dominio/progressive disclosure e non strutturano il percorso principale dell’autore;
- **Costruisci la visita** è un unico composer search-first: a sinistra l’autore cerca tra tutti i contenuti che il principal corrente può usare nella visita; a destra vede la sequenza in costruzione, aggiunge/rimuove contenuti e ne modifica ordine e importanza. La raccolta editoriale/source resta provenance e filtro facoltativo, non uno scope obbligatorio della Visit;
- la candidate projection è backend-authoritative e unifica contenuti owned e contenuti utilizzabili tramite capability/Entitlement, inclusi contenuti provenienti da raccolte differenti. Il client non costruisce l’universo dei candidati interrogando separatamente raccolte, licenze o Item raw;
- l’azione primaria di una card è **Aggiungi**. Il ruolo iniziale può usare `recommended` come default e `core | recommended | optional` viene modificato dopo l’aggiunta senza introdurre frizione nel percorso principale;
- `contentEntries[]` e `visitAnchors[]` restano strutture di dominio separate. La semplificazione della UI non trasforma la Visit in un array piatto di testi: gli Item restano contenuti e le indicazioni/logistica restano fuori dagli Item;
- quando un contenuto viene aggiunto, il backend può inferire il suo delivery fisico senza obbligare l’utente a un secondo step: una singola occorrenza fisica applicabile può creare/riusare automaticamente il relativo `VisitAnchor`; occorrenze multiple richiedono una scelta esplicita; in assenza di un’occorrenza fisica il contenuto resta contestuale con `deliveryAnchorId = null`. L’autore può successivamente usare **Presenta in…** per associare o spostare esplicitamente il contenuto;
- il PhysicalScope continua a derivare esclusivamente dai `VisitAnchor` effettivamente presenti. Venue/source selector sono strumenti di ricerca e authoring, non campi persistenti che definiscono artificialmente lo scope della Visit;
- la colonna **La tua visita** raggruppa visivamente i contenuti per `deliveryAnchorId`, mostrando il target fisico come metadato user-facing. I contenuti contestuali vengono mostrati nello stesso composer come gruppo privo di tappa specifica, senza una seconda schermata separata;
- il riordino ha due semantiche distinte. Trascinare un **blocco/tappa fisica** riordina i `VisitAnchor` e quindi i blocchi di contenuti; trascinare un **contenuto** riordina soltanto i `ContentEntry` che appartengono allo stesso `deliveryAnchorId`. Il drag di un contenuto fra tappe diverse non viene interpretato come semplice reorder perché modificherebbe il delivery fisico: questa operazione resta esplicita tramite attach/detach/“Presenta in…”;
- drag & drop non è l’unico meccanismo disponibile: i controlli **Sposta prima/Sposta dopo** o equivalenti restano fallback da tastiera/touch e rendono il riordino testabile e accessibile;
- la sequenza persistente e la sequenza runtime devono essere coerenti. Il backend usa una canonicalizzazione condivisa: l’ordine dei `VisitAnchor` determina l’ordine dei blocchi fisici, mentre l’ordine relativo dei `ContentEntry` dentro ciascun blocco viene preservato. La costruzione del `SessionPlan` usa la stessa canonicalizzazione, impedendo divergenze fra ordine mostrato nell’Editor e ordine realmente eseguito dal Navigator;
- una tappa senza contenuti rimane una capacità valida del dominio ma secondaria nel workflow: viene esposta sotto **Opzioni avanzate** e non occupa uno step obbligatorio. La rimozione di un Anchor continua a rispettare i riferimenti di ContentEntry e RouteHint invece di riparare silenziosamente il grafo nel frontend;
- `presentationBaseline` rimane namespace-neutral e viene presentata nello step **Impostazioni** come preferenza facoltativa di profondità, complessità linguistica e locale; il mapping verso Representation concrete resta backend-side;
- **Percorso** verifica e configura gli aspetti fisici/logistici derivati dalla sequenza costruita. `preVisitNotes` e `routeHints` restano dominio logistico distinto dagli Item/ContentEntry e il routing indoor continua a essere responsabilità del Physical Domain;
- integrity e workflow restano backend-authoritative. **Pubblicazione** mostra riepilogo, blocker e operazioni disponibili, ma soltanto `availableOperations[]` determina check, review, richiesta modifiche e publication; la publication editoriale della Visit resta distinta dalla creazione di un MarketplaceListing;
- modificare una Visit pubblicata continua a usare il normale boundary che crea una working revision al primo write; il client non clona revisioni e non muta snapshot pubblicati;
- il composer deve preservare ricerca/paginazione server-side, filtri facoltativi, multi-Venue, contenuti contestuali, occurrence multiple, ruoli, riordino/rimozione, logistica strutturata e capability autorizzate, mantenendo il percorso principale **cerca → aggiungi → ordina → continua**.
